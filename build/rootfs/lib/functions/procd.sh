...
#	ubus call service "$cmd" "$(json_dump)"   
    if [ "$EXECENV_INDEX" != "" ];then
    ubus call SoftwareModules.Service.$EXECENV_INDEX "$cmd" "$(json_dump)"
else
    ubus call SoftwareModules.Service.1 "$cmd" "$(json_dump)"
fi
	json_cleanup
}
...
_procd_add_event_trigger() {
	json_add_array
	_procd_add_array_data "event.$1"
	shift
	local timeout=$1
	shift
	json_add_array
...
_procd_kill() {
	...
	_procd_ubus_call delete_instance
}
...
_procd_set_config_changed() {
    ...
    if [ "$EXECENV_INDEX" != "" ];then
	ubus call SoftwareModules.Service.$EXECENV_INDEX event "$(json_dump)"
else
	ubus call SoftwareModules.Service.1 event "$(json_dump)"
fi
}
...
_procd_wrapper \
	...
	procd_add_event_trigger \
	procd_add_event_name_trigger \
	procd_open_trigger \
	procd_close_trigger \
	procd_open_instance \
	procd_close_instance \
