...
//**************************************************************************************
#ifdef __SC_BUILD__
void setLed_ex (short led_gpio, unsigned short led_state)
{
...
#ifdef __SC_BUILD__
// toggle the Red LED
static void redLedToggle(PLED_CTRL pLed)
{
    short led_gpio;
    short red_led_gpio;
...
static void ledTimerExpire(void)
{
    ...
    for (i = 0, pCurLed = gLedCtrl; i < kLedEnd; i++, pCurLed++)
    {
        ...
        case kLedStateFastBlinkContinues:
            ...
            break;
#ifdef __SC_BUILD__
        case kRedLedStateFastBlinkContinues:
            if (pCurLed->blinkCountDown-- == 0)
            {
                pCurLed->blinkCountDown = kFastBlinkCount;
                redLedToggle(pCurLed);
            }
            gTimerOnRequests++;
            spin_unlock_irqrestore(&brcm_ledlock, flags);
            break;
