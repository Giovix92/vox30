...
void BcmReloadWdTimer(ulong timeUs)
{
    if(!watchdog_fired)
    {
        TIMER->WatchDogDefCount = timeUs * (FPERIPH/1000000);
...
EXPORT_SYMBOL(BcmStartWdTimer);
EXPORT_SYMBOL(BcmSetWdTimerFired);
EXPORT_SYMBOL(BcmReloadWdTimer);
EXPORT_SYMBOL(BcmStopWdTimer);

EXPORT_SYMBOL(kerSysBlParmsGetSecKey);
#endif
