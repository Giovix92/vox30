...
#ifdef __SC_BUILD__
#define SC_FW_VERSION_BUFFER_LEN (32)
#define SC_CSN_BUFFER_LEN (128)
#define SC_SYS_VENDOR_ID_BUFFER_LEN (32)
#define SC_MODEM_VENDOR_ID_BUFFER_LEN (32)
char board_fw_version[SC_FW_VERSION_BUFFER_LEN]="01.00.023";
char board_csn[SC_CSN_BUFFER_LEN]="Speedport W 724V";
char board_modem_vendor_id[SC_MODEM_VENDOR_ID_BUFFER_LEN]="\x26""\x00""SRCM""\x00\x00";
char board_sys_vendor_id[SC_SYS_VENDOR_ID_BUFFER_LEN]="\x26""\x00""SRCM""\x00\x00";
EXPORT_SYMBOL(board_fw_version);
EXPORT_SYMBOL(board_csn);
EXPORT_SYMBOL(board_modem_vendor_id);
EXPORT_SYMBOL(board_sys_vendor_id);
static ssize_t proc_read_fw_version(struct file * file, char * buff, size_t len, loff_t *pos)
{
    int ret = 0;
    if(*pos == 0)
    {
        *pos = sprintf(buff, "%s", board_fw_version);
        ret=*pos;
    }
    return ret;
}
static ssize_t proc_write_fw_version (struct file *file, const char *buf, size_t cnt, loff_t *offset)
{
    int ret = cnt;
    char *p;
    if (cnt > sizeof(board_fw_version))
        return -EFAULT;
    memset(board_fw_version, 0, sizeof(board_fw_version));
    if (copy_from_user(board_fw_version, buf, cnt))
    {
        return -EFAULT;
    }
    board_fw_version[SC_FW_VERSION_BUFFER_LEN-1]='\0';
    if((p=strchr(board_fw_version,'\n'))!=NULL)
        *p='\0';
    return ret;
}
static ssize_t proc_read_csn(struct file * file, char * buff, size_t len, loff_t *pos)
{
    int ret = 0;
    if(*pos == 0)
    {
        *pos = sprintf(buff, "%s", board_csn);
        ret=*pos;
    }
    return ret;
}
static ssize_t proc_write_csn (struct file *file, const char *buf, size_t cnt, loff_t *offset)
{
    int ret = cnt;
    char *p;
    if (cnt > sizeof(board_csn))
        return -EFAULT;
    memset(board_csn, 0, sizeof(board_csn));
    if (copy_from_user(board_csn, buf, cnt))
    {
        return -EFAULT;
    }
    board_csn[SC_CSN_BUFFER_LEN-1]='\0';
    if((p=strchr(board_csn,'\n'))!=NULL)
        *p='\0';
    return ret;
}
static ssize_t proc_read_modem_vendor_id(struct file * file, char * buff, size_t len, loff_t *pos)
{
    int ret = 0;
    if(*pos == 0)
    {
        *pos = sprintf(buff, "%s", board_modem_vendor_id);
        ret=*pos;
    }
    return ret;
}
static ssize_t proc_write_modem_vendor_id (struct file *file, const char *buf, size_t cnt, loff_t *offset)
{
    int ret = cnt;
    char *p;
    if (cnt > sizeof(board_modem_vendor_id))
        return -EFAULT;
    memset(board_modem_vendor_id, 0, sizeof(board_modem_vendor_id));
    if (copy_from_user(board_modem_vendor_id, buf, cnt))
    {
        return -EFAULT;
    }
    board_modem_vendor_id[SC_MODEM_VENDOR_ID_BUFFER_LEN-1]='\0';
    if((p=strchr(board_modem_vendor_id,'\n'))!=NULL)
        *p='\0';
    return ret;
}
static ssize_t proc_read_sys_vendor_id(struct file * file, char * buff, size_t len, loff_t *pos)
{
    int ret = 0;
    if(*pos == 0)
    {
        *pos = sprintf(buff, "%s", board_sys_vendor_id);
        ret=*pos;
    }
    return ret;
}
static ssize_t proc_write_sys_vendor_id (struct file *file, const char *buf, size_t cnt, loff_t *offset)
{
    int ret = cnt;
    char *p;
    if (cnt > sizeof(board_sys_vendor_id))
        return -EFAULT;
    memset(board_sys_vendor_id, 0, sizeof(board_sys_vendor_id));
    if (copy_from_user(board_sys_vendor_id, buf, cnt))
    {
        return -EFAULT;
    }
    board_sys_vendor_id[SC_SYS_VENDOR_ID_BUFFER_LEN-1]='\0';
    if((p=strchr(board_sys_vendor_id,'\n'))!=NULL)
        *p='\0';
    return ret;
}
...
#ifdef __SC_BUILD__
    static struct file_operations fw_version_proc = {
        .read=proc_read_fw_version,
        .write=proc_write_fw_version,
    };
    static struct file_operations csn_proc = {
        .read=proc_read_csn,
        .write=proc_write_csn,
    };
    static struct file_operations modem_vid_proc = {
        .read=proc_read_modem_vendor_id,
        .write=proc_write_modem_vendor_id,
    };
    static struct file_operations sys_vid_proc = {
        .read=proc_read_sys_vendor_id,
        .write=proc_write_sys_vendor_id,
    };
#endif
#endif
...
#define offset(type, elem) ((size_t)&((type *)0)->elem)
...
#ifdef __SC_BUILD__
    struct proc_dir_entry *p6;
    struct proc_dir_entry *p7;
    struct proc_dir_entry *p8;
    struct proc_dir_entry *p9;
#endif
    ...
    if (p5 == NULL)
        return -1;
#ifdef __SC_BUILD__
    p6 = proc_create("FW_VERSION", 0644, p0, &fw_version_proc);
    if (p6 == NULL)
        return -1;
    p7 = proc_create("CSN", 0644, p0, &csn_proc);
    if (p7 == NULL)
        return -1;
    p8 = proc_create("MODEM_VENDOR_ID", 0644, p0, &modem_vid_proc);
    if (p8 == NULL)
        return -1;
    p9 = proc_create("SYS_VENDOR_ID", 0644, p0, &sys_vid_proc);
    if (p9 == NULL)
        return -1;
#endif
#endif
