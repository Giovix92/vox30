...
static uint8 defaultIPG = 4;
#ifdef ESSENTIAL
uint8 defaultwanmap = 0x0; // all external switch port as lan port
#endif
#endif
#endif
...
extern int (*iperf_rx_hook)(struct sk_buff *skb);
#endif
#endif
...
#ifdef __SC_BUILD__
static void bcm63xx_enet_clear_stats(struct net_device * dev)
{
    BcmEnet_devctrl *pDevCtrlTemp;
    struct rtnl_link_stats64 *vstats;

    bcmeapi_reset_mib_cnt(port_id_from_dev(dev));
    pDevCtrlTemp = ((BcmEnet_devctrl *)netdev_priv(dev));
...
#ifdef __SC_BUILD__
        dev->clr_stats              = bcm63xx_enet_clear_stats;
#endif
#if defined(CONFIG_BCM_PKTRUNNER_GSO)
...
#ifdef __SC_BUILD__
extern wait_queue_head_t eth_port_queue;
extern int eth_port_flag;
#endif
...
void link_change_handler(int port, int cb_port, int linkstatus, int speed, int duplex)
{
...
#if defined(CONFIG_BCM_JUMBO_FRAME) && !defined(CONFIG_BCM_MAX_MTU_SIZE)
    {
        ...
        int max_mtu = (speed == 1000 ? ENET_MAX_MTU_PAYLOAD_SIZE : ENET_NON_JUMBO_MAX_MTU_PAYLOAD_SIZE);
            ...
            {
                eth_port_flag = 1;
                //wake_up_interruptible(&eth_port_queue);
            }
            // kerSysSetGpioState(BP_GPIO_17_AH + sw_port, 1);
#ifdef CONFIG_BCM963268
            //add LAN LOG, switch port 3 : WAN, switch port 2: LAN1, switch port 1: LAN2, switc
#ifdef VD1018
            if(sw_port < 3)
                LOG_LAN(KERN_INFO,NORM_LOG,LOG_NONUSE_ID,LOG_NONUSE_BLOCK_TIME,
                        "LAN%d Link UP %d mbps %s duplex\n",
                        (3-sw_port), speed, duplex?"full":"half");
#else
            if(port > 8)
                LOG_LAN(KERN_INFO,NORM_LOG,LOG_NONUSE_ID,LOG_NONUSE_BLOCK_TIME,
                        "LAN%d Link UP %d mbps %s duplex\n",
                        (port-8), speed, duplex?"full":"half");
            ...
            if(port >= 8)
                LOG_LAN(KERN_INFO,NORM_LOG,LOG_NONUSE_ID,LOG_NONUSE_BLOCK_TIME,
                        "LAN%d Link UP %d mbps %s duplex\n",
                        port-7, speed, duplex?"full":"half");
#else
            //add LAN LOG, switch port 0 : WAN, switch port 3: LAN1, switch port 2: LAN2, switc
            if(sw_port > 0)
                LOG_LAN(KERN_INFO,NORM_LOG,LOG_NONUSE_ID,LOG_NONUSE_BLOCK_TIME,
                        "LAN%d Link UP %d mbps %s duplex\n",
                        (4-sw_port), speed, duplex?"full":"half");
#endif
#endif
#endif
            ...
            //  kerSysSetGpioState(BP_GPIO_17_AH + sw_port, 0);
#ifdef CONFIG_BCM963268
            //add LAN LOG, switch port 3 : WAN, switch port 2: LAN1, switch port 1: LAN2, switc
#ifdef VD1018
            if(sw_port < 3)
                ...
                if ( vnet_dev[i] != NULL )
                {
                            ...
                            (priv->linkState & mask) != tmp)
                    {
                        ...
                        if (phys.lnk)
                        {
                            ...
                            //  kerSysSetGpioState(BP_GPIO_17_AH + i, 1);
                            LOG_LAN(KERN_INFO,NORM_LOG,LOG_NONUSE_ID,LOG_NONUSE_BLOCK_TIME,
                                    "%s Link UP %d mbps %s duplex\n", vnet_dev[i]->name, speed, phys.fdx?"full":"half");
#endif
                            if (netif_carrier_ok(vnet_dev[i]) == 0)
                            {
                        ...
                        else
                        {
                            ...
                            //  kerSysSetGpioState(BP_GPIO_17_AH + i, 0);
                            LOG_LAN(KERN_INFO,NORM_LOG,LOG_NONUSE_ID,LOG_NONUSE_BLOCK_TIME,
                                    "%s Link DOWN.\n", vnet_dev[i]->name);
#endif
                        }
...
static inline int bcm63xx_enet_xmit2(struct net_device *dev, EnetXmitParams *pParam)
{
    ...
    if (gemMirrorCfg[MIRROR_DIR_OUT].nStatus && dev && dev->priv_flags & IFF_WANDEV)
    {
        struct sk_buff * pNBuffSkb = NULL; /* If pNBuff is sk_buff: protocol access */
        pNBuffSkb = pParam->skb;    /* translate to skb */
        if (pNBuffSkb != (struct sk_buff *)NULL)
        {
            MirrorPacket( pNBuffSkb, gemMirrorCfg[MIRROR_DIR_OUT].szMirrorInterface, 1);
...
static uint32 bcm63xx_rx(void *ptr, uint32 budget)
{
    ...
    for(bcmeapi_prepare_rx(); --budget > 0 && (rxpktgood & ENET_POLL_DONE) == 0; dev = NULL, pBuf = NULL, skb = NULL)
    {
...
#if defined(__SC_BUILD__)
        if (gemMirrorCfg[MIRROR_DIR_IN].nStatus && dev && dev->priv_flags & IFF_WANDEV)
        {
            ...
            {
                MirrorPacket(skb_m, gemMirrorCfg[MIRROR_DIR_IN].szMirrorInterface, 0);
        ...
        psh->mark = SCMARK_SET_PORT(psh->mark,(phy_port_id-7));
#endif
        local_bh_disable();
#ifdef CONFIG_SUPPORT_IPERF
        if(iperf_rx_hook)
        {
            if(!iperf_rx_hook(skb))
...
#ifdef __SC_BUILD__
        case SIOCPORTMIRROR:
            if(copy_from_user((void*)&mirrorCfg,rq->ifr_data,sizeof(MirrorCfg)))
                val = -EFAULT;
            else
            {
        ...
        case SIOCETHSWCTLOPS:
            ...
            switch(e->op) {
                ...
                case ETHSWDUMPMIDEX:
                    BCM_ENET_DEBUG("ethswctl ETHSWDUMPMIB ioctl");
                    if (e->unit) {
                        val = bcmsw_dump_mib_ext_ex(e->port, e->type, &(e->eth_stat));
                    }
                    else
                    {
                        val = bcmeapi_ethsw_dump_mib_ex(e->port, e->type, &(e->eth_stat));
                    }
                    break;
#endif
...
static int __init bcmenet_module_init(void)
{
    ...
    /*Fix IPG issue, in vodafone with alu ONU, IPG SHOULD be set to 64bits*/
#if defined FD1018 || defined(VOX25) || defined(ESSENTIAL)
    ethsw_wreg(PAGE_CONTROL, 0x0A, (uint8 *)&defaultIPG, 1);
#ifdef ESSENTIAL
    SW_WRITE_REG(1,PAGE_CONTROL, 0x26, (uint8 *)&defaultwanmap, 2);
#endif
