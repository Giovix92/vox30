...
#ifdef __SC_BUILD__
int enet_arl_access_dump_ex(unsigned char *mac, int *port)
{
    int timeout, count = 0;
        ...
        ethsw_rreg(PAGE_AVTBL_ACCESS, REG_ARL_SRCH_CTRL, (uint8_t *)&v16, 2))
    {
        ...
        {
            *port = (((1<<15)|(v16 & 0xf800)>>1)|((v16&0x3fe)>>1)) & 0x1ff;
            return 1;
        }
    }
    return 0;
...
#ifdef __SC_BUILD__
int bcmeapi_ethsw_dump_mib_ex(int port, int type, EthSwCounter *eth_stat)
{
    volatile EthSwMIBRegs *e;
    int base;
    ...
    if (gmac_info_pg->enabled && (port == GMAC_PORT_ID) )
    {
        ...
        /* Display Tx statistics */
        eth_stat->TxUnicastPkts = e->TxUnicastPkts + g->TxUnicastPkts;
        eth_stat->TxMulticastPkts = e->TxMulticastPkts + g->TxMulticastPkts;
        eth_stat->TxBroadcastPkts = e->TxBroadcastPkts + g->TxBroadcastPkts;
        eth_stat->TxDropPkts = e->TxDropPkts + (g->TxPkts - g->TxGoodPkts);

        if (type) {
            eth_stat->TxOctetsLo = e->TxOctetsLo + g->TxOctetsLo;
            eth_stat->TxOctetsHi = e->TxOctetsHi;
            eth_stat->TxQoSPkts = e->TxQoSPkts + g->TxGoodPkts;
            eth_stat->TxCol = e->TxCol + g->TxCol;
            eth_stat->TxSingleCol = e->TxSingleCol + g->TxSingleCol;
            eth_stat->TxMultipleCol = e->TxMultipleCol + g->TxMultipleCol;
            eth_stat->TxDeferredTx = e->TxDeferredTx + g->TxDeferredTx;
            eth_stat->TxLateCol = e->TxLateCol + g->TxLateCol;
            eth_stat->TxExcessiveCol = e->TxExcessiveCol + g->TxExcessiveCol;
            eth_stat->TxFrameInDisc = e->TxFrameInDisc;
            eth_stat->TxPausePkts = e->TxPausePkts + g->TxPausePkts;
            eth_stat->TxQoSOctetsLo = e->TxQoSOctetsLo + g->TxOctetsLo;
            eth_stat->TxQoSOctetsHi = e->TxQoSOctetsHi;
        }

        /* Display Rx statistics */
        eth_stat->RxUnicastPkts = e->RxUnicastPkts + g->RxUnicastPkts;
        eth_stat->RxMulticastPkts = e->RxMulticastPkts + g->RxMulticastPkts;
        eth_stat->RxBroadcastPkts = e->RxBroadcastPkts + g->RxBroadcastPkts;
        eth_stat->RxDropPkts = e->RxDropPkts + (g->RxPkts - g->RxGoodPkts);
        /* Display remaining rx stats only if requested */
        if (type) {
            eth_stat->RxJabbers = e->RxJabbers + g->RxJabbers;
            eth_stat->RxAlignErrs = e->RxAlignErrs + g->RxAlignErrs;
            eth_stat->RxFCSErrs = e->RxFCSErrs + g->RxFCSErrs;
            eth_stat->RxFragments = e->RxFragments + g->RxFragments;
            eth_stat->RxOversizePkts = e->RxOversizePkts + g->RxOversizePkts;
            eth_stat->RxExcessSizeDisc = e->RxExcessSizeDisc + g->RxExcessSizeDisc;
            eth_stat->RxOctetsLo = e->RxOctetsLo + g->RxOctetsLo;
            eth_stat->RxOctetsHi = e->RxOctetsHi;
            eth_stat->RxUndersizePkts = e->RxUndersizePkts + g->RxUndersizePkts;
            eth_stat->RxPausePkts = e->RxPausePkts + g->RxPausePkts;
            eth_stat->RxGoodOctetsLo = e->RxGoodOctetsLo + g->RxOctetsLo;
            eth_stat->RxGoodOctetsHi = e->RxGoodOctetsHi;
            eth_stat->RxSAChanges = e->RxSAChanges;
            eth_stat->RxSymbolError = e->RxSymbolError + g->RxSymbolError;
            eth_stat->RxQoSPkts = e->RxQoSPkts + g->RxGoodPkts;
            eth_stat->RxQoSOctetsLo = e->RxQoSOctetsLo + g->RxOctetsLo;
            eth_stat->RxQoSOctetsHi = e->RxQoSOctetsHi;
            eth_stat->Pkts64Octets = e->Pkts64Octets + g->Pkts64Octets;
            eth_stat->Pkts65to127Octets = e->Pkts65to127Octets + g->Pkts65to127Octets;
            eth_stat->Pkts128to255Octets = e->Pkts128to255Octets + g->Pkts128to255Octets;
            eth_stat->Pkts128to255Octets = e->Pkts256to511Octets + g->Pkts256to511Octets;
            eth_stat->Pkts512to1023Octets = e->Pkts512to1023Octets + g->Pkts512to1023Octets;
            eth_stat->Pkts1024to1522Octets = e->Pkts1024to1522Octets +
                (g->Pkts1024to1518Octets + g->Pkts1519to1522);
            eth_stat->Pkts1523to2047 = e->Pkts1523to2047 + g->Pkts1523to2047;
            eth_stat->Pkts2048to4095 = e->Pkts2048to4095 + g->Pkts2048to4095;
            eth_stat->Pkts4096to8191 = e->Pkts4096to8191 + g->Pkts4096to8191;
            eth_stat->Pkts8192to9728 = e->Pkts8192to9728;
        }
#if defined(CONFIG_BCM_ETH_PWRSAVE)
        spin_unlock_irqrestore(&bcmsw_pll_control_lock, flags);
#endif
    ...
    /* Display Tx statistics */
    eth_stat->TxUnicastPkts = e->TxUnicastPkts;
    eth_stat->TxMulticastPkts = e->TxMulticastPkts;
    eth_stat->TxBroadcastPkts = e->TxBroadcastPkts;
    eth_stat->TxDropPkts = e->TxDropPkts;
    /* Display remaining tx stats only if requested */
    if (type) {
        eth_stat->TxOctetsLo = e->TxOctetsLo;
        eth_stat->TxOctetsHi = e->TxOctetsHi;
        eth_stat->TxQoSPkts =  e->TxQoSPkts;
        eth_stat->TxCol =  e->TxCol;
        eth_stat->TxSingleCol = e->TxSingleCol;
        eth_stat->TxMultipleCol = e->TxMultipleCol;
        eth_stat->TxDeferredTx = e->TxDeferredTx;
        eth_stat->TxLateCol = e->TxLateCol;
        eth_stat->TxExcessiveCol = e->TxExcessiveCol;
        eth_stat->TxFrameInDisc = e->TxFrameInDisc;
        eth_stat->TxPausePkts = e->TxPausePkts;
        eth_stat->TxQoSOctetsLo = e->TxQoSOctetsLo;
        eth_stat->TxQoSOctetsHi = e->TxQoSOctetsHi;
    }

    /* Display Rx statistics */
    eth_stat->RxUnicastPkts = e->RxUnicastPkts;
    eth_stat->RxMulticastPkts = e->RxMulticastPkts;
    eth_stat->RxBroadcastPkts = e->RxBroadcastPkts;
    eth_stat->RxDropPkts = e->RxDropPkts;
    /* Display remaining rx stats only if requested */
    if (type) {
        eth_stat->RxJabbers = e->RxJabbers;
        eth_stat->RxAlignErrs = e->RxAlignErrs;
        eth_stat->RxFCSErrs = e->RxFCSErrs;
        eth_stat->RxFragments = e->RxFragments;
        eth_stat->RxOversizePkts = e->RxOversizePkts;
        eth_stat->RxExcessSizeDisc = e->RxExcessSizeDisc;
        eth_stat->RxOctetsLo = e->RxOctetsLo;
        eth_stat->RxOctetsHi = e->RxOctetsHi;
        eth_stat->RxUndersizePkts = e->RxUndersizePkts;
        eth_stat->RxPausePkts = e->RxPausePkts;
        eth_stat->RxGoodOctetsLo = e->RxGoodOctetsLo;
        eth_stat->RxGoodOctetsHi = e->RxGoodOctetsHi;
        eth_stat->RxSAChanges = e->RxSAChanges;
        eth_stat->RxSymbolError = e->RxSymbolError;
        eth_stat->RxQoSPkts = e->RxQoSPkts;
        eth_stat->RxQoSOctetsLo = e->RxQoSOctetsLo;
        eth_stat->RxQoSOctetsHi = e->RxQoSOctetsHi;
        eth_stat->Pkts64Octets = e->Pkts64Octets;
        eth_stat->Pkts65to127Octets = e->Pkts65to127Octets;
        eth_stat->Pkts128to255Octets = e->Pkts128to255Octets;
        eth_stat->Pkts256to511Octets = e->Pkts256to511Octets;
        eth_stat->Pkts512to1023Octets = e->Pkts512to1023Octets;
        eth_stat->Pkts1024to1522Octets = e->Pkts1024to1522Octets;
        eth_stat->Pkts1523to2047 = e->Pkts1523to2047;
        eth_stat->Pkts2048to4095 = e->Pkts2048to4095;
        eth_stat->Pkts4096to8191 = e->Pkts4096to8191;
        eth_stat->Pkts8192to9728 = e->Pkts8192to9728;
    }
#if defined(CONFIG_BCM_ETH_PWRSAVE)
        spin_unlock_irqrestore(&bcmsw_pll_control_lock, flags);
