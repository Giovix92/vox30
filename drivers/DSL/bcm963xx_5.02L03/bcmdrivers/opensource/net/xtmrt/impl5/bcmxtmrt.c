...
#ifdef __SC_BUILD__
static void MirrorIPoAPacket(char *intfName, pNBuff_t pNBuf);
#endif
...
#ifdef __SC_BUILD__
   else if( pDevCtx->szMirrorIntfOut[0] != '\0' &&
           !isAtmCell &&
           (hdrType ==  HT_LLC_SNAP_ROUTE_IP ||
            hdrType ==  HT_VC_MUX_IPOA) )
   {
       spin_unlock_bh(xtmlock);
       MirrorIPoAPacket( pDevCtx->szMirrorIntfOut, pNBuf );
       spin_lock_bh(xtmlock);
   }
#endif
...
#ifdef __SC_BUILD__
   else if( pDevCtx->szMirrorIntfIn[0] != '\0' &&
         (hdrType ==  HT_LLC_SNAP_ROUTE_IP ||
            hdrType ==  HT_VC_MUX_IPOA) )
   {
           MirrorIPoAPacket(pDevCtx->szMirrorIntfIn, pNBuf);
   }
#endif
...
#if defined(CONFIG_SUPPORT_IPERF) && defined(__SC_BUILD__)
            if(iperf_rx_hook)
            {
...
#ifdef __SC_BUILD__
static void MirrorIPoAPacket(char *mirrorIntf, pNBuff_t pNBuf)
{
   ...
   size = (len + 3 + sizeof(struct ethhdr)) & ~3;  /* align to word boundary */
   ...
   skb_put(skb2, sizeof(struct ethhdr));
   skb_reset_mac_header(skb2);
   eth = (struct ethhdr*)skb2->data;
   if(netDev->dev_addr)
       memcpy(eth->h_source, netDev->dev_addr, NVRAM_MAC_ADDRESS_LEN);
   else
       memset(eth->h_source, 0x00, NVRAM_MAC_ADDRESS_LEN);
   memcpy(eth->h_dest, dmac, NVRAM_MAC_ADDRESS_LEN);
   eth->h_proto = __constant_htons(ETH_P_IP);
   skb_put(skb2, len);
   memcpy(skb2->data+sizeof(struct ethhdr), data_p, len);

   /* pad with zero */      
   if (size > (len+sizeof(struct ethhdr)))
       memset(skb2->data+len+sizeof(struct ethhdr), 0, size-len-sizeof(struct ethhdr));
   skb2->dev      = netDev;
