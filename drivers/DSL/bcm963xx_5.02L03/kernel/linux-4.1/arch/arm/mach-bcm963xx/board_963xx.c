...
#if defined(CONFIG_CRASH_LOG)
/* Reserve memory for Kernel Crash */
#define KERNEL_CRASH_SDRAM_RESERVE_SIZE		MB_ALIGNED(KERNEL_SDRAM_IMAGE_SIZE)

/***************************************************************************
 * Function Name: kerSysGetKernelCrashMemory
 * Description  : return the start address of the reserved KERNEL CRASH SDRAM. The memory
 * 		  is reserved in the arch dependent setup.c
 * Returns      : physical address of the reserved  KERNEL CRASH  SDRAM
 ***************************************************************************/
void *kerSysGetKernelCrashMemory(unsigned long last_phy_addr)
{
	if (is_memory_reserved == true) {
			return (void *)(last_phy_addr - MB_ALIGNED(KERNEL_CRASH_SDRAM_RESERVE_SIZE));
	} else
		return NULL;
}

EXPORT_SYMBOL(kerSysGetKernelCrashMemory);
unsigned long boot_free_address = 0;
EXPORT_SYMBOL(boot_free_address);
#endif
...
{
#if defined(CONFIG_CRASH_LOG)
	struct map_desc desc[TOTAL_RESERVE_MEM_NUM+2];
#else
...
#if defined(CONFIG_CRASH_LOG)
	/* create a noncacheable memory device mapping for Kernel Creash to
	 * access the reserved memory */
	desc[i].virtual = (unsigned long)phys_to_virt(
			(unsigned long)kerSysGetKernelCrashMemory((unsigned long)reserve_mem[i-1].phys_addr));
	desc[i].pfn = __phys_to_pfn((unsigned long)kerSysGetKernelCrashMemory((unsigned long)reserve_mem[i-1].phys_addr));
	desc[i].length = MB_ALIGNED(KERNEL_CRASH_SDRAM_RESERVE_SIZE);
	desc[i].type = MT_MEMORY_RWX_NONCACHED;
    boot_free_address = desc[i].virtual;
	printk("creating a MT_MEMORY_NONCACHED device at physical address of "
			"0x%08lx to virtual address at "
			"0x%08lx with size of 0x%lx byte for Kernel Crash\n",
			(unsigned long)kerSysGetKernelCrashMemory(((unsigned long)reserve_mem[i-1].phys_addr)),
			desc[i].virtual, desc[i].length);
	memblock_remove((unsigned long)kerSysGetKernelCrashMemory((unsigned long)reserve_mem[i-1].phys_addr), desc[i].length);
	i++;
#endif
