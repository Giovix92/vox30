...
/* block function */
int (*sc_check_and_block_hook)(struct sk_buff *skb, const struct nf_conntrack_tuple *tuple) = NULL;
...
int (*sc_delete_special_block_pattern_hook)(int dport_min, int dport_max,
										int type, u_int8_t protonum) = NULL;
int (*sc_check_block_src_protonum)(const struct sk_buff *skb);
...
unsigned int (*sc_psd_and_special_udp_detect_hook)(struct sk_buff *skb, struct nf_conn *ct, u_int8_t protonum, unsigned int dataoff) = NULL;


/* tcp syn flood detect */
unsigned int (*sc_tcp_check_hook)(struct sk_buff *skb, struct nf_conn *ct, u_int8_t pf, unsigned int dataoff) = NULL;
void (*sc_tcp_destroy_hook)(struct nf_conn *ct) = NULL;
void (*sc_tcp_deal_establish_hook)(struct nf_conn *ct) = NULL;

unsigned int (*sc_tcp_fin_check_hook)(struct sk_buff *skb, u_int8_t pf, u_int8_t protonum) = NULL;
/* icmp flood detect */
unsigned int (*sc_icmp_check_hook)(struct sk_buff *skb, u_int8_t pf, struct nf_conn *ct) = NULL;

unsigned int (*sc_brdcst_src_check_hook)(struct sk_buff *skb) = NULL;
...
unsigned int (*sc_udp_check_hook)(struct sk_buff *skb, struct nf_conn *ct) = NULL;
void (*sc_udp_destroy_hook)(struct nf_conn *ct) = NULL;

/* ip check */
unsigned int (*sc_packet_ip_check_hook)(struct sk_buff *skb, u_int8_t pf) = NULL;


EXPORT_SYMBOL(sc_check_and_block_hook);
EXPORT_SYMBOL(sc_check_block_src_protonum);
...
EXPORT_SYMBOL(sc_tcp_fin_check_hook);
EXPORT_SYMBOL(sc_icmp_check_hook);

EXPORT_SYMBOL(sc_brdcst_src_check_hook);
...
EXPORT_SYMBOL(sc_udp_destroy_hook);
EXPORT_SYMBOL(sc_udp_check_hook);
