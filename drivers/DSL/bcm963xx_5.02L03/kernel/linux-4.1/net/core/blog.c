 ...
 */
#ifdef __SC_BUILD__
int blog_tcpack_prio_enable;
unsigned int blog_tcpack_xtmprio;
unsigned int blog_tcpack_ethprio;
EXPORT_SYMBOL(blog_tcpack_prio_enable);
EXPORT_SYMBOL(blog_tcpack_xtmprio);
EXPORT_SYMBOL(blog_tcpack_ethprio);
#endif
...
static void blog_tcpack_prio( Blog_t * blog_p, int len )
{
    int max_ack_len = 0;
#ifdef __SC_BUILD__
    int blog_tcpack_max_count = 0;

    if(blog_tcpack_prio_enable == 0)
        blog_tcpack_max_count = 0;
    else
        blog_tcpack_max_count = 4;
#endif
...
#define DEFAULT_QUEUE 2
             (SKBMARK_GET_Q(blog_p->mark) != DEFAULT_QUEUE ) )
#else
                ...
                switch ( blog_p->tx.info.phyHdrType )
                {
                    case BLOG_XTMPHY:
                    {
                        if ( blog_xtm_get_tx_chan_fn )
                        {
                            blog_p->mark = 
#ifdef __SC_BUILD__
                                SKBMARK_SET_Q(blog_p->mark, blog_tcpack_xtmprio );
#else
...
#ifdef __SC_BUILD__
                                    blog_tcpack_ethprio, blog_p->mark );
#else
...
#if 0
#ifdef __SC_BUILD__ 
     /*martin_huang@sdc.sercomm.com: this is for HTTP Filter*/
     if ( ! RX_IPinIP(blog_p) && RX_IPV4(blog_p)
           && ((blog_p->rx.tuple.port.dest == 80)
           || (blog_p->rx.tuple.port.source == 80))
