...
#if (defined(__SC_BUILD__) && (defined(CONFIG_IPV6_FIX_INTERFACE)))
static int addrconf_ifid_eui64_fix(u8 *eui);
static int ipv6_generate_eui64_fix(u8 *eui, struct net_device *dev);
#endif
...
static int ipv6_generate_eui64_fix(u8 *eui, struct net_device *dev)
{
	...
	case ARPHRD_SIT:
	case ARPHRD_IPGRE:
		return addrconf_ifid_eui64_fix(eui);
	}
	return -1;
}
#endif
...
#if (defined(__SC_BUILD__) && (defined(CONFIG_IPV6_FIX_INTERFACE)))
			if (ipv6_generate_eui64_fix(addr.s6_addr + 8, dev) &&
...
static void addrconf_dad_completed(struct inet6_ifaddr *ifp)
{
	...
	/* WAN interface needs to act as a host. */
#ifdef __SC_BUILD__
		  ( (ifp->idev->cnf.forwarding == 0) || (ifp->idev->cnf.forwarding == 2) || 
#else
