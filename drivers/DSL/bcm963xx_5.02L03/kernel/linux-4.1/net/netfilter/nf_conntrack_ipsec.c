...
#ifdef __SC_BUILD__
#define CT_REFRESH_TIMEOUT (60 * HZ * 60)
#else
                             ...
                             struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
...
#ifdef __SC_BUILD__
   {
       struct iphdr *iph = (struct iphdr *)skb_network_header(skb);//(*pskb)->nh.iph;
       struct udphdr *udph = (void *)iph + iph->ihl * 4;

       /* make sure source port is 500 */
       if( dir == IP_CT_DIR_ORIGINAL )
       {
           if(udph->source > htons(IPSEC_PORT))
               return NF_ACCEPT;//NF_REPEAT
       }
       else
       {
           if(udph->dest > htons(IPSEC_PORT))
               return NF_ACCEPT;//NF_REPEAT
       }
   }
#endif
   ...
   else
   {
      ...
      {
#ifdef __SC_BUILD__
	 if(ct != ipsec_entry->ct)
	 {
	     test_and_set_bit(IPS_SEEN_REPLY_BIT, &ipsec_entry->ct->status);
	     test_and_set_bit(IPS_ASSURED_BIT, &ipsec_entry->ct->status);
             nf_ct_refresh_acct(ct, 0, skb, CT_REFRESH_TIMEOUT);
   	     ipsec_entry->nfskb_p.nfct = (struct nf_conntrack *)ipsec_entry->ct;
             nf_ct_refresh_acct(ipsec_entry->ct, 0, &(ipsec_entry->nfskb_p), CT_REFRESH_TIMEOUT);
	 }
	 else
	 {
             nf_ct_refresh_acct(ct, 0, skb, CT_REFRESH_TIMEOUT);
