...
typedef struct {
	...
	u_int16_t trig_first_port;		/* Trigger first port */
	u_int16_t trig_last_port;		/* Trigger last port */
	u_int16_t trig_proto;			/* Trigger protocol */
	u_int16_t rel_proto;			/* Related protocol */
	u_int16_t related_first_port;	/* Related first port */
	u_int16_t related_last_port;	/* Related last port */
	u_int8_t  reply;				/* Confirm a reply connection */

	u_int32_t fw_rule_index;		/* NP firewall rule index */
	u_int32_t fw_rule_active;		/* 1 - Active if rule exists */
} xt_porttrig;
...
{
	struct list_head *p;
    uint16_t dst = ntohs(tcph->dest);

    DEBUGP2("%s:TRIG FIND ENTRY IN: iph.proto:[%d] tcph->dest_port:[%d,%d]\n", __FUNCTION__,
    	iph->protocol,
    	tcph->dest, dst);
	...
	list_for_each_prev(p, &porttrig_list) {
		...
		if ((found->trig_proto == iph->protocol) &&
			(((__be16) found->trig_first_port)  >= dst) &&
			(((__be16) found->trig_last_port)   <= dst)
			) {
	...
	struct tcphdr *tcph)
{
	struct list_head *p;
    uint16_t dst = ntohs(tcph->dest);
    DEBUGP2("%s:TRIG FIND REPLY ENTRY IN: iph.proto:[%d] tcph->dest_port:[%d,%d]\n", __FUNCTION__,
    	iph->protocol,
    	tcph->dest, dst);
	...
	list_for_each_prev(p, &porttrig_list) {
        ...
        DEBUGP2("%s,rel:%x,trg port :%x, first:%d,last:%d.\n",__FUNCTION__,found->rel_proto, found->trig_proto,found->related_first_port,found->related_last_port);
  
#ifdef __SC_BUILD__
		if ((found->rel_proto == iph->protocol || found->rel_proto == IPPROTO_ALL)
#else
		if ((found->trig_proto == iph->protocol)
#endif
			&& (((__be16) found->related_first_port) <= dst)
			&& (((__be16) found->related_last_port)  >= dst)) {
	...
	const void *targinfo)
{
   ...
   /* Only TCP, UDP and ALL protocols are supported */
	...
	if ((ptinfo->trigger_first_port <= dst) &&
		(ptinfo->trigger_last_port >= dst)
	   ) { /* Check destination triggering port */

		DEBUGP("TARGET:[%s] MATCHED OK: target-ports&tcph->dest:[%d...%d&%d,%d]\n",
			target->name, ptinfo->trigger_first_port, ptinfo->trigger_last_port, tcph->dest, dst);
	...
	const void *targinfo)
{
    ...
    if (found) {
		...
		range1.min_addr.ip = range1.max_addr.ip = found->srcip;

		/* hook doesn't matter, but it has to do destination manip */
		nf_nat_setup_info(ct, &range1, NF_NAT_RANGE_MAP_IPS);
