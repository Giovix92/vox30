 ...
 * +------+ +------+ +------+ +------+
 * eeddddcc ccPppppp pYyyyXxx xxxxzzzz
 * +------+ +------+ +------+ +------+
 * ee     --- flow_id (0~3)
 * dddd   --- app_id (0~15)
 * cccc   --- phy_port id (0~15)
...
#define SCMARK_PORT_M          (0x0f << SCMARK_PORT_S)
#define SCMARK_GET_PORT(MARK) \
    ((MARK & SCMARK_PORT_M) >> SCMARK_PORT_S)
#define SCMARK_SET_PORT(MARK, PORT) \
    ((MARK & ~SCMARK_PORT_M) | (PORT << SCMARK_PORT_S))
#define SCMARK_APP_S           26
#define SCMARK_APP_M          (0x0f << SCMARK_APP_S)
#define SCMARK_GET_APPID(MARK) \
    ((MARK & SCMARK_APP_M) >> SCMARK_APP_S)
#define SCMARK_SET_APPID(MARK, APP) \
    ((MARK & ~SCMARK_APP_M) | (APP << SCMARK_APP_S))
#define SCMARK_FLOW_S           30
#define SCMARK_FLOW_M          (0x03 << SCMARK_FLOW_S)
#define SCMARK_GET_FLOWID(MARK) \
    ((MARK & SCMARK_FLOW_M) >> SCMARK_FLOW_S)
#define SCMARK_SET_FLOWID(MARK, FLOW) \
    ((MARK & ~SCMARK_FLOW_M) | (FLOW << SCMARK_FLOW_S))
