...
#ifdef __SC_BUILD__
        //memset(&skb->pkt_info, 0, sizeof(struct packet_info));
        memset(skb->sercomm_header, 0, sizeof(skb->sercomm_header));
#endif       
...
#ifdef __SC_BUILD__
        //memset(&skb->pkt_info, 0, sizeof(struct packet_info));
        memset(skb_p->sercomm_header, 0, sizeof(skb_p->sercomm_header));
#endif
...
#define IP_OFFSET	0x1FFF		/* "Fragment Offset" part	*/
	...
	DEBUG_SKBFRAG(("skb_frag_xmit4:enter origskb=%p,netdev=%p,is_pppoe=%d,\
				...
				minMtu, ipp));
#ifdef __SC_BUILD__
    if(unlikely(txdev == NULL))
    {
    	kfree_skb(origskb);
        printk(KERN_ERR"txdev == NULL!!!\n");
		return ;
    }
#endif
...
#endif /* CONFIG_BCM_KF_NBUFF */
#ifdef __SC_BUILD__
        memcpy(new->sercomm_header, old->sercomm_header, sizeof(new->sercomm_header));
        //memcpy(&new->pkt_info,&old->pkt_info,sizeof(struct packet_info));
#endif
#if defined(CONFIG_BCM_KF_WL)
