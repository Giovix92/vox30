...
#ifdef __SC_BUILD__
typedef size_t (*curl_read_callback)(int session_idx, char *buffer,
                                      size_t size,
                                      size_t nitems,
                                      void *instream);
#else
...
typedef struct ic_info{
    unsigned long int test_bs;
    unsigned long int test_br;
    unsigned long int total_bs;
    unsigned long int total_br;
    struct timeval startTime;
    struct timeval endTime;
}ic_info_t;
typedef struct if_inf{
    char ifname[16];
...
#ifdef __SC_BUILD__
#define HAVE_TOS_OPT 1
#define HAVE_ETHERNET_PRIORITY_OPT 1
#endif
typedef enum {
...
#ifdef __SC_BUILD__
#if defined(HAVE_TOS_OPT)
  CINIT(TOS, LONG, 242),
#endif
#if defined(HAVE_ETHERNET_PRIORITY_OPT)
  CINIT(ETHERNET_PRIORITY, LONG, 243),
#endif
  CINIT(TCP_RES, LONG, 244),
  CINIT(SESSION_IDX, LONG, 245),
  CINIT(MAX_SESSION_NUM, LONG, 246),
  CINIT(TB_TEST_DURATION, LONG, 247),
  CINIT(TB_TEST_MINTERVAL, LONG, 248),
  CINIT(TB_TEST_MOFFSET, LONG, 249),
  CINIT(TCP_COUNT, LONG, 250),
  CINIT(TCP_NOCOPY, LONG, 251),
  CINIT(TCP_WINDOWSIZE, LONG, 252),
  CINIT(TCP_USER_TIMEOUT, LONG, 253),
  CINIT(TCP_SYN_RETRY, LONG, 254),
#endif
...
#ifdef __SC_BUILD__
#define CURLINFO_TIMEVAL  0x600000
#define CURLINFO_ICINFO   0x700000
#endif
...
#ifdef __SC_BUILD__
  CURLINFO_ROM_TIME     = CURLINFO_TIMEVAL + 45,
  CURLINFO_REQUEST_TIME     = CURLINFO_TIMEVAL + 46,
  CURLINFO_RESPONSE_TIME     = CURLINFO_TIMEVAL + 47,
  CURLINFO_TESTBR_FL    = CURLINFO_DOUBLE + 48,
  CURLINFO_TESTBS_FL    = CURLINFO_DOUBLE + 49,
  CURLINFO_TOTALBR_FL    = CURLINFO_DOUBLE + 50,
  CURLINFO_TOTALBS_FL    = CURLINFO_DOUBLE + 51,
  CURLINFO_PERIOD_FL    = CURLINFO_DOUBLE + 52,
  CURLINFO_GET_ICINFO    = CURLINFO_ICINFO + 53,
  CURLINFO_TOTALBR    = CURLINFO_DOUBLE + 54,
  CURLINFO_TOTALBS    = CURLINFO_DOUBLE + 55,
  CURLINFO_BOM_TIME     = CURLINFO_TIMEVAL + 56,
  CURLINFO_EOM_TIME     = CURLINFO_TIMEVAL + 57,
  CURLINFO_TIME_UP     = CURLINFO_LONG + 58,
  CURLINFO_INTERFACE_RX_START     = CURLINFO_DOUBLE + 59,
  CURLINFO_INTERFACE_TX_START     = CURLINFO_DOUBLE + 60,
  CURLINFO_INTERFACE_RX_END     = CURLINFO_DOUBLE + 61,
  CURLINFO_INTERFACE_TX_END     = CURLINFO_DOUBLE + 62,
