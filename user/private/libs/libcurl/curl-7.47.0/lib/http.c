 ...
 */
#ifdef __SC_BUILD__
static size_t readmoredata(int session_idx, char *buffer,
                           size_t size,
                           size_t nitems,
                           void *userp)
#else
  ...
  if(data->set.str[STRING_CUSTOMREQUEST])
    ...
    if(data->set.opt_no_body)
      ...
      case HTTPREQ_PUT:
        request = "PUT";
#ifdef __SC_BUILD__
        Curl_pgrsTime(conn->data, TIMER_ROM);
#endif
        break;
...
#ifdef __SC_BUILD__
            size_t actuallyread =
              data->state.fread_func(data->set.session_idx, data->state.buffer, 1, readthisamountnow,
                                     data->state.in);
#else
            size_t actuallyread =
...
#define HEADER1 k->p /* no conversion needed, just use k->p */
  ...
  /* We might have reached the end of the header part here, but
     ...
     buffer. */
#ifdef __SC_BUILD__
        if (data->set.tcp_nocopy && (data->progress.flags & PGRS_DL_SIZE_KNOWN))
        {
            int len = data->progress.size_dl;
            if (len > *nread)
                len = len - *nread;
            if (0 == setsockopt(conn->sock[0], IPPROTO_TCP, TCP_NOCOPY_LEN, (void*)&len, sizeof(len)))
                data->set.tcp_nocopy_len = len;
        }
#endif
