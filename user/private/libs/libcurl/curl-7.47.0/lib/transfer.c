...
#ifdef __SC_BUILD__
#ifdef DEBUGD
#define printfd(format,...) printf("["__FILE__"][%d]:"format"",__LINE__, ##__VA_ARGS__)
#else
#define printfd(format,...)
#endif
ic_info_t icInfo[512];
unsigned int icCounter = 1;
unsigned int timeUp = 0;
struct timeval tm_lst_ic;
if_inf_t if_info_lst_ic;
unsigned int long testBytesReceived_ic = 0, testBytesSent_ic = 0;
unsigned int long testBytesReceived_lst_ic = 0, testBytesSent_lst_ic = 0;
unsigned int bomCounter = 0, eomCounter = 0;
if_inf_t if_info_bom_fl, if_info_eom_fl;
if_inf_t if_info_for_total_start;
if_inf_t if_info_for_total_end;
struct timeval bomTime_fl, eomTime_fl, bomTime_fst, time_now;
unsigned long int testBytesReceived_fl = 0, testBytesSent_fl = 0;
unsigned int max_session_num = 0;
unsigned int tb_test_duration = 0,tb_test_minterval,tb_test_moffset = 0;
unsigned int tb_calc_start = 0;
...
static int getIFAdvInfo(char *if_name, if_inf_t *if_info)
{
    FILE *fh;
	char buf[1024];
	struct net_dev_info stats;
	...
	while (fgets(buf, sizeof(buf), fh)) {
	    ...
	    {
	        memset(&stats, 0, sizeof(struct net_dev_info));
...
#ifdef __SC_BUILD__
  nread = (int)data->state.fread_func(data->set.session_idx, data->req.upload_fromhere, 1,
  ...
  *done = FALSE;
  ...
  do {
    ...
    if(bomCounter == 1)
    {
        ...
        bomTime_fst = time_now;
        getIFAdvInfo(interface,&if_info_for_total_start); 
    }
    if(max_session_num > 1)
    {
        if(bomCounter == max_session_num)
        {
            gettimeofday(&time_now,NULL);
            bomTime_fl.tv_sec = time_now.tv_sec; 
            bomTime_fl.tv_usec = time_now.tv_usec;
            getIFAdvInfo(interface,&if_info_bom_fl); 
        }
    }
    if(((tb_test_duration > 0)&&(tb_test_minterval > 0))&&(tb_calc_start == 0))
    {
        gettimeofday(&time_now,NULL);
        if(Curl_tvdiff_secs(time_now,bomTime_fst) >= tb_test_moffset)
        {
            tb_calc_start = 1;
            tm_lst_ic = time_now;
            testBytesReceived_ic = 0;
            getIFAdvInfo(interface,&if_info_lst_ic);
        }
    }
#endif
...
#ifdef __SC_BUILD__
            if(data->set.tcp_nocopy && data->set.tcp_nocopy_len)
            {
                Curl_pgrsTime((struct SessionHandle *)conn, TIMER_TCPCOUNT);
                gettimeofday(&time_now,NULL);
                testBytesReceived_ic += nread;
                if_inf_t if_info_now;
                getIFAdvInfo(interface,&if_info_now);
                icInfo[icCounter].startTime.tv_sec = tm_lst_ic.tv_sec;
                icInfo[icCounter].startTime.tv_usec = tm_lst_ic.tv_usec;
                icInfo[icCounter].endTime.tv_sec = time_now.tv_sec;
                icInfo[icCounter].endTime.tv_usec = time_now.tv_usec;
                icInfo[icCounter].test_br = testBytesReceived_ic - testBytesReceived_lst_ic;
                icInfo[icCounter].total_br = if_info_now.rx_bytes - if_info_lst_ic.rx_bytes; 
                icInfo[icCounter].total_bs = if_info_now.tx_bytes - if_info_lst_ic.tx_bytes;
                tm_lst_ic.tv_sec = time_now.tv_sec;
                tm_lst_ic.tv_usec = time_now.tv_usec;
                testBytesReceived_lst_ic = testBytesReceived_ic;
                if_info_lst_ic.rx_bytes = if_info_now.rx_bytes;
                if_info_lst_ic.tx_bytes = if_info_now.tx_bytes;
                icCounter++;
                k->bytecount += nread;
    ...
    *didwhat |= KEEP_RECV;
...
#ifdef __SC_BUILD__
    gettimeofday(&time_now,NULL);
    if((tb_test_duration > 0)&&(Curl_tvdiff_secs(time_now,data->progress.t_bom) >= tb_test_duration))
    {
        timeUp = 1;
        data->progress.t_eom.tv_sec = data->progress.t_bom.tv_sec + tb_test_duration;
        data->progress.t_eom.tv_usec = data->progress.t_bom.tv_usec; 
    }

    if(timeUp == 1)
    {
        Curl_pgrsTime((struct SessionHandle *)conn, TIMER_TIMEUP);
        if_inf_t if_info_now;
        getIFAdvInfo(interface,&if_info_now);
        icInfo[icCounter].startTime.tv_sec = tm_lst_ic.tv_sec;
        icInfo[icCounter].startTime.tv_usec = tm_lst_ic.tv_usec;
    ...
    if(max_session_num > 1)
    {
        ...
        {
            gettimeofday(&time_now,NULL);
            eomTime_fl.tv_sec = time_now.tv_sec;
            eomTime_fl.tv_usec = time_now.tv_usec;
            getIFAdvInfo(interface,&if_info_eom_fl); 
        }
    }
#endif
    ...
    if(max_session_num > 1)
    {
      ...
      k->bytecount += nread;
#ifdef __SC_BUILD__
    //conn->data->progress.total_br+=nread;
    if(((tb_test_duration > 0)&&(tb_test_minterval > 0))&&(tb_calc_start == 1))
    {
        gettimeofday(&time_now,NULL);
        testBytesReceived_ic += nread;
        if(Curl_tvdiff_secs(time_now,tm_lst_ic) >= tb_test_minterval)
        {
            if_inf_t if_info_now;
            getIFAdvInfo(interface,&if_info_now);
            icInfo[icCounter].startTime.tv_sec = tm_lst_ic.tv_sec;
    ...
    if(max_session_num > 1)
    {
        if(bomTime_fl.tv_sec !=0 && bomTime_fl.tv_usec !=0 
            && eomTime_fl.tv_sec == 0 && eomTime_fl.tv_usec == 0)
        testBytesReceived_fl += nread;
    }
#endif
...
#ifdef __SC_BUILD__
  if((k->keepon & KEEP_RECV) == 0)
    {
        getIFAdvInfo(interface,&if_info_for_total_end); 
    }
#endif
  ...
  do {
...
#ifdef __SC_BUILD__
    //conn->data->progress.total_bs+=bytes_written;
    if(((tb_test_duration > 0)&&(tb_test_minterval > 0))&&(tb_calc_start == 1))
    {
        gettimeofday(&time_now,NULL);
        testBytesSent_ic += bytes_written;
        if(Curl_tvdiff_secs(time_now,tm_lst_ic) >= tb_test_minterval)
        {
            if_inf_t if_info_now;
            getIFAdvInfo(interface,&if_info_now);
            ...
            icInfo[icCounter].endTime.tv_sec = time_now.tv_sec;
            icInfo[icCounter].endTime.tv_usec = time_now.tv_usec;
            icInfo[icCounter].test_bs = testBytesSent_ic - testBytesSent_lst_ic;
            icInfo[icCounter].total_br = if_info_now.rx_bytes - if_info_lst_ic.rx_bytes; 
            icInfo[icCounter].total_bs = if_info_now.tx_bytes - if_info_lst_ic.tx_bytes;
            tm_lst_ic.tv_sec = time_now.tv_sec;
            tm_lst_ic.tv_usec = time_now.tv_usec;
            testBytesSent_lst_ic = testBytesSent_ic;
            if_info_lst_ic.rx_bytes = if_info_now.rx_bytes;
            if_info_lst_ic.tx_bytes = if_info_now.tx_bytes;
			icCounter++;
        }
    ...
    if(max_session_num > 1)
    {
        if(bomTime_fl.tv_sec !=0 && bomTime_fl.tv_usec !=0 
            && eomTime_fl.tv_sec == 0 && eomTime_fl.tv_usec == 0)
        {
            testBytesSent_fl += bytes_written;
        }
    }
    gettimeofday(&time_now,NULL);
    if((tb_test_duration > 0)&&(Curl_tvdiff_secs(time_now,data->progress.t_bom) >= tb_test_duration))
...
#ifdef __SC_BUILD__
    if((k->writebytecount == data->state.infilesize)||(timeUp == 1)) {
#else
