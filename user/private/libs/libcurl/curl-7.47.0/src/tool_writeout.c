          ...
          *end = 0; /* zero terminate */
          for(i = 0; replacements[i].name; i++) {
            if(curl_strequal(ptr, replacements[i].name)) {
              ...
              case VAR_ROM_TIME:
                if(CURLE_OK ==
                   curl_easy_getinfo(curl, CURLINFO_ROM_TIME, &doubleinfo))
                  fprintf(stream, "%.3f", doubleinfo);
                break;
              case VAR_REQUEST_TIME:
                if(CURLE_OK ==
                   curl_easy_getinfo(curl, CURLINFO_REQUEST_TIME, &doubleinfo))
                  fprintf(stream, "%.3f", doubleinfo);
                break;
              case VAR_RESPONSE_TIME:
                if(CURLE_OK ==
                   curl_easy_getinfo(curl, CURLINFO_RESPONSE_TIME, &doubleinfo))
                  fprintf(stream, "%.3f", doubleinfo);
                break;
#endif
