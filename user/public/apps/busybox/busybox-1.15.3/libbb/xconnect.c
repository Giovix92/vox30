...
void alarm_wakeup (int signum, siginfo_t* siginfo, void* context)
{
    if(!my_query_data.query_done){
        pthread_kill(query_thread_id, SIGUSR1);
...
void *query_thread(void *arg)
{
    ...
    hint.ai_family = pdata->ai_family;
	/* Needed. Or else we will get each address thrice (or more)
	 * for each possible socket type (tcp,udp,raw...): */
	hint.ai_socktype = SOCK_STREAM;
	hint.ai_flags = pdata->ai_flags;
	rc = getaddrinfo(pdata->domain, NULL, &hint, pdata->result);
    pdata->ret=rc;
    pdata->query_done=1;
    return NULL;
}

int getaddrinfo_r(char *hostname, struct addrinfo *hints, struct addrinfo **result, int timeout)
{
    struct sigaction act, oact;
    sigset_t set, oset;
    int rc = 0;
    struct itimerval tout_val;
    strncpy(my_query_data.domain,hostname,MY_BUFFER_LENGTH);
    my_query_data.domain[MY_BUFFER_LENGTH-1]=0;
    my_query_data.ai_flags= hints->ai_flags;
    my_query_data.ai_family= hints->ai_family;
    my_query_data.ret=1;
    my_query_data.result= result;
    my_query_data.query_done=0;

    tout_val.it_interval.tv_sec = 0;
    tout_val.it_interval.tv_usec = 0;
    tout_val.it_value.tv_sec = timeout;
...
#ifdef __SC_BUILD__
static len_and_sockaddr* str2sockaddr_r(
		const char *host, int port,
IF_FEATURE_IPV6(sa_family_t af,)
		int ai_flags, int timeout)
{
	int rc;
	len_and_sockaddr *r;
	struct addrinfo *result = NULL;
 ...
 skip: ;
	...
	hint.ai_socktype = SOCK_STREAM;
	hint.ai_flags = ai_flags & ~DIE_ON_ERROR;
	rc = getaddrinfo_r(host, &hint, &result, timeout?:5);
	if (rc || !result) {
		bb_error_msg("bad address '%s'", org_host);
		if (ai_flags & DIE_ON_ERROR)
			xfunc_die();
...
#ifdef __SC_BUILD__
len_and_sockaddr* FAST_FUNC xhost_and_af2sockaddr_r(const char *host, int port, sa_family_t af, int timeout)
{
	return str2sockaddr_r(host, port, af, DIE_ON_ERROR, timeout);
}
#endif
#endif
