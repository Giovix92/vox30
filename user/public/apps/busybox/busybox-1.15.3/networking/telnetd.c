...
#ifdef _SC_CLI_
static char g_remote_ipaddr[64] = "";
...
*/
 again:
	...
	if (!IS_INETD && FD_ISSET(master_fd, &rdfdset)) {
        ...
        int nl = sizeof(clientaddr);

        fd = accept(master_fd, (struct sockaddr *)&addr, &addrlen);
        memset(&clientaddr,0x00,sizeof(clientaddr));
        getpeername(fd,(struct sockaddr *)&clientaddr,&nl);

        if(clientaddr.ss_family == AF_INET)
        {
            struct sockaddr_in *s = (struct sockaddr_in *)&clientaddr;
            snprintf(g_remote_ipaddr, sizeof(g_remote_ipaddr), 
                    inet_ntoa(s->sin_addr));
        }
        else
        {
            struct sockaddr_in6 *s = (struct sockaddr_in6 *)&clientaddr;
            inet_ntop(AF_INET6,&s->sin6_addr,g_remote_ipaddr,sizeof(g_remote_ipaddr));
        }
