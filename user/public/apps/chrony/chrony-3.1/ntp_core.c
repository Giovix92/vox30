...
#ifdef __SC_BUILD__
static double get_transmit_delay(NCR_Instance inst, int on_tx, double last_tx, int index);
#else
...
start_initial_timeout(NCR_Instance inst)
{
  ...
  if (last_tx < 0.0)
    last_tx = 0.0;
#ifdef __SC_BUILD__
  delay = get_transmit_delay(inst, 0, 0.0, 0) - last_tx;
#else
...
#ifdef __SC_BUILD__
      restart_timeout(instance, get_transmit_delay(instance, 0, 0.0, 0));
#else
...
#ifdef __SC_BUILD__
static double
get_transmit_delay(NCR_Instance inst, int on_tx, double last_tx, int index)
#else
static double
          ...
          /* Client/server association - aim at some randomised time
            ...
            if(index == 1)
            {
                if(connect_success == 1)
                    delay_time = success_time;
                else
                    delay_time = fail_time * count_num;
                if(count_num > fail_count)
                    count_num = 1;
                else
                    count_num++;
            }
...
#ifdef __SC_BUILD__
  if(inst->success_index == 1)
  {
      connect_success = 0;
      inst->success_index = 0;
  }
#endif
...
#ifdef __SC_BUILD__
  restart_timeout(inst, get_transmit_delay(inst, 1, 0.0, 1));
#else
  ...
  if (valid_packet) {
    ...
    if (inst->opmode != MD_OFFLINE) {
      ...
      delay_time = get_transmit_delay(inst, 0,
                     UTI_DiffTimespecsToDouble(&inst->local_rx.ts, &inst->local_tx.ts), 0);
#else
      delay_time = get_transmit_delay(inst, 0,
