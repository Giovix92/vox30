                 ...
                 )
{
  ...
  {
      if(*sal_ntp_get_syn_status() != '1')
      {
          system("/usr/sbin/rc set_first_use_date restart &");
#ifdef SUPPORT_WIFI
          system("/usr/sbin/rc cron restart wlan_scheduler &");
#endif
#ifdef SUPPORT_SNMP
          system("/usr/sbin/rc snmp restart& 2> /dev/null");
#endif
#ifdef CONFIG_SAFE_NETWORK
	if (0 == access("/usr/sbin/emaze_proxy_server", F_OK))
          system("/usr/sbin/rc safe_network restart &");
	else
	      system("/bin/ubus send System.Time {\\\"Code\\\":1','\\\"Name\\\":\\\"SYSTEM_TIME_SYNCHRONIZED\\\"}");
#endif
      }
	  sal_ntp_set_syn_status(NTP_STATUS_OK);
	  sal_ntp_set_last_status(NTP_STATUS_OK);
	  log_sys(LOG_INFO, CRIT_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Sync up with NTP Server successfully.\n");
 
      system("/usr/sbin/cmset -u &");
  }
  else
  {
	  sal_ntp_set_syn_status(NTP_STATUS_FAIL);
	  sal_ntp_set_last_status(NTP_STATUS_FAIL);
	  log_sys(LOG_INFO, CRIT_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Sync up with NTP Server failed.\n");
  }
#endif
...
REF_SetUnsynchronised(void)
{
  ...
  sal_ntp_set_syn_status(NTP_STATUS_FAIL);
  sal_ntp_set_last_status(NTP_STATUS_FAIL);
  log_sys(LOG_INFO, CRIT_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "NTP Server  error failed to sync.\n");
#endif
