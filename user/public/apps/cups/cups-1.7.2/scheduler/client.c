...
static void do_clear_caches(void)
{
        ...
        do
	{
	  ...
	  else if (bytes > 0)
	  {
        ...
        if(!is_ram_enough())
        {
            ...
            do_clear_caches();
            log_usb(LOG_ERR, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "[Client %d] HTTP_STATE_PUT_RECV Closing because file is too big\n",con->http.fd);
            cupsdSendError(con, HTTP_STATUS_REQUEST_TOO_LARGE, CUPSD_AUTH_NONE);
