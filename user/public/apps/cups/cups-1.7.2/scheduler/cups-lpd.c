...
#define LIMIT_FREE_SIZE (10*1024*1024)
#include <sys/sysinfo.h>
#include <stdlib.h>

static int is_ram_enough(int file_size)  
{
    struct sysinfo info;
    int free_size = 0, ret = 0;
    ...
    sysinfo(&info);
    free_size = info.freeram;

    if (free_size <= LIMIT_FREE_SIZE)
    {
        /* file and job is less than 10% of RAM */
        if (file_size * 2 > free_size * 0.1) 
            ret = 0;
        else
            ret = 1;
    }
    ...
    {
        if ((file_size * 2 - LIMIT_FREE_SIZE * 0.1) > (free_size - LIMIT_FREE_SIZE))  
            ret = 0;
        else
            ret = 1;
    }
...
#ifdef __SC_BUILD__    
          if (!is_ram_enough(atoi(count)))
          {
              syslog(LOG_ERR, "Request Size is too large");
              putchar(1);
              status = 1;
              break;
          }
