...
#define	CACHE_TIMEUNIT		60
	 ...
	 */
#ifdef __SC_BUILD__
#define	CACHE_NEGTIME		1//	(    5 * CACHE_TIMEUNIT)
#else
...
#ifdef __SC_BUILD__
static void dump_dnrd_cache(void)
{
    FILE *fp;
    cache_t	*cx = NULL;

    fp = fopen("/tmp/dnrd_cache", "w+");
    if(fp)
    {
        for (cx = cachelist; cx != NULL; cx = cx->next)
	{
            fprintf(fp, "domain_name =%s create_time =%lu expire_time =%lu \n", cx->name, cx->created, cx->expires);
	}
        fclose(fp);
    }
    return;
...
int cache_dnspacket(void *packet, int len, srvnode_t *server)
{
    ...
    cx->expires  = cx->lastused +
	           ((cx->p->ancount > 0) ? query.ttl : CACHE_NEGTIME);
...
int cache_lookup(void *packet, int len)
{
	...
	if (cx->positive > 0) {
	  cx->lastused = time(NULL);
	  /* Remove this since we need not refresh expires according to the last used time. */
 ...
 */
static int expire_oldest(unsigned int itemcount)
{
    int	     i, n;
    cache_t *cx, **cv;
    ...
    for (i = 0, cx = cachelist; i < itemcount && cx != NULL;  i++, cx = cx->next) {
	cv[i] = cx;
    }
...
int cache_expire(void)
{
	...
	/* Only expire the cache dns entry according to the TTL */
    ...
    if (expired > 0) {
	log_debug(2, "cache: %u of %u expired, %u remaining\n",
...
void cache_expire_by_domain(char* name)
{
	cache_t	  *cx, *next;
