...
char provisioncode[128] = {0};
char mainsubnet[32] = {0};
char mainmask[32] = {0};
#ifdef CONFIG_SUPPORT_FON
char fonsubnet[32] = {0};
char fonmask[32] = {0};
char ofonsubnet[32] = {0};
char ofonmask[32] = {0};
#endif
#endif
#ifdef SCM_BINDING
/* Communication socket - Local socket */
int csock = -1;
struct sockaddr_un dnrd_local;
const char *dnrd_ls_path = "/tmp/dnrd.cls";
#endif
...
void log_msg(int type, const char *fmt, ...)
{
    ...
    log_dns(LOG_INFO, NORM_LOG, 0, 0, "%s\n", buf);
    va_end(ap);	
#endif
#endif
}
