...
#ifdef __SC_BUILD__
	    if ((k = get_objectname(msg, limit, &p, string, RR_NAMESIZE, k, ++depth))<0)
#else
...
#ifdef __SC_BUILD__
    k = get_objectname((unsigned char *)x->packet, limit, &here, y->name, sizeof(y->name), 0, 0);
#else
...
unsigned char *parse_query(rr_t *y, unsigned char *msg, int len)
{
    int	k;
    unsigned char *here;
    unsigned char *ttl_here = (unsigned char *)"";
    unsigned long ttl_conv = 0;
    unsigned short int conv;
    ...
    */
...
#ifdef __SC_BUILD__
    if ((k=get_objectname(msg, limit, &here, y->name, sizeof(y->name),0, 0))< 0) 
#else
    ...
    if(ntohs(((dnsheader_t *)msg)->ancount) > 0)
    {
        ttl_here = here + 6; /* skip Name, Type and Class field */
        memcpy(&ttl_conv, ttl_here, sizeof(unsigned long));
        y->ttl = ntohl(ttl_conv);
    }
