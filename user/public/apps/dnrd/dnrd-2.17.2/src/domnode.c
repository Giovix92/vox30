...
domnode_t *del_domain(domnode_t *head, char *cname, const int maxlen) {
    ...
    while (p->next != head) {
        if (strncmp(p->next->domain, cname, maxlen) == 0) {
            destroy_domnode(p->next);
            p->next = (p->next)->next;
            break;
        }
        p = p->next;
...
domnode_t *search_domnode_by_dnametype(domnode_t *head, const char *name, uint32_t dtype) {
  ...
  while ((d=d->next) != head) {
    if (strcmp(d->domain, name) == 0) {
        if(dtype == d->domain_type)
        return d;
    }
  }
  return NULL;
...
#endif
{
  ...
  if ((name == NULL) || (d == NULL)) return head;
#if (defined(CONFIG_SUPPORT_IPV6) && (!defined(CONFIG_USE_DNSMASQ_IPV6_ONLY)))
    inet_ntop(AF_INET6, &fromaddrp->sin6_addr, buf, sizeof(buf));
#else
    snprintf(buf, sizeof(buf), "%s", inet_ntoa(fromaddrp->sin_addr));
#endif
//match source ip address
    ip = make_cname(buf, sizeof(buf)); 
 
  while ((d=d->next) != head) {
    if (0 == strncmp(d->domain, ip, strlen(ip)))
    {
        curr = d;
        break;
    }
  ...
  if (s) {
    ...
    if (d->roundrobin) {
      log_debug(3, "Setting server %s for domain %s",
		inet_ntop(AF_INET6,&s->addr.sin6_addr,buf, sizeof(buf)), cname2asc(d->domain));
    } else {
...
*/
#ifdef __SC_BUILD__
srvnode_t *next_active(domnode_t *d, int local) {
#else
...
#ifdef __SC_BUILD__
  if (DOMAIN_TYPE_UNKNOWN == d->domain_type || d->domain_type == -1)
    for (s=start->next; (s->inactive || (s->local != local)) && (s != start); s = s->next);
  else
