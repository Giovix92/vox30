...
int main(int argc, char *argv[])
{
  /*    int                i;*/
    ...
    /*martin_huang@sdc.sercomm.com: we set pid earlier for detecting dnrd process*/
#ifndef DNRD_DAEMON
...
#ifdef CONFIG_SUPPORT_CORE_DUMP
#else
    /*
     *miscCrash_register(NULL);
     */
#endif
    /*
     ...
     */
    /*martin_huang@sdc.sercomm:this is not safe, actually this should be no use*/
    //kill_current();
#endif
...
#if (defined(__SC_BUILD__) && defined(CONFIG_SUPPORT_IPV6) && (!defined(CONFIG_USE_DNSMASQ_IPV6_ONLY)))
    recv_addr.sin6_port = servent ? servent->s_port : htons(53);
    /*
     * Setup our DNS query reception socket for ipv6.
     */
    if ((isock = socket(AF_INET6, SOCK_DGRAM, 0)) < 0) {
...
#ifdef SCM_BINDING
    /*
     * Setup DNS communication socket.
     */
    unlink(dnrd_ls_path);
    if((csock = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0) {
        log_msg(LOG_ERR, "csock: Couldn't open socket");
        cleanexit(-1);
    }
    
    memset(&dnrd_local, 0, sizeof(dnrd_local));
    dnrd_local.sun_family = AF_LOCAL;
    strcpy(dnrd_local.sun_path, dnrd_ls_path);
    
    if(bind(csock, (struct sockaddr *)&dnrd_local, sizeof(dnrd_local)) < 0) {
	    log_msg(LOG_ERR, "tcpsock: Couldn't bind local address");
	    cleanexit(-1);
    }
#endif
	...
	*/
     ...
     */
#ifdef __SC_BUILD__
    DNS_NOTIFY_REG_IN("server in");
#endif
    run();
#ifdef __SC_BUILD__
    DNS_NOTIFY_REG_OUT("server out");
#endif
