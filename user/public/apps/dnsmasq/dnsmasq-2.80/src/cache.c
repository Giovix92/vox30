...
static struct crec *cache_scan_free(char *name, struct all_addr *addr, time_t now, unsigned short flags,
				    struct crec **target_crec, unsigned int *target_uid, int dtype)
#else
static struct crec *cache_scan_free(char *name, struct all_addr *addr, time_t now, unsigned short flags,
...
#ifdef __SC_BUILD__
                (dtype == 0 || dtype == crecp->dtype || (dtype == -1 && crecp->dtype == 1)) &&
#endif
...

{
...
#ifdef __SC_BUILD__
  if ((new = cache_scan_free(name, addr, now, flags, &target_crec, &target_uid, dtype)))
#else
  ...
  if (!target_crec)
...
#ifdef __SC_BUILD__
	  cache_scan_free(cache_get_name(new), &free_addr, now, new->flags, NULL, NULL, dtype);
#else
	...
	{
#ifdef __SC_BUILD__
        cache_scan_free(NULL, NULL, now, 0, NULL, NULL, dtype);
#else
...
#ifdef __SC_BUILD__
    new->dtype = dtype;
    memcpy(new->client_mac, client_mac, DHCP_CHADDR_MAX); 
#endif
  new->next = new_chain;
  new_chain = new;
...
{
  MY_CFPRINTF("###### cache_find_by_name start \n");
  struct crec *ans;
  int no_rr = prot & F_NO_RR;

#ifdef __SC_BUILD__
    int check_local = prot & F_NO_RD;
	prot &= ~F_NO_RD;
#endif
  prot &= ~F_NO_RR;
	  ...
	  if (!is_expired(now, crecp) && !is_outdated_cname_pointer(crecp))
	    {
		  ...
		  (crecp->flags & prot) &&
#ifdef __SC_BUILD__
          ((dtype == 0 || crecp->dtype == 0) || (dtype == -1 && crecp->dtype == 1) || (dtype == crecp->dtype)) &&
#endif
		  hostname_isequal(cache_get_name(crecp), name))
		{
#ifdef __SC_BUILD__
            if(option_bool(OPT_ADD_MAC) && strcmp(crecp->client_mac, "") != 0)
            {
                if(strcmp(client_mac, crecp->client_mac) == 0)
                {
                    if (crecp->flags & (F_HOSTS | F_DHCP | F_CONFIG))
                    {
                        *chainp = crecp;
...
#ifdef __SC_BUILD__
        if (check_local 
                && !(ans->flags &(F_HOSTS | F_DHCP | F_CONFIG))) {
            my_syslog(LOG_INFO, "name[%s], no recursive diseried, ans->flags=%08x\n", cache_get_name(ans), ans->flags);
            return NULL;
        }
#endif
...
{  
  ...
  while ((atnl = gettok(f, token)) != EOF)
	...
	{
#ifdef __SC_BUILD__
	  my_syslog(LOG_INFO, _("bad address at %s line %d"), filename, lineno); 
#else
...
#ifdef __SC_BUILD__
	    my_syslog(LOG_INFO, _("bad name at %s line %d"), filename, lineno); 
#else
