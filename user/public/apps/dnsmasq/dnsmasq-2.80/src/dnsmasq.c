...
int main (int argc, char **argv)
{
  ...
  if (option_bool(OPT_EXTRALOG))
    ...
    INIT_LIST_HEAD(&(daemon->head_filterlist));
#endif
    daemon->redirect_ip = safe_malloc(512);
    daemon->redirect_ipv6 = safe_malloc(512);
    memset(daemon->redirect_ip,0,512);
    memset(daemon->redirect_ipv6,0,512);
...
#ifdef __SC_BUILD__
          if (!read_write(fd, (unsigned char *)daemon->namebuff, strlen(daemon->namebuff), 0, 0))
#else
...
#ifdef __SC_BUILD__
  if (!read_write(fd, (unsigned char *)evp, sizeof(struct event_desc), 1, 0))
#else
...
#ifdef __SC_BUILD__
      read_write(fd, (unsigned char *)buf, evp->msg_sz, 1, 0)
#else
...
#ifdef __SC_BUILD__
        if (daemon->dynamic_servers_dir->fname) {
            read_servers_dir();
            check = 1;
        }
#ifdef CONFIG_SUPPORT_WEB_URL_FILTER
        if(daemon->urlfilter_file)
        {
            read_url_filter_file();
        }
        else
            my_syslog(LOG_WARNING, "cannot find url filter file");
#endif
#ifdef CONFIG_SUPPORT_SECURE_NET
        if(daemon->edns0_if_file)
        {
            read_edns0_interface_file();
        }
#endif
#endif
