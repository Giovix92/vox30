...
#ifdef __SC_BUILD__
#define DNSMASQ_CFG_LOCK                          "/var/lock/dnsmasq_cfg.lock"
#define DNSMASQ_CFG_DIR                           "/tmp/dnsmasq.d"
#define DNS_REDIRECT_CFG                          DNSMASQ_CFG_DIR"/dnsmasq.redirect.cnf"
#define DNS_SERVERS_DIR                           "/tmp/dnsmasq_servers.d"
#define DNS_DEFAULT_WAN_CFG                       DNS_SERVERS_DIR"/dnsmasq.default.wan.conf"
#define DNS_DEFAULT_WAN_MAN_CFG                       DNS_SERVERS_DIR"/dnsmasq.default.wan.man.conf"
#define DNS_NTP_WAN_CFG                           DNS_SERVERS_DIR"/dnsmasq.ntp.wan.conf"
#define DNS_VOIP_WAN_CFG                          DNS_SERVERS_DIR"/dnsmasq.voip.wan.conf"
#define DNS_IPTV_WAN_CFG                          DNS_SERVERS_DIR"/dnsmasq.iptv.wan.conf"
#define DNS_TR069_WAN_CFG                         DNS_SERVERS_DIR"/dnsmasq.tr069.wan.conf"
#define DNS_IPPHONE_WAN_CFG                       DNS_SERVERS_DIR"/dnsmasq.ipphone.wan.conf"
#define DNS_NAMESRV_CFG                           DNSMASQ_CFG_DIR"/dnsmasq.namesrv.cnf"
...
#ifdef __SC_BUILD__
#define SERV_OPT_FROM_DATA_CONF             1  /**/
#define SERV_OPT_FROM_DATA_MAN_CONF         2  /**/
#define SERV_OPT_FROM_TR069_CONF            3  /**/
#define SERV_OPT_FROM_NTP_CONF              4  /**/
#define SERV_OPT_FROM_VOIP_CONF             5  /**/
#define SERV_OPT_FROM_IPPHONE_CONF          6  /**/
#define SERV_OPT_FROM_NAME_SRV              7
#define MAX_DNS_SERVER                      7 
#define SERV_OPT_FROM_IPTV_CONF             8  /**/
#endif
...
#ifdef __SC_BUILD__
#define SERVERS_OPT 64
#define CONF_DIR_OPT 128
#endif
struct hostsfile {
...
#ifdef __SC_BUILD__
  int plen;
  char orig_name[MAXDNAME],new_name[MAXDNAME];
#endif
  time_t time;
  unsigned char *hash[HASH_SIZE];
#ifdef __SC_BUILD__
    struct timeval tv;
    char header[EDNS_PKTSZ];
    int try_count;
    int curr_srv_pri;
    int pri;
    int retry;
    int dtype;                /*Domain type*/
    char resp_err;
    char islast;
    int v4counter;
    int v6counter;
...
#ifdef __SC_BUILD__
#define STANDARD_DEVIATION 10000
#define DEFAULT_ID -1
#define DEFAULT_QUERY_TIMEOUT 300000
#define DEFAULT_QUERY_TCP_TIMEOUT 1000000
#define DEFAULT_QUERY_TCP_CONNECT_TIMEOUT 2
    int id;
#endif
  ...
  char *redirect_ipv6;
#endif
  struct servers_dir *dynamic_servers_dir;
  struct conf_dir *dynamic_conf_dir;
  int   query_timeout;
  int   query_tcp_timeout;
  int   query_tcp_connect_timeout;
  int   random_bit20;
#ifdef CONFIG_SUPPORT_WEB_URL_FILTER
  char  *urlfilter_file;
  struct list_head head_filterlist;
#endif
#ifdef CONFIG_SUPPORT_SECURE_NET
  char *edns0_if_file;
  struct edns0_interface_info if_info[IF_MAX_NUM];
#endif
#endif
...
struct crec *cache_insert(char *name, struct all_addr *addr,
			  time_t now, unsigned long ttl, unsigned short flags, int dtype, unsigned char *client_mac);
struct crec *cache_find_by_name(struct crec *crecp, 
				char *name, time_t now, unsigned int prot, int dtype, unsigned char *client_mac);
#else
struct crec *cache_insert(char *name, struct all_addr *addr,
...
int extract_addresses(struct dns_header *header, size_t qlen, char *name,
		      ...
		      int no_cache_dnssec, int secure, int *doctored, int dtype, unsigned char *client_mac);
size_t answer_request(struct dns_header *header, char *limit, size_t qlen,  
		      struct in_addr local_addr, struct in_addr local_netmask, 
		      time_t now, int ad_reqd, int do_bit, int have_pseudoheader, int dtype, unsigned char *client_mac);
int check_for_bogus_wildcard(struct dns_header *header, size_t qlen, char *name, 
			     struct bogus_addr *baddr, time_t now, int dtype, unsigned char *client_mac);
#else
int extract_addresses(struct dns_header *header, size_t qlen, char *name,
		      time_t now, char **ipsets, int is_sign, int check_rebind,
...
size_t answer_auth(struct dns_header *header, char *limit, size_t qlen, 
		   ...
		   int do_bit, int have_pseudoheader, int dtype, unsigned char *client_mac);
#else
size_t answer_auth(struct dns_header *header, char *limit, size_t qlen, 
		   time_t now, union mysockaddr *peer_addr, int local_query,
...
#ifdef __SC_BUILD__
int dnssec_validate_by_ds(time_t now, struct dns_header *header, size_t plen, char *name, char *keyname, int class, int dtype, unsigned char *client_mac);
int dnssec_validate_ds(time_t now, struct dns_header *header, size_t plen, char *name, char *keyname, int class, int dtype, unsigned char *client_mac);
int dnssec_validate_reply(time_t now, struct dns_header *header, size_t plen, char *name, char *keyname, int *class,
			  int check_unsigned, int *neganswer, int *nons, int dtype, unsigned char *client_mac);
#else
...
#ifdef __SC_BUILD__
int read_write(int fd, unsigned char *packet, int size, int rw, int timeout);
int connect_timeout(int fd, const struct sockaddr *addr, socklen_t addrlen, int wait_time);
#else
...
char *parse_server(char *arg, union mysockaddr *addr, 
		   union mysockaddr *source_addr, char *interface, int *srv_pri, int *flags);
#else
char *parse_server(char *arg, union mysockaddr *addr, 
...
#ifdef __SC_BUILD__
void scan_servers_list(int flag);
#endif
void cleanup_servers(void);
#ifdef __SC_BUILD__
void cleanup_servers_by_flag(int flag);
void cleanup_servers_by_domain_type(int flag);
#endif
...
void resume_domain(struct dns_header *header, size_t qlen, char *rname);
void create_newname_follow_bit20(char *cname);
#endif
#endif
...
#ifdef __SC_BUILD__
extern int util_lock_reg(int fd, short type, int retry_time);
extern int util_lock(char *file, int retry_time);
extern void util_unlock(int fd);
#endif
