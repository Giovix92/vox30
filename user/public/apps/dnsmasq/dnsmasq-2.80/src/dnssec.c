...
static int validate_rrset(time_t now, struct dns_header *header, size_t plen, int class, int type, int sigidx, int rrsetidx, 
			  char *name, char *keyname, char **wildcard_out, struct blockdata *key, int keylen, int algo_in, int keytag_in, int dtype, unsigned char *client_mac)
#else
static int validate_rrset(time_t now, struct dns_header *header, size_t plen, int class, int type, int sigidx, int rrsetidx, 
	    ...
	    
	    {
		 ...
		 we can't move this outside the loop. */
		  ...
		  validate_rrset(now, header, plen, class, T_DNSKEY, sigcnt, rrcnt, name, keyname, 
				 NULL, key, rdlen - 4, algo, keytag, dtype, client_mac) == STAT_SECURE
#else
		  validate_rrset(now, header, plen, class, T_DNSKEY, sigcnt, rrcnt, name, keyname, 
...
#ifdef __SC_BUILD__
    rc = dnssec_validate_reply(now, header, plen, name, keyname, NULL, 0, &neganswer, &nons, dtype, client_mac);
#else
...
*/
#ifdef __SC_BUILD__
static int zone_status(char *name, int class, char *keyname, time_t now, int dtype, client_mac)
#else
...
int dnssec_validate_reply(time_t now, struct dns_header *header, size_t plen, char *name, char *keyname, 
			  int *class, int check_unsigned, int *neganswer, int *nons, int dtype, unsigned char *client_mac)
#else
int dnssec_validate_reply(time_t now, struct dns_header *header, size_t plen, char *name, char *keyname, 
  ...
  for (p1 = ans_start, i = 0; i < ntohs(header->ancount) + ntohs(header->nscount); i++)
      ...
      else
	{
	  ...
	  if (sigcnt == 0)
		...
		{
#ifdef __SC_BUILD__
		  rc = zone_status(name, class1, keyname, now, dtype, client_mac);
#else
...
#ifdef __SC_BUILD__
	      rc = zone_status(daemon->workspacename, class1, keyname, now, dtype, client_mac);
#else
	      ...
	      if (rc == STAT_SECURE)
		{
		  ...
		  rc = validate_rrset(now, header, plen, class1, type1, sigcnt,
				      rrcnt, name, keyname, &wildname, NULL, 0, 0, 0, dtype, client_mac);
#else
		  rc = validate_rrset(now, header, plen, class1, type1, sigcnt,
...
#ifdef __SC_BUILD__
	      if ((rc = zone_status(name, qclass, keyname, now, dtype, client_mac)) != STAT_SECURE)
#else
