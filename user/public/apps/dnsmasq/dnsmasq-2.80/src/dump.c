...
void dump_init(void)
{
  ...
  if (stat(daemon->dump_file, &buf) == -1)
...
#ifdef __SC_BUILD__
	  !read_write(daemon->dumpfd, (void *)&header, sizeof(header), 0, 0)
#else
...
#ifdef __SC_BUILD__
       !read_write(daemon->dumpfd, (void *)&header, sizeof(header), 1, 0)
#else
  ...
  else
    {
      ...
      while (
#ifdef __SC_BUILD__
             read_write(daemon->dumpfd, (void *)&pcap_header, sizeof(pcap_header), 1, 0)
#else
             read_write(daemon->dumpfd, (void *)&pcap_header, sizeof(pcap_header), 1)
#endif
             )
	{
...
void dump_packet(int mask, void *packet, size_t len, union mysockaddr *src, union mysockaddr *dst)
{
  ...
  if (rc == -1 ||
#ifdef __SC_BUILD__
      !read_write(daemon->dumpfd, (void *)&pcap_header, sizeof(pcap_header), 0, 0) ||
      !read_write(daemon->dumpfd, iphdr, ipsz, 0, 0) ||
      !read_write(daemon->dumpfd, (void *)&udp, sizeof(udp), 0, 0) ||
      !read_write(daemon->dumpfd, (void *)packet, len, 0, 0)
#else
