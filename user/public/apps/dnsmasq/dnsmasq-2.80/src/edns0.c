...
int check_need_to_be_add(unsigned int ip)
{
    int i = 0;
    for(i = 0; i < IF_MAX_NUM; i++)
    {
        if(daemon->if_info[i].ip)
        {
            if((daemon->if_info[i].ip & daemon->if_info[i].mask) == (ip & daemon->if_info[i].mask))
                return 1;
        }
    }
...
#ifdef CONFIG_SUPPORT_SECURE_NET
  if(daemon->if_info[0].ip && check_need_to_be_add((unsigned int)(l3->in.sin_addr.s_addr)) == 0)
      return plen;
#endif
