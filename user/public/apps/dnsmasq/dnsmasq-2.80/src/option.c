...
#define LOPT_CAA           356
 #ifdef __SC_BUILD__
#define LOPT_SERVERS_DIR   357
#define LOPT_SERVERS_DIR_INTIFY 358
#define LOPT_QUERY_TIMEOUT 359
#define LOPT_QUERY_TCP_TIMEOUT 360
#define LOPT_QUERY_TCP_CONNECT_TIMEOUT 361
#define LOPT_DNSBIT20 362
#ifdef CONFIG_SUPPORT_WEB_URL_FILTER
#define LOPT_URL_FILTER_FILE 363
#endif
#ifdef CONFIG_SUPPORT_SECURE_NET
#define LOPT_EDNS0_INTERFACE_FILE 364
#endif
#endif
...
#ifdef __SC_BUILD__
    { "servers-dir", 1, 0, LOPT_SERVERS_DIR },
    { "query-timeout", 1, 0, LOPT_QUERY_TIMEOUT },
    { "query-tcp-timeout", 1, 0, LOPT_QUERY_TCP_TIMEOUT },
    { "query-tcp-connect-timeout", 1, 0, LOPT_QUERY_TCP_CONNECT_TIMEOUT },
    { "dns-random-bit20", 1, 0, LOPT_DNSBIT20 },
#ifdef CONFIG_SUPPORT_WEB_URL_FILTER
    { "url-filter-file", 1, 0, LOPT_URL_FILTER_FILE },
#endif
#ifdef CONFIG_SUPPORT_SECURE_NET
    { "edns0-interface-file", 1, 0, LOPT_EDNS0_INTERFACE_FILE },
#endif
#endif
    { "mx-host", 1, 0, 'm' },
    { "mx-target", 1, 0, 't' },
...
#ifdef __SC_BUILD__
	{ LOPT_SERVERS_DIR, ARG_ONE, "<path>", gettext_noop("Specify path to dir with server= options"), NULL },
#endif
...
char *parse_server(char *arg, union mysockaddr *addr, union mysockaddr *source_addr, char *interface 
#ifdef __SC_BUILD__
                   , int *srv_pri
#endif
                   , int *flags)
...
#ifdef __SC_BUILD__
static int one_opt(int option, char *arg, char *errstr, char *gen_err, int command_line, int servers_only, int domain_type_flags)
#else
...
{      
  ...
  switch (option)
	...
	comma = split(arg);
#ifdef __SC_BUILD__
    struct conf_dir *conf_new = opt_malloc(sizeof(struct conf_dir));
    if (!(conf_new->fname= opt_string_alloc(arg)))
        break;
    conf_new->flags = 0;
    daemon->dynamic_conf_dir = conf_new;
    conf_new->flags |= CONF_DIR_OPT;
#else
...
#else
    if (!(dir_stream = opendir(daemon->dynamic_conf_dir->fname)))
      die(_("cannot access directory %s: %s"), daemon->dynamic_conf_dir->fname, EC_FILE);
#endif
	...
	while ((ent = readdir(dir_stream)))
	  {
	    ...
	    if (li)
	      continue;
#ifdef __SC_BUILD__
        path = opt_malloc(strlen(daemon->dynamic_conf_dir->fname) + len + 2);
        strcpy(path, daemon->dynamic_conf_dir->fname);
#else
    ...
    case LOPT_SERVERS_FILE:
      ...
      break;
#ifdef __SC_BUILD__
    case LOPT_SERVERS_DIR: {
      struct servers_dir *servers_new = opt_malloc(sizeof(struct servers_dir));
      servers_new->fname = opt_string_alloc(arg);
      servers_new->flags = 0;
      daemon->dynamic_servers_dir = servers_new;
      servers_new->flags |= SERVERS_OPT;
      break;
    }
#ifdef CONFIG_SUPPORT_WEB_URL_FILTER
    case LOPT_URL_FILTER_FILE: {
      daemon->urlfilter_file = opt_string_alloc(arg);
      break;
    }
#endif
#ifdef CONFIG_SUPPORT_SECURE_NET
    case LOPT_EDNS0_INTERFACE_FILE: {
      daemon->edns0_if_file = opt_string_alloc(arg);
      break;
    }
#endif
    case LOPT_QUERY_TIMEOUT: {
      daemon->query_timeout = atoi(arg);            
      break;
    }
    case LOPT_QUERY_TCP_TIMEOUT: {
      daemon->query_tcp_timeout = atoi(arg);            
      break;
    }
    case LOPT_QUERY_TCP_CONNECT_TIMEOUT: {
      daemon->query_tcp_connect_timeout = atoi(arg);            
      break;
    }
    case LOPT_DNSBIT20:{
      daemon->random_bit20 = atoi(arg);
      break;
    }
#endif
    case 'm':  /* --mx-host */
...
#  endif
...
#ifdef __SC_BUILD__
    newlist->domain_type = domain_type_flags;
    MY_CFPRINTF("###### domain_type_flags is [%d]\n", newlist->domain_type);
#endif
	...
	if (!arg || !*arg)
	  ...
	  {
        MY_CFPRINTF("###### one_opt arg is %s type [%d]\n", arg, newlist->flags);
	    char *err = parse_server(arg, &newlist->addr, &newlist->source_addr, newlist->interface
#ifdef __SC_BUILD__
                                 , &newlist->srv_pri
#endif
	...
	string = parse_server(comma, &serv->addr, &serv->source_addr, serv->interface
#ifdef __SC_BUILD__
                          , &serv->srv_pri
#endif
                          , &serv->flags);
...
#ifdef __SC_BUILD__
    serv->domain_type = domain_type_flags;
#endif
	if (servers_only) {
...
{
#ifdef __SC_BUILD__
	volatile  int domain_type_flags = 0;
	if (strcmp(file,DNS_DEFAULT_WAN_CFG) == 0)
		domain_type_flags = SERV_OPT_FROM_DATA_CONF;
	if (strcmp(file,DNS_DEFAULT_WAN_MAN_CFG) == 0)
		domain_type_flags = SERV_OPT_FROM_DATA_MAN_CONF;
	else if (strcmp(file,DNS_NTP_WAN_CFG) == 0)
		domain_type_flags = SERV_OPT_FROM_NTP_CONF;
	else if (strcmp(file,DNS_VOIP_WAN_CFG) == 0)
		domain_type_flags = SERV_OPT_FROM_VOIP_CONF;
	else if (strcmp(file,DNS_IPTV_WAN_CFG) == 0)
		domain_type_flags = SERV_OPT_FROM_IPTV_CONF;
	else if (strcmp(file,DNS_TR069_WAN_CFG) == 0)
		domain_type_flags = SERV_OPT_FROM_TR069_CONF;
	else if (strcmp(file,DNS_IPPHONE_WAN_CFG) == 0)
		domain_type_flags = SERV_OPT_FROM_IPPHONE_CONF;
	else
		domain_type_flags = 0;
#endif
  ...
  while (fgets(buff, MAXDNAME, f))
    {
      int white, i;
#ifdef __SC_BUILD__
      volatile int option = ((hard_opt == LOPT_REV_SERV) || (hard_opt == 65)) ? 0 : hard_opt;
#else
...
#ifdef __SC_BUILD__
      if (option != 0 || hard_opt == LOPT_REV_SERV || hard_opt == 65)
#else
...
#ifdef __SC_BUILD__
      if (errmess || !one_opt(option, arg, daemon->namebuff, _("error"), 0, hard_opt == LOPT_REV_SERV, domain_type_flags))
#else 
...
#ifdef __SC_BUILD__
  if (strcmp(file,DNS_REDIRECT_CFG) == 0) {
      return 1;
...
void read_edns0_interface_file(void)
{
  char buf[256] = {0};
  int i = 0;
  FILE *f = NULL;
  char ip[32] = "";
  char mask[32] = "";
  memset(daemon->if_info,0,sizeof(daemon->if_info));
  if (!(f = fopen(daemon->edns0_if_file, "r")))
  {
      my_syslog(LOG_ERR, "cannot read %s: %s",daemon->edns0_if_file, strerror(errno));
      return;
  }
  
  while (fgets(buf, sizeof(buf), f) && (i<IF_MAX_NUM))
  {
      sscanf(buf, "%[^,],%s", ip, mask);
      daemon->if_info[i].ip = convert_ip_to_unsignedint(ip);
      daemon->if_info[i].mask = convert_ip_to_unsignedint(mask);
      i++;
  }
  fclose(f);
}
...
int add_domain_mac_list(char *domain, char *mac)
{
    ...
    list_add(&domain_node->list, &(daemon->head_filterlist));

    return 0;
}

void read_url_filter_file(void)
{
  char buf[256] = {0};
  FILE *f = NULL;
  char *sep = NULL;
  char *domain = NULL;
  char *mac = NULL;
  if (!(f = fopen(daemon->urlfilter_file, "r")))
  {
      my_syslog(LOG_ERR, "cannot read %s: %s",daemon->urlfilter_file, strerror(errno));
      return;
  }
  cleanup_filterlist();
  ...
  while (fgets(buf, sizeof(buf), f))
  {
      ...
      if (sep)
      {
          *sep = '\0';
          ...
          mac = sep + 1;
          mac[17] = '\0';
          add_domain_mac_list(domain,mac);
      }
  }
  fclose(f);
}
#endif
void read_servers_dir(void)
{
	int fd = 0;
	DIR *servers_dir;
	FILE *f;
	...
	*/
	fd = util_lock(DNSMASQ_CFG_LOCK, 0);
	if (!(servers_dir= opendir(daemon->dynamic_servers_dir->fname))) {
		//my_syslog(LOG_ERR, _("cannot read %s: %s"), daemon->servers_dir, strerror(errno));
		util_unlock(fd);
		return;
	}
	while ((filename = readdir(servers_dir)) != NULL) {
		if (!strcmp(filename->d_name,".") || !strcmp(filename->d_name,".."))
			continue;
		sprintf(file_path,"%s/%s",daemon->dynamic_servers_dir->fname,filename->d_name);
		if (!(f = fopen(file_path, "r"))) {
			my_syslog(LOG_ERR, _("cannot read %s: %s"), file_path, strerror(errno));
			util_unlock(fd);
			return;
		}
		if (strcmp(file_path,DNS_DEFAULT_WAN_CFG) == 0)
			read_file(file_path, f, LOPT_REV_SERV);
		if (strcmp(file_path,DNS_DEFAULT_WAN_MAN_CFG) == 0)
			read_file(file_path, f, LOPT_REV_SERV);
		//one_file(file_path,0);
		if (strcmp(file_path,DNS_NTP_WAN_CFG) == 0)
			read_file(file_path, f, LOPT_REV_SERV);
		//one_file(file_path,0);
		if (strcmp(file_path,DNS_VOIP_WAN_CFG) == 0)
			read_file(file_path, f, LOPT_REV_SERV);
		if (strcmp(file_path,DNS_IPTV_WAN_CFG) == 0)
			read_file(file_path, f, LOPT_REV_SERV);
		//one_file(file_path,0);//one file will missed ,no reread
		if (strcmp(file_path,DNS_TR069_WAN_CFG) == 0)
			read_file(file_path, f, LOPT_REV_SERV);
        if (strcmp(file_path,DNS_IPPHONE_WAN_CFG) == 0)
			read_file(file_path, f, LOPT_REV_SERV);

		//one_file(file_path,0);
		//read_file(file_path, f, 0);//some bad effect on conf-dir
	}
	closedir(servers_dir);
	util_unlock(fd);
}
#endif
...
#ifdef __SC_BUILD__
  set_dynamic_inotify(AH_DHCP_HST | AH_DHCP_OPT | SERVERS_OPT | CONF_DIR_OPT, 0, NULL, 0);
#else
...
#ifdef __SC_BUILD__
    daemon->id = DEFAULT_ID;
    daemon->query_timeout = DEFAULT_QUERY_TIMEOUT;
    daemon->query_tcp_timeout = DEFAULT_QUERY_TCP_TIMEOUT;
    daemon->query_tcp_connect_timeout = DEFAULT_QUERY_TCP_CONNECT_TIMEOUT;
#endif
...
#ifdef HAVE_GETOPT_LONG
#ifdef __SC_BUILD__
	  if (!one_opt(option, arg, daemon->namebuff, _("try --help"), 1, 0, 0))
#else
	  if (!one_opt(option, arg, daemon->namebuff, _("try --help"), 1, 0))
#endif
#else 
#ifdef __SC_BUILD__
	    if (!one_opt(option, arg, daemon->namebuff, _("try -w"), 1, 0, 0)) 
#else
