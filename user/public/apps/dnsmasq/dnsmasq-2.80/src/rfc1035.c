...
#ifdef __SC_BUILD__
static int resume_name(struct dns_header *header, size_t plen, unsigned char **pp, int extrabytes, char *rname)
{
  ...
  while (1)
	...
	{ /* label_type = 0 -> label. */
	  ...
	  for(j=0; j<l; j++, p++,rname++)
      {
		if (!isascii(*p) || iscntrl(*p) || *p == '.')
		  return 0;
        else
		  *p = *rname;
      }
...
void resume_domain(struct dns_header *header, size_t qlen, char *rname)
{
  ...
  resume_name(header, qlen, &p, 4,rname);

  return;
} 

/*create domain follow draft-vixie-dnsext-dns0x20*/
void create_newname_follow_bit20(char *cname)
{
  ...
  int i = 0;
  unsigned int ret = 0; 
  unsigned char ran[4] = {0};
  get_random_bytes(ran,sizeof(ran));
  ret = (ran[0] << 24) + (ran[1] << 16) + (ran[2] << 8) + ran[3];

  my_syslog(LOG_INFO, "%s#%d,receive %s,random num = %u", __func__, __LINE__, cname, ret);

  for(i = 1; (i <= 32) && (*cname != 0); cname++)
  {
      if(*cname >= 'a' && *cname <= 'z')
      {
          if(!((ret >>(32-i)) & 0x1))
              ...
              ) 
{
  ...
  if (header->hb4 & HB4_CD)
    ...
    int serverfail = 0;
    if (!(header->hb3 & HB3_RD)) {
        my_syslog(LOG_DEBUG, "got a query with no HB3_RD, name=%s\n", name);
        myflag = F_NO_RD;
    }
#endif
...
#ifdef __SC_BUILD__
    if(serverfail) {
        SET_RCODE(header, SERVFAIL);/*server_fail*/
    } else
#endif 
  if (nxdomain)
    SET_RCODE(header, NXDOMAIN);
