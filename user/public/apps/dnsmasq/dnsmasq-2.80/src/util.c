...
int connect_timeout(int fd, const struct sockaddr *addr, socklen_t addrlen, int wait_time) 
{
    ...
    MY_CFPRINTF("###### connect_timeout wait_time [%d]\n", wait_time);
...
int read_timeout(int fd, void *buf, int size, int wait_time)
{
    ssize_t n = 0;
...
void rand_init()
{
  ...
  if (fd == -1 ||
#ifdef __SC_BUILD__
      !read_write(fd, (unsigned char *)&seed, sizeof(seed), 1, 0) ||
      !read_write(fd, (unsigned char *)&in, sizeof(in), 1, 0)
#else
...
#endif
{
  ...
  for (done = 0; done < size; done += n)
	...
	if (rw)
#ifdef __SC_BUILD__
	  n = read_timeout(fd, &packet[done], (size_t)(size - done), timeout);
#else
