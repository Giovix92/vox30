...
{
	{ "sc-mark-target" , required_argument, 0, MARK_TARGET },
	/* an oldtime messup, we should have always used the scheme
	 * <extension-name>-<option> */
	{ "sc-set-mark"    , required_argument, 0, MARK_SETMARK },
	{ "sc-mark-set"    , required_argument, 0, MARK_SETMARK },
	{ "sc-mark-or"     , required_argument, 0, MARK_ORMARK  },
	{ "sc-mark-and"    , required_argument, 0, MARK_ANDMARK },
	{ "sc-mark-xor"    , required_argument, 0, MARK_XORMARK },
	{ 0 }
};
...
static void print_help()
{
	...
	" --sc-mark-set value     : Set nfmark value\n"
	" --sc-mark-or  value     : Or nfmark with value (nfmark |= value)\n"
	" --sc-mark-and value     : And nfmark with value (nfmark &= value)\n"
	" --sc-mark-xor value     : Xor nfmark with value (nfmark ^= value)\n"
	" --sc-mark-target target : ACCEPT, DROP, RETURN or CONTINUE\n");
}

static void init(struct ebt_entry_target *target)
{
	struct ebt_sc_mark_t_info *markinfo =
	   (struct ebt_sc_mark_t_info *)target->data;
   ...
   struct ebt_entry_target **target)
{
	...
	case MARK_TARGET:
		...
		if (FILL_TARGET(optarg, tmp))
			ebt_print_error2("Illegal --sc-mark-target target");
	...
	case MARK_SETMARK:
		...
		if (*flags & (OPT_MARK_ORMARK|OPT_MARK_ANDMARK|OPT_MARK_XORMARK))
			ebt_print_error2("--sc-mark-set cannot be used together with specific --mark option");
                break;
	case MARK_ORMARK:
		ebt_check_option2(flags, OPT_MARK_ORMARK);
		if (*flags & (OPT_MARK_SETMARK|OPT_MARK_ANDMARK|OPT_MARK_XORMARK))
			ebt_print_error2("--sc-mark-or cannot be used together with specific --mark option");
	...
	case MARK_ANDMARK:
		...
		if (*flags & (OPT_MARK_SETMARK|OPT_MARK_ORMARK|OPT_MARK_XORMARK))
			ebt_print_error2("--sc-mark-and cannot be used together with specific --mark option");
	...
	case MARK_XORMARK:
		...
		if (*flags & (OPT_MARK_SETMARK|OPT_MARK_ANDMARK|OPT_MARK_ORMARK))
			ebt_print_error2("--sc-mark-xor cannot be used together with specific --mark option");
   ...
   unsigned int hookmask, unsigned int time)
{
	...
	if (time == 0 && mark_supplied == 0) {
		...
		ebt_print_error("--sc-mark-target RETURN not allowed on base chain");
}
   ...
   const struct ebt_entry_target *target)
{
	...
	tmp = markinfo->target | ~EBT_VERDICT_BITS;
	printf(" --sc-mark-target %s", TARGET_NAME(tmp));
}

static int compare(const struct ebt_entry_target *t1,
   const struct ebt_entry_target *t2)
{
	struct ebt_sc_mark_t_info *markinfo1 =
	   (struct ebt_sc_mark_t_info *)t1->data;
	struct ebt_sc_mark_t_info *markinfo2 =
	   (struct ebt_sc_mark_t_info *)t2->data;

	return markinfo1->target == markinfo2->target &&
	   markinfo1->mark == markinfo2->mark;
}

static struct ebt_u_target sc_mark_target =
{
	.name		= EBT_SC_MARK_TARGET,
	.size		= sizeof(struct ebt_sc_mark_t_info),
	.help		= print_help,
	.init		= init,
	.parse		= parse,
...
void _init(void)
{
	ebt_register_target(&sc_mark_target);
}
