...
#ifdef __SC_BUILD__
                    log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "client type doesn't match. Calculated sha1 %s for size_type %d\n", hash_str, size_type);
#else
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "No uamsecret, parameter can't be encrypted, set to"
				" zeros instead\n");
#else
...
static void captive_redir(struct captive_thread *thread, enum client_login res,
	...
	char urlbuf_wisp[URLBUFSIZE]; /* A copy of notyet url with res=login instead */
	int urloff_wisp = 0;
...
#ifdef __SC_BUILD__
#else
	syslog(LOG_DEBUG, "Redirect client %i to %s", client_id(cl), urlbuf);
#endif
	captive_destroy(thread);
}
