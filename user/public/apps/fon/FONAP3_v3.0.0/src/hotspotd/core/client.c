...
static int client_apply() {
	...
	eapacct_conf = config_get_string("client", "eapacct_conf", "radeapacct");

	if (auth && !backend) {
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Could not resolve client backend.\n");
#else
...
#ifdef __SC_BUILD__
extern int wl_brcm_get_client_packets(char* if_name, unsigned char *mac, unsigned long *rx_packets, unsigned long *tx_packets);
int sc_traffic_stats(uint16_t classid,
uint64_t *byte_in, uint64_t *byte_out, uint32_t *pkt_in, uint32_t *pkt_out) 
{
    char mac[64] = {0};
    classid -- ;
    snprintf(mac,sizeof(mac),"%02X-%02X-%02X-%02X-%02X-%02X", clients[classid].mac[0], clients[classid].mac[1],
            clients[classid].mac[2], clients[classid].mac[3], clients[classid].mac[4], clients[classid].mac[5]);
    wl_brcm_get_client_packets("wl0.2", (unsigned char *)mac, (unsigned long *)pkt_in, (unsigned long *) pkt_out);
    return 0;
}
#endif
...
#else
	if (!cl->sess.stats_cb)
		cl->sess.stats_cb = &sc_traffic_stats;
#endif
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "LOGIN not equal to SUCCESS status(%d)", status);
#else
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to setup traffic control "
...
#ifdef __SC_BUILD__
        log_wifi(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Logout request of client %i", client_id(cl));
#else
...
#endif
#ifdef __SC_BUILD__
        log_wifi(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Client %u (%02x:%02x:%02x:%02x:%02x:%02x) logout",
...
#ifdef __SC_BUILD__
    log_wifi(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Client %u (%02x:%02x:%02x:%02x:%02x:%02x) removed",
				client_id(cl), cl->mac[0], cl->mac[1],
				cl->mac[2], cl->mac[3], cl->mac[4], cl->mac[5]);
#else
...
struct client* client_register(int ifindex, int af, const void *addr
	...
	if (!cl) {
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Ignoring new client (limit reached)");
#else
