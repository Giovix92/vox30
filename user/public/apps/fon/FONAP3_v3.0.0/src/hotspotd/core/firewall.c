...
#ifdef __SC_BUILD__
	char wifi_iface[IFNAMSIZ];	/* Interface name */
	char wifi_eap_iface[IFNAMSIZ];	/* Interface name */
#endif
...
static int firewall_resident_init() {
	...
	strncpy(cfg.wifi_iface, wifi_iface, sizeof(cfg.wifi_iface) - 1);
	...
	strncpy(cfg.wifi_eap_iface, wifi_eap_iface, sizeof(cfg.wifi_eap_iface) - 1);
#endif
	...
	strcat(chain_input, cfg.wifi_iface);

	strcpy(chain_output, "hotspot_output_");
	strcat(chain_output, cfg.wifi_iface);

	strcpy(chain_service, "hotspot_service_");
	strcat(chain_service, cfg.wifi_iface);

	strcpy(chain_forward, "hotspot_forward_");
	strcat(chain_forward, cfg.wifi_iface);

	strcpy(chain_blocked, "hotspot_blocked_");
	strcat(chain_blocked, cfg.wifi_iface);

	strcpy(chain_reject, "hotspot_reject_");
	strcat(chain_reject, cfg.wifi_iface);

	strcpy(chain_proxy, "hotspot_proxy_");
	strcat(chain_proxy, cfg.wifi_iface);

	strcpy(chain_garden, "hotspot_garden_");
	strcat(chain_garden, cfg.wifi_iface);

	strcpy(chain_prert, "hotspot_prert_");
	strcat(chain_prert, cfg.wifi_iface);

	strcpy(chain_postrt, "hotspot_postrt_");
	strcat(chain_postrt, cfg.wifi_iface);

	strcpy(chain_qos, "hotspot_qos_");
	strcat(chain_qos, cfg.wifi_iface);

	strcpy(chain_auth, "hotspot_auth_");
	strcat(chain_auth, cfg.wifi_iface);
#else
	strcpy(chain_input, "hotspot_input_");
	...
	if (cfg.iface[0]) {
		...
		ipt_filter(IPT_PREPEND, cfg.filter_forward_ipv4,"-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", "DROP");
		ipt_filter(IPT_PREPEND, cfg.filter_input_ipv4,"-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", "DROP");
		ip6t_filter(IPT_PREPEND, cfg.filter_forward_ipv6,"-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", "DROP");
...
static void firewall_resident_deinit() {
	if (cfg.iface[0]) {
		...
		ipt_filter(IPT_DELETE, cfg.filter_input_ipv4,"-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", "DROP");
		ip6t_filter(IPT_DELETE, cfg.filter_forward_ipv6,"-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", "DROP");
...
static int firewall_init() {
	...
	if (ioctl(iosock, SIOCGIFINDEX, &ifr)) {
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Unable to resolve iface: %s\n", ifr.ifr_name);
		close(iosock);
		return -1;
	}
	routing_cfg.wifi_iface_index = ifr.ifr_ifindex;
    close(iosock);
	...
	routing_cfg.wifi_eap_iface_index = ifr.ifr_ifindex;
    close(iosock);
#endif
...
static int firewall_ipv4_deinit() {
...
#ifdef __SC_BUILD__
	ipt_filter(IPT_DELETE, cfg.filter_input_ipv4, "-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_input);
	ipt_filter(IPT_DELETE, cfg.filter_output_ipv4, "-o", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_output);
	ipt_filter(IPT_DELETE, cfg.filter_forward_ipv4, "-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_forward);
	...
	ipt_mangle(IPT_DELETE, cfg.mangle_prerouting_ipv4, "-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_prert);
	ipt_mangle(IPT_DELETE, cfg.mangle_postrouting_ipv4, "-o", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_postrt);
	...
	ipt_nat(IPT_DELETE, cfg.mangle_prerouting_ipv4, "-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-m", "mark", "--mark", buffer, "-j", chain_proxy);
	...
	if (routing_cfg.wifi_eap_iface_index &&
		...
		ipt_filter(IPT_DELETE, cfg.filter_input_ipv4, "-i", routing_cfg.iface_eap_name, 
                "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_eap_iface), "-j", chain_input);
		ipt_filter(IPT_DELETE, cfg.filter_output_ipv4, "-o", routing_cfg.iface_eap_name, 
                "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_eap_iface), "-j", chain_output);
		ipt_filter(IPT_DELETE, cfg.filter_forward_ipv4, "-i", routing_cfg.iface_eap_name, 
                "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_eap_iface), "-j", chain_forward);
		...
		ipt_mangle(IPT_DELETE, cfg.mangle_postrouting_ipv4, "-o", routing_cfg.iface_eap_name, 
                "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_eap_iface), "-j", chain_postrt);
		ipt_mangle(IPT_DELETE, cfg.mangle_prerouting_ipv4, "-i", routing_cfg.iface_eap_name, 
                "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_eap_iface), "-j", chain_prert);
	}
#else
...
static int firewall_ipv4_init() {
	...
	IPT_FILTER(IPT_PREPEND, cfg.filter_input_ipv4, "-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_input);
	IPT_FILTER(IPT_PREPEND, cfg.filter_output_ipv4, "-o", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_output);
	IPT_FILTER(IPT_PREPEND, cfg.filter_forward_ipv4, "-i", cfg.iface,
						"-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_forward);
	...
	if (routing_cfg.wifi_eap_iface_index &&
		...
		IPT_FILTER(IPT_PREPEND, cfg.filter_input_ipv4, "-i", routing_cfg.iface_eap_name, 
                "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_eap_iface), "-j", chain_input);
		IPT_FILTER(IPT_PREPEND, cfg.filter_output_ipv4, "-o", routing_cfg.iface_eap_name, 
                "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_eap_iface), "-j", chain_output);
...
#ifdef __SC_BUILD__
	IPT_MANGLE(IPT_APPEND, chain_prert, "-i", cfg.iface, "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), 
            "-m", "mark", "--mark", buffer, "-m", "state", "--state", "new", "-j", chain_garden);
	IPT_MANGLE(IPT_APPEND, chain_prert, "-i", cfg.iface, "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), 
            "-m", "mark", "--mark", buffer, "-j", chain_proxy);
#else
	IPT_MANGLE(IPT_APPEND, chain_prert, "-i", cfg.iface, "-m", "mark", "--mark", buffer, "-m", "state", "--state", "new", "-j", chain_garden);
	IPT_MANGLE(IPT_APPEND, chain_prert, "-i", cfg.iface, "-m", "mark", "--mark", buffer, "-j", chain_proxy);
#endif
	IPT_MANGLE(IPT_APPEND, chain_prert, "-j", chain_qos);
	snprintf(buffer, sizeof(buffer), "%u", FIREWALL_AUTHMASK | FIREWALL_CLASSMASK);
	IPT_MANGLE(IPT_APPEND, chain_prert, "-j", "CONNMARK", "--save-mark", "--mask", buffer);

	if (!strncmp(routing_cfg.ifb_name, "imq", 3)) {
		IPT_MANGLE(IPT_APPEND, chain_prert, "-j", "IMQ", "--todev", routing_cfg.ifb_name + 3);
		IPT_MANGLE(IPT_APPEND, chain_postrt, "-j", "IMQ", "--todev", routing_cfg.ifb2_name + 3);
	}

	/* postrt */
	snprintf(buffer, sizeof(buffer), "%u", FIREWALL_AUTHMASK | FIREWALL_CLASSMASK);
	IPT_MANGLE(IPT_APPEND, chain_postrt, "-j", "CONNMARK", "--restore-mark", "--mask", buffer);
	IPT_MANGLE(IPT_APPEND, chain_postrt, "-j", chain_qos);
	IPT_MANGLE(IPT_APPEND, chain_postrt, "-j", "CONNMARK", "--save-mark", "--mask", buffer);
#ifndef __SC_BUILD__
	IPT_NAT(IPT_APPEND, chain_postrt, "-j", "MASQUERADE");
#endif
...
#ifdef __SC_BUILD__
	IPT_FILTER(IPT_APPEND, chain_forward, "-o", cfg.iface, "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_iface), "-j", chain_reject);

	if (routing_cfg.wifi_eap_iface_index &&
			routing_cfg.wifi_eap_iface_index != routing_cfg.wifi_iface_index)
		IPT_FILTER(IPT_APPEND, chain_forward, "-o", routing_cfg.iface_eap_name, 
                "-m", "sc_mark", "--sc_mark", _get_fon_if_sc_mark_s(cfg.wifi_eap_iface), "-j", chain_reject);
#else
