...
static int routing_module_init()
{
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Unable to resolve iface: %s", ifr.ifr_name);
#else
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Error setting hotspotd iface hw address\n");
#else
	...
	if (!ioctl(iosock, SIOCGIFINDEX, &ifr)) {
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Resolve eap iface: %s\n", ifr.ifr_name);
#else
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Error trying to resolve eap iface %s",  ifr.ifr_name);
#else
...
static int routing_init() {
	...
	if (ioctl(iosock, SIOCGIFINDEX, &ifr)) {
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Unable to resolve loopback interface\n");
#else
	...
	if (ioctl(iosock, SIOCGIFINDEX, &ifr)) {
		...
		if (ioctl(iosock, SIOCGIFINDEX, &ifr)) {
#ifdef __SC_BUILD__
            log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Unable to resolve ifb interface: %s. "
	...
	if (ioctl(iosock, SIOCGIFINDEX, &ifr)) {
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Unable to resolve secondary ifb interface: "
				"%s", ifr.ifr_name);
#else
...
int routing_policy_new(const char *iface, int af, uint32_t fwmark,
	...
	if (policy < 0) {
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Out of routing policy slots. Leak?\n");
#else
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to setup routing table %i: %s",
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to setup routing policy %i: %s",
			policy + rttbl_offset, strerror(errno));
#else
	...
	if (!s1 && !s2) {
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Created routing policy %i for target %u",
