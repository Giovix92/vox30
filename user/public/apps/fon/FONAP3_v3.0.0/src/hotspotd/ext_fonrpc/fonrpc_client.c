...
#ifdef __SC_BUILD__
       log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "fonsmc: Invalid AES-key\n");
#else
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "fonrpc: heartbeat request time out\n");
#else
		syslog(LOG_WARNING, "fonrpc: heartbeat request time out");
#endif
		if (hb_retry > cfg_threshold && hb_online) {
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "fonrpc: status => offline\n");
#else
...
#ifdef __SC_BUILD__
        log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "fonrpc: unable to send heartbeat: %s\n", strerror(errno));
#else
...
static void fonrpc_receive(struct event_epoll *event, uint32_t revent) {
	...
	while ((len = recv(event->fd, buf, sizeof(buf), MSG_DONTWAIT)) >= 0) {
...
#ifdef __SC_BUILD__
                log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "fonrpc: Access Denied\n");
#else
				syslog(LOG_WARNING, "fonrpc: Access Denied");
#endif
			} else {
#ifdef __SC_BUILD__
                log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "fonrpc: error %s\n",
						strerror(rep->code));
#else
		...
		if (!hb_online) {
#ifdef __SC_BUILD__
            log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "fonrpc: status => online\n");
#else
...
#ifdef __SC_BUILD__
            log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "fonrpc: unknown option %s %s", key, val);
#else
