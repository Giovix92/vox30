    ...
    if (tunnel_type == TUNNEL_GRE) {
        for(i = 0; i < args.idx && i < TUNNEL_LISTCOUNT; i++)
        {
            ...
            else
            {
                ...
                log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel %d is not enabled", i);
                continue;
            }
        }
    }
...
int tunnel_init() {
    ...
    for(i = 0; i < args.idx; i++)
    {
        ...
        if (!strcmp(val, "none")) {
            tunnel_type = TUNNEL_NONE;
            log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "can not parse tunnel type");
            return 0;
    ...
    for (int i = 0; i < TUNNEL_LISTCOUNT; ++i) {
        ...
        if (tunnel_type == TUNNEL_GRE) {
            ...
            if(tunnel_gre_up(tunnelid) < 0)
            {
                ...
                log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel %d can not setup \n", tunnelid);
                return -1;
            }
            else
            {
...
#ifdef __SC_BUILD__
    log_wifi(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Tunnel shutdown %i ",
				tunnel_id );
#else
...
static int tunnel_gre_up(int tunnel_id) {
    ...
    if(!(gre_cfg[tunnel_id].enable))
        return -1;
#ifdef __SC_BUILD__
    log_wifi(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel: Connecting to server %s",
...
static void tunnel_gre_turn_down_ssid(int tunnel_id)
{
    ...
    list_for_each_entry(p, &(gre_cfg[tunnel_id].ssid), _head)
    {
        if(p->is_up == 1)
        {
            ...
            p->is_up = 0;
            log_wifi(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel: %d disable ssid %s ",
...
static int tunnel_gre_up_by_server_index(int tunnel_id, int server_index) 
{
    ...
    {
        log_wifi(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel: there is no server %d for tunnel", server_index, tunnel_id);
    }
    return 0;
}
