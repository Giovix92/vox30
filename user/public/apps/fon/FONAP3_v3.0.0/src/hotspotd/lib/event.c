...
extern void tunnel_deinit(void);
#endif
...
#ifdef __SC_BUILD__
static int bandwidth_check(void)
{
    int ret = -1;
    int min_bw = config_get_int("main", "min_bandwidth", 0);
    int wan_bw = config_get_int("main", "wan_bandwidth", -1);
    if(wan_bw >= (min_bw*1000))
        ret = 0;
    return ret;
}
...
#endif
#ifdef __SC_BUILD__
int event_run(int tunnel_need_setup) {
#else
int event_run() {
#endif
#ifdef __SC_BUILD__
    if(tunnel_need_setup)
    {
        if(bandwidth_check() == 0)
        {
            if(tunnel_init() == 0)
            {
                server_status = HOTSPOT_OK;
            }
            else
            {
                log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, 15, "GRE Tunnel Init failed \n");
                return HOTSPOT_TUNNEL_FAIL;
            }
        }
        else
        {
            log_wifi(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, 15, "GRE Tunnel Init failed due to low bandwidth\n");
            return HOTSPOT_LOWER_BW;
        }
