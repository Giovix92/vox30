...
int hotspot_control(enum hotspot_control cmd) {
	if (cmd == HOTSPOT_CTRL_INIT || cmd == HOTSPOT_RESIDENT_INIT) {
		...
		list_for_each_entry(mod, &mods, _head) {
			...
			if (mod->init()) {
#ifdef __SC_BUILD__
                log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to initialize %s: %s",
		...
		list_for_each_entry_reverse(mod, &mods, _head) {
			...
			|| (cmd != HOTSPOT_RESIDENT_DEINIT && mod->resident))
				continue;
#ifdef __SC_BUILD__
            log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Deinitializing module: %s",
								mod->name);
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to read configuration, check "
...
int main(int argc, char *const *argv) {
	...
	if (getuid() != 0) {
#ifdef __SC_BUILD__
        log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Must have superuser rights to run FON!\n");
#else
...
#ifdef __SC_BUILD__
    log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "FON staring up...\n");
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "unable to initialize FON core\n");
#else
    ...
    do {
        ...
        {
            if(server_status == HOTSPOT_CONFIGRELOAD)
            {
                struct hotspot_module *mod;
                list_for_each_entry(mod, &mods, _head) {
                    if (mod->status && !strncmp(mod->name, "radius_resident",strlen(mod->name)) && mod->resident)
                    {
                        log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Deinitializing module: %s",
                                mod->name);
                        mod->deinit();
