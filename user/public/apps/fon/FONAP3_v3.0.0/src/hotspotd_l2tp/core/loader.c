...
#ifdef __SC_BUILD__
	const char *cfg_iface = config_get_string("main", "wifi_iface", NULL);
#else
...
#ifdef __SC_BUILD__
	const char *cfg_iface_eap = config_get_string("main", "wifi_eap_iface", NULL);
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "hotspot setup failed, rolling back...");
#else
