...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to setup hfsc base classes: %s",
				strerror(errno));
#else
...
static int traffic_init() {
	...
	if (strncmp(routing_cfg.ifb_name, "imq", 3)) {
        ...
        {
            qos_ingress(unl, routing_cfg.wifi_iface_index[i], 0, 0);
            qos_ingress(unl, routing_cfg.wifi_iface_index[i], 0, cl_major);
            if (routing_cfg.wifi_iface_index[i] != routing_cfg.wifi_eap_iface_index[i]) {
                qos_ingress(unl, routing_cfg.wifi_eap_iface_index[i],
                        0, 0);
                qos_ingress(unl, routing_cfg.wifi_eap_iface_index[i],
                        0, cl_major);
            }
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to setup hfsc qdiscs: %s",
							strerror(errno));
#else
    ...
    for(i = 0; i < WIFI_IF_NUM; i++)
    {
        if (qos_ingress(unl, routing_cfg.wifi_iface_index[i], routing_cfg.ifb_index, 0)
                || qos_ingress(unl, routing_cfg.wifi_iface_index[i], routing_cfg.ifb2_index, cl_major)) {
#else
...
#endif
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "traffic: ingress redirect setup failed: %s. "
    ...
    for(i = 0; i < WIFI_IF_NUM; i++)
    {
        ...
        if (qos_ingress(unl, routing_cfg.wifi_eap_iface_index[i], routing_cfg.ifb_index, 0)
                || qos_ingress(unl, routing_cfg.wifi_eap_iface_index[i], routing_cfg.ifb2_index, cl_major)) {
            log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "traffic: ingress redirect setup failed: %s. "
                    "Is the act_connmark kernel module present?", strerror(errno));
            return -1;
        }
	...
	if (unl) {
		...
		if (strncmp(routing_cfg.ifb_name, "imq", 3)) {
            ...
            for(i = 0; i < WIFI_IF_NUM; i++)
            {
                qos_ingress(unl, routing_cfg.wifi_iface_index[i], 0, 0);
                qos_ingress(unl, routing_cfg.wifi_iface_index[i], 0, cl_major);
                qos_ingress(unl, routing_cfg.wifi_eap_iface_index[i], 0, 0);
                qos_ingress(unl, routing_cfg.wifi_eap_iface_index[i], 0, cl_major);
            }
#else
			qos_ingress(unl, routing_cfg.iface_index, 0, 0);
			qos_ingress(unl, routing_cfg.iface_index, 0, cl_major);
...
int traffic_create(uint16_t classid) {
	...
	if (s1 || s2) {
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to setup hfsc user classes: %s",
							strerror(errno));
#else
	...
	if (s1 || s2) {
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to setup tc fw filters: %s",
							strerror(errno));
#else
