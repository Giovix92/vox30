...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "trigger: unable to execute %s: %s\n",
...
#ifdef __SC_BUILD__
    log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "trigger: launched %s (PID %i)", cmd, (int)pid);
#else
...
static void trigger_handle(struct trigger_handle *hndl, short code, short status) {
#ifdef __SC_BUILD__
    log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "trigger: %i exited (status: %i, code: %i)",
