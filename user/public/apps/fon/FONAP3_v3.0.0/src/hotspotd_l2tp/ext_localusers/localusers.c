...
{
#ifdef __SC_BUILD__
    log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "localusers: called prelogin\n");
#else
	...
	if (!fp) {
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "localusers: failed to open %s", cfg_file);
#else
