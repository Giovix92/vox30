...
int private_client_set_mark(struct client *cl, bool set) {
    ...
    {
#ifdef __SC_BUILD__
        log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Private client with mac addr %s marked for accounting", macbuf);
#else
...
#ifdef __SC_BUILD__
    log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Private client %u (%02x:%02x:%02x:%02x:%02x:%02x) unregistered",
	...
	if (iptc_h) {
		...
		if (rule_prert && rule_postrt) {
			...
			*byte_in = rule_postrt->counters.bcnt;
			...
			err = 2;
#ifdef __SC_BUILD__
            log_fon(LOG_ERR, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "%s: couldn't get rules for private user %i", __FUNCTION__, id);
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_ERR, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "%s: couldn't get iptc handler for table %s",__FUNCTION__, table);
#else
...
#ifdef __SC_BUILD__
            log_fon(LOG_ERR, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Unable to get stats for private user %i",
...
#ifdef __SC_BUILD__
            log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Could not resolve private client backend.");
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Ignoring new private client (limit reached)");
#else
...
static void private_handler(uint8_t mac[6], char ip[INET6_ADDRSTRLEN], enum neigh_event nevent)
{
	...
	switch (nevent) { 
		...
		default:
#ifdef __SC_BUILD__
            log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "neigh event not recognized");
#else
