...
static int rpc_sys_status(struct rpc_handle *hndl, struct frmsg *frr) {
    ...
    {
        if(l2tp_status == L2TP_ESTABLISHED || l2tp_status == L2TP_RTX)
            frm_put_string(frm, FRA_SYS_STATUS, "Connected");
        else if(l2tp_status == L2TP_HANDSHAKE || l2tp_status == L2TP_ERROR)
            frm_put_string(frm, FRA_SYS_STATUS, "Error");
        else
            frm_put_string(frm, FRA_SYS_STATUS, "Disconnected");
    }
	frm->frm_seq = frr->frm_seq;
	rpc_send(hndl, frm, frm_length(frm));
	return 1;
