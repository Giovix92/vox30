...
static void service_add_service(const char *name, void *ctx)
{
	...
	if(!serv->str) {
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "no cap strings for service %s",
				serv->name);
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "no port for service %s\n", serv->name);
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "unknown proto for service %s\n", serv->name);
#else
