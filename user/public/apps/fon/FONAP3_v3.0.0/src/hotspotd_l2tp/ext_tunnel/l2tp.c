...
struct l2tp_lac* l2tp_create(const char *host, const struct l2tp_cfg *cfg) {
...
#ifdef __SC_BUILD__
    if(lac->cfg.link_account && strlen(lac->cfg.link_account))
    {
        char buf[256] = "";
        snprintf(buf, sizeof(buf), "%s@%s", lac->cfg.hotspot_mac, lac->cfg.link_account);
        l2tp_msg_avp_string(msg, AVP_CALLING_NUMBER, buf);
    }
    l2tp_msg_avp_string(msg, AVP_CALLED_STATION_ID, lac->cfg.hotspot_mac);
#endif
	...
	if (st) {
#ifdef __SC_BUILD__
       log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "getaddrinfo() failed: %s\n",
						gai_strerror(st));
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Unable to open PPPoX socket, try modprobe l2tp_ppp / pppol2tp?");
#else
...
static int l2tp_icrq(struct l2tp_lac *lac, uint16_t sid) {
    ...
    if(lac->cfg.link_account && strlen(lac->cfg.link_account))
        l2tp_msg_avp_string(msg, AVP_CALLING_NUMBER, lac->cfg.link_account);
