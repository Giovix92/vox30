...
#ifdef __SC_BUILD__
static enum l2tp_state service_status = L2TP_NONE;
#endif
#endif
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel: PPP daemon not found: %s", val);
#else
...
#ifdef __SC_BUILD__
        l2tp_cfg.link_account = config_get_string("tunnel", "l2tp_link_account", NULL);
        /*if(!hotspot_assertconf_string("tunnel.l2tp_link_account", l2tp_cfg.link_account)) {
            return -1;
        }*/
        l2tp_cfg.hotspot_mac = config_get_string("main", "nasid", NULL);
        if(!hotspot_assertconf_string("main.nasid", l2tp_cfg.hotspot_mac)) {
            return -1;
        }
#endif
#endif
...
static int tunnel_open(struct client *cl, client_update_cb *report) {
	...
	if (tunnelid < 0) {
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Out of tunnel slots. Leak?");
#else
...
static void tunnel_shutdown(int tunnelid, int status) {
	...
	pppif_alloc[tunnelid] = NULL;
#ifdef __SC_BUILD__
    log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Tunnel shutdown %i (code %i)",
				tunnelid + 1, status);
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel: LNS connection established\n");
#else
...
#ifdef __SC_BUILD__
        {
            service_status = L2TP_ERROR;
            log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel: No more LNS to try. Giving up.");
        }
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "tunnel: Connecting to LNS %s",
...
static void tunnel_timeout(struct event_timer *timer, int64_t now) {
	for (int i = 0; i < TUNNEL_PPPIFCOUNT; ++i) {
		...
		if (!(tun = pppif_alloc[i]) || tun->timeout > now / 1000)
			continue;
#ifdef __SC_BUILD__
        log_fon(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Tunnel timeout reached for %i", i + 1);
#else
...
{
    int status = l2tp_oldstate;
    if(status == L2TP_NONE && service_status == L2TP_ERROR)
        status = L2TP_ERROR;
    return status;
}
int tunnel_reset_status(void)
{
    service_status = L2TP_NONE;
    return 0;
}
#endif
