...
static void radius_config_server(const char *server, void *cb) {
	...
	if (fd < 0) {
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to connect to RADIUS %s:%s (%s)",
...
const struct radius_pkt *pkt, radius_cb *cb, void *ctx) {
	if (cl->count < 1) {
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "No RADIUS connection found\n");
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "RADIUS queue is full!\n");
#else
...
#ifdef __SC_BUILD__
        log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "%s: Error removing handle from radius queue (handle %d)", 
			__FUNCTION__, handle);
#else
...
static void radius_receiver(struct event_epoll *ev, uint32_t events) {
	...
	while ((len = recv(ev->fd, buffer, sizeof(buffer), MSG_DONTWAIT)) >= 0) {
		...
		if (eap && !authen) {
#ifdef __SC_BUILD__
            log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "RADIUS: discarding unauth'd EAP "
...
#ifdef __SC_BUILD__
                log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "RADIUS: dropped packet "
...
#ifdef __SC_BUILD__
            log_fon(LOG_WARNING, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "RADIUS: Request lost!\n");
#else
