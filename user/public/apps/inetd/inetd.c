  ...
  for(p=servicelist; p!=NULL; p=p->next) {
    ...
    if (p->tcp) {
      ...
      if ((s = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP)) == -1) {
        printf("Unable to create ipv6 socket");
      }
      close_on_exec(s);
      ...
      if(bind(s, (struct sockaddr *)&server_sockaddr, 
        ...
        printf("Unable to bind server ipv6 socket: ");
        close(s);
      }

      if(listen(s, 1) == -1) {
        printf("Unable to listen to ipv6 socket\n");
      }
#endif
#else
      int true;
      ...
      if(bind(s, (struct sockaddr *)&server_sockaddr,
        sizeof(server_sockaddr)) == -1)  {
        printf("Unable to bind ipv6 socket\n");
        close(s);
      }
#endif
#else
