...
#ifdef __SC_BUILD__
static void sal_diag_set_per_report(iperf_size_t BytesReceived, iperf_size_t BytesSent, unsigned int id, unsigned int sender, int packet_cnt)
{
    char tmp_val_s[128] = "";
    char tmp_val_r[128] = "";
    snprintf(tmp_val_r, sizeof(tmp_val_r), "%llu", BytesReceived);
    snprintf(tmp_val_s, sizeof(tmp_val_s), "%llu", BytesSent);
    if(id > 1)
        id--;
    if(sender)
    {
        sal_diag_set_upload_per_total_br(tmp_val_r, id);
        sal_diag_set_upload_per_total_bs(tmp_val_s, id);
        BytesSent -= ((14+20) * packet_cnt);
        snprintf(tmp_val_s, sizeof(tmp_val_s), "%llu", BytesSent);
        sal_diag_set_upload_per_test_bs(tmp_val_s, id);
    }
    else
    {
        sal_diag_set_download_per_total_br(tmp_val_r, id);
        BytesReceived-=((14+20) * packet_cnt);
        snprintf(tmp_val_r, sizeof(tmp_val_r), "%llu", BytesReceived);
        sal_diag_set_download_per_test_br(tmp_val_r, id);
        sal_diag_set_download_per_total_bs(tmp_val_s, id);
    }
}
struct tm *st_report = NULL;
time_t tmp_time = 0;
long usec = 0;
char BOMTime[32] = {0};
char EOMTime[32] = {0};
...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
	...
	{"srcMAC", required_argument, NULL, OPT_SOURCE_MAC},
	{"destMAC", required_argument, NULL, OPT_DEST_MAC},
	{"localip", required_argument, NULL, OPT_LOCAL_IP},
	{"session", required_argument, NULL, OPT_SID},
	{"conMode", required_argument, NULL, OPT_CON_MODE},
#endif
        {NULL, 0, NULL, 0}
    };
    int flag;
...
#ifdef __SC_BUILD__
	    case OPT_IFNAME:
        test->iperf_stream_setting->ifName = strdup(optarg);
#ifdef SC_IPERF_DEBUG 
        iperf_printf(test, "iperf_stream_setting.ifName:%s\n", test->iperf_stream_setting->ifName);
#endif
		break;
	    case OPT_SOURCE_MAC:
        test->iperf_stream_setting->src_mac = strdup(optarg);
#ifdef SC_IPERF_DEBUG 
        iperf_printf(test, "iperf_stream_setting.src_mac:%s\n", test->iperf_stream_setting->src_mac);
#endif
		break;
	    case OPT_DEST_MAC:
        test->iperf_stream_setting->dest_mac = strdup(optarg);
#ifdef SC_IPERF_DEBUG 
        iperf_printf(test, "iperf_stream_setting.src_mac:%s\n", test->iperf_stream_setting->dest_mac);
#endif
		break;
	    case OPT_LOCAL_IP:
        test->iperf_stream_setting->local_ip= strdup(optarg);
#ifdef SC_IPERF_DEBUG 
        iperf_printf(test, "iperf_stream_setting.local:%s\n", test->iperf_stream_setting->local_ip);
#endif
	    case OPT_SID:
        test->iperf_stream_setting->session= unit_atoi(optarg);
		break;
	    case OPT_CON_MODE:
        test->iperf_stream_setting->connection_mode = unit_atoi(optarg);
        break;
#endif
	    case 'h':
		usage_long(stdout);
...
#ifdef __SC_BUILD__
	getIFAdvInfo(test->iperf_stream_setting->ifName,&(test->if_stats_start));
#endif
...
#ifdef __SC_BUILD__
    test->iperf_stream_setting = (struct sc_iperf_stream_setting*) malloc(sizeof(struct sc_iperf_stream_setting));
    if (!test->iperf_stream_setting) {
        free(test->settings);
        free(test);
	i_errno = IENEWTEST;
	return NULL;
    }
    memset(test->iperf_stream_setting, 0, sizeof(struct sc_iperf_stream_setting));
#endif
...
iperf_free_test(struct iperf_test *test)
{
    ...
    free(test->settings);
#ifdef __SC_BUILD__
    if (test->iperf_stream_setting->ifName)
        free(test->iperf_stream_setting->ifName);
    if (test->iperf_stream_setting->src_mac)
        free(test->iperf_stream_setting->src_mac);
    if (test->iperf_stream_setting->dest_mac)
        free(test->iperf_stream_setting->dest_mac);
    if (test->iperf_stream_setting->local_ip)
        free(test->iperf_stream_setting->local_ip);
    if (test->iperf_stream_setting)
        free(test->iperf_stream_setting);
#endif
    if (test->title)
	free(test->title);
    if (test->congestion)
...
iperf_stats_callback(struct iperf_test *test)
{
    ...
    struct kernel_report{
        iperf_size_t    bytes_received_total; 
        iperf_size_t    bytes_received_this_interval; 
        iperf_size_t    packets_received_total;
        iperf_size_t    packets_received_this_interval;
        iperf_size_t    bytes_sent_total; 
        iperf_size_t    bytes_sent_this_interval; 
        iperf_size_t    packets_sent_total;
        iperf_size_t    packets_sent_this_interval;

        iperf_size_t    outoforder_packets;
        iperf_size_t    cnt_error;
    } report[MAX_STREAMS];

    memset(&report, 0, sizeof(struct kernel_report)*MAX_STREAMS);
    util_scDrv_get_iperf_stream_interval(&report);
	getIFAdvInfo(test->iperf_stream_setting->ifName,&(test->if_stats_end));
#endif
...
#ifdef __SC_BUILD__
        if(test->bind_port)
            i = sp->local_port - test->bind_port;
        else
            i = sp->local_port - 9999;
        sp->result->bytes_received = report[i].bytes_received_total;
        sp->result->bytes_sent = report[i].bytes_sent_total;
        sp->outoforder_packets = report[i].outoforder_packets;
        sp->cnt_error = report[i].cnt_error;
        sp->packet_count = test->sender ? report[i].packets_sent_total : report[i].packets_received_total;

        temp.bytes_transferred = test->sender ? report[i].bytes_sent_this_interval : report[i].bytes_received_this_interval;
        temp.bytes_bg_tx = test->if_stats_end.tx_bytes - test->if_stats_start.tx_bytes;
        temp.bytes_bg_rx = test->if_stats_end.rx_bytes - test->if_stats_start.rx_bytes;
#else
...
#ifdef __SC_BUILD__
    memcpy(&(test->if_stats_start), &(test->if_stats_end), sizeof(test->if_stats_start));
#endif
}
...
iperf_print_intermediate(struct iperf_test *test)
{
    ...
    double st = 0., et = 0.;
    char tmp_val[128] = "";
    test->cm_instance_id += 1;
    iperf_size_t bytes_bg_tx = 0;
    iperf_size_t bytes_bg_rx = 0;
#endif
...
#ifdef __SC_BUILD__
        bytes_bg_tx = irp->bytes_bg_tx;
        bytes_bg_rx = irp->bytes_bg_rx;
#endif
	if (test->protocol->id == Ptcp) {
	    if (test->sender && test->sender_has_retransmits) {
...
#ifdef __SC_BUILD__
            snprintf(tmp_val, sizeof(tmp_val), "%llu", bytes);/*summary of all the streams each interval */
            if(test->sender)
            {
                sal_diag_set_upload_ic_test_bs(tmp_val, test->cm_instance_id);
                snprintf(tmp_val, sizeof(tmp_val), "%llu", bytes_bg_rx);
                sal_diag_set_upload_ic_total_br(tmp_val, test->cm_instance_id);
                snprintf(tmp_val, sizeof(tmp_val), "%llu", bytes_bg_tx);
                sal_diag_set_upload_ic_total_bs(tmp_val, test->cm_instance_id);
            }
            else
            {
                sal_diag_set_download_ic_test_br(tmp_val, test->cm_instance_id);
                snprintf(tmp_val, sizeof(tmp_val), "%llu", bytes_bg_tx);
                sal_diag_set_download_ic_total_bs(tmp_val, test->cm_instance_id);
                snprintf(tmp_val, sizeof(tmp_val), "%llu", bytes_bg_rx+bytes);
                sal_diag_set_download_ic_total_br(tmp_val, test->cm_instance_id);
            }

            sp = SLIST_FIRST(&test->streams);
	        irp = TAILQ_LAST(&sp->result->interval_results, irlisthead);
            tmp_time = irp->interval_start_time.tv_sec;
            usec = irp->interval_start_time.tv_usec;
            st_report = localtime(&tmp_time);
            snprintf(BOMTime, sizeof(BOMTime), "%d-%02d-%02dT%02d:%02d:%02d.%06ld",
                     st_report->tm_year+1900, st_report->tm_mon+1, st_report->tm_mday,
                     st_report->tm_hour, st_report->tm_min, st_report->tm_sec, usec);
            if(test->sender)
                sal_diag_set_upload_ic_start_time(BOMTime, test->cm_instance_id);
            else
                sal_diag_set_download_ic_start_time(BOMTime, test->cm_instance_id);

            tmp_time = irp->interval_end_time.tv_sec;
            usec = irp->interval_end_time.tv_usec;
            st_report = localtime(&tmp_time);
            snprintf(EOMTime, sizeof(EOMTime), "%d-%02d-%02dT%02d:%02d:%02d.%06ld",
                     st_report->tm_year+1900, st_report->tm_mon+1, st_report->tm_mday,
                     st_report->tm_hour, st_report->tm_min, st_report->tm_sec, usec);
            if(test->sender)
                sal_diag_set_upload_ic_end_time(EOMTime, test->cm_instance_id);
            else
                sal_diag_set_download_ic_end_time(EOMTime, test->cm_instance_id);
#endif
...
static void
iperf_gather_results(struct iperf_test *test)
{
    iperf_size_t per_test_bytes_sent, test_bytes_sent = 0;
    iperf_size_t per_test_bytes_received, test_bytes_received = 0, test_bytes_real_received = 0;
    double lost_percent = 0.0; 
    int receiver_total_packets = 0, lost_packets = 0;
    char tmp_val[128] = "";
    struct iperf_stream *sp = NULL;
    SLIST_FOREACH(sp, &test->streams, streams)
    {
        tmp_time = sp->result->start_time.tv_sec;
        usec = sp->result->start_time.tv_usec;
        st_report = localtime(&tmp_time);
        snprintf(BOMTime, sizeof(BOMTime), "%d-%02d-%02dT%02d:%02d:%02d.%06ld",
                 st_report->tm_year+1900, st_report->tm_mon+1, st_report->tm_mday,
                 st_report->tm_hour, st_report->tm_min, st_report->tm_sec, usec);

        tmp_time = sp->result->end_time.tv_sec;
        usec = sp->result->end_time.tv_usec;
        st_report = localtime(&tmp_time);
        snprintf(EOMTime, sizeof(EOMTime), "%d-%02d-%02dT%02d:%02d:%02d.%06ld",
                st_report->tm_year+1900, st_report->tm_mon+1, st_report->tm_mday,
                st_report->tm_hour, st_report->tm_min, st_report->tm_sec, usec);
        ...
        if(test->sender)
        {
            sal_diag_set_upload_per_bom_time(BOMTime, sp->id);
            sal_diag_set_upload_per_eom_time(EOMTime, sp->id);
            if(sp->id == 1)
            {
                sal_diag_set_upload_bom_time(BOMTime);
                sal_diag_set_upload_eom_time(EOMTime);
            }

            per_test_bytes_sent = sp->result->bytes_sent;
            receiver_total_packets += sp->peer_packet_count - sp->omitted_packet_count;
            lost_packets += sp->cnt_error - sp->omitted_cnt_error;
            test_bytes_sent += per_test_bytes_sent;
            sal_diag_set_per_report(0, per_test_bytes_sent, sp->id, test->sender, sp->packet_count);
        }
        else
        {
            sal_diag_set_download_per_bom_time(BOMTime, sp->id);
            sal_diag_set_download_per_eom_time(EOMTime, sp->id);
            if(sp->id == 1)
            {
                sal_diag_set_download_bom_time(BOMTime);
                sal_diag_set_download_eom_time(EOMTime);
            }

            per_test_bytes_received = sp->result->bytes_received;
            test_bytes_received += per_test_bytes_received;
            sal_diag_set_per_report(per_test_bytes_received, 0, sp->id, test->sender, sp->packet_count);
        }
    }
        
    if(test->sender)
    {
        snprintf(tmp_val, sizeof(tmp_val), "%llu", test_bytes_sent);
        memset(tmp_val, 0, sizeof(tmp_val));
        lost_percent = 1.0 * lost_packets/receiver_total_packets;
        test_bytes_real_received = test_bytes_sent * (1 - lost_percent);
        snprintf(tmp_val, sizeof(tmp_val), "%llu", test_bytes_real_received);
        sal_diag_set_upload_test_bs(tmp_val);
    }
    else
    {
        snprintf(tmp_val, sizeof(tmp_val), "%llu", test_bytes_received);
        sal_diag_set_download_test_br(tmp_val);
    }
}
/**
...
{
    //iperf_printf(test, "**********in iperf_new_stream**********\n");
    struct iperf_stream *sp;
    ...
    //iperf_printf(test, "**********iperf_new_stream 1**********\n");
    ...
    //iperf_printf(test, "**********iperf_new_stream 2**********\n");
    ...
    //iperf_printf(test, "**********iperf_new_stream done**********\n");
...
#ifdef __SC_BUILD__
        if(test->bind_port)
            sp->local_port = test->bind_port + i - 2;
        else
            sp->local_port = 9999 + i - 2;
#endif
    }
    //iperf_printf(test, "**********iperf_add_stream done**********\n");
}
...
#ifdef __SC_BUILD__
int sc_iperf_send(struct iperf_test *test)
{
    return 0;
}
int sc_iperf_recv(struct iperf_test *test)
{
    return 0;
}
...
//	fgets(buf, sizeof(buf), fh);    /* third line is lan interface*/
	...
	while (fgets(buf, sizeof(buf), fh)) {
	    ...
	    if(strcmp(if_name, name) == 0)
	    {
	        ...
	        sscanf(++s, "%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu%llu",
        	   ...
        	   &stats.rx_multicast_packets, /* missing for <= 1 */
            ...
            if_info->tx_multicast_packets = stats.tx_multicast_packets;
