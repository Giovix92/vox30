...
#ifdef CONFIG_SCM_SUPPORT
" --log-suffix suffix		Suffix log messages with this Suffix.\n\n"
#endif
...
#ifdef CONFIG_SCM_SUPPORT
	{.name = "log-suffix", .id = O_LOG_SUFFIX, .type = XTTYPE_STRING,
	 .flags = XTOPT_PUT, XTOPT_POINTER(s, suffix), .min = 1},
#endif
...
static void LOG_parse(struct xt_option_call *cb)
{
    ...
    case O_LOG_SUFFIX:
		if (strchr(cb->arg, '\n') != NULL)
			xtables_error(PARAMETER_PROBLEM,
				   "Newlines not allowed in --log-suffix");
		break;
#endif
	case O_LOG_TCPSEQ:
...
#ifdef CONFIG_SCM_SUPPORT
    if (strcmp(loginfo->suffix, "") != 0)
		printf(" prefix \"%s\"", loginfo->suffix);
#endif
}
...
static void LOG_save(const void *ip, const struct xt_entry_target *target)
{
    ...
    if (strcmp(loginfo->suffix, "") != 0) {
		printf(" --log-suffix");
		xtables_save_string(loginfo->suffix);
	}
#endif
