...
#ifdef __SC_BUILD__
#define L2TP_CONFIG_FILENAME	"/tmp/ppp/l2tpd.conf"
#define L2TP_EXTRA_CONFIG_FILENAME	"/tmp/ppp/l2tpd-extra.conf"
#else
...
#ifdef __SC_BUILD__
    {
		f = fopen(L2TP_EXTRA_CONFIG_FILENAME, "r");
        if(f != NULL)
        {
            char line[80] = {0};
            char *wan_id_s = "wan_id=";
            char *p;

            fgets(line, sizeof(line)-1, f);
            if((p = strstr(line, wan_id_s)))
            {
                p += strlen(wan_id_s);
                sc_wan_id = atoi(p);
            }
