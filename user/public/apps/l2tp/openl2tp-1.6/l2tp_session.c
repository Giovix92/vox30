...
#ifdef __SC_BUILD__
void write_config_to_proc_l2tp(struct l2tp_tunnel *t, struct l2tp_session *c, __u32 type)
{
    struct l2tp_user_info info;
    // for getting mac&ip
    struct ifreq ifr;
    int sockfd;
    FILE *fp_arp;
    ...
    unsigned char gw_hwaddr[6];
    char gw_ipaddr[32];
    char remote_hwaddr[32];

    unsigned mac_0, mac_1, mac_2, mac_3, mac_4, mac_5;
    ...
    if( info.type )
    {
        info.tid = l2tp_tunnel_peer_id(t);
        info.cid = c->status.peer_session_id;
        strncpy(info.wan_dev_name, WAN_DEV_NAME, IF_NAME_SIZE);

        //local ipaddr and hwaddr
        ...
        strncpy(ifr.ifr_name, WAN_DEV_NAME, sizeof(ifr.ifr_name));
        if( ioctl(sockfd, SIOCGIFHWADDR, &ifr) >= 0 )
            memcpy(info.smac, ifr.ifr_hwaddr.sa_data, 6);
        memset( &ifr, 0, sizeof(ifr));
        strncpy(ifr.ifr_name, WAN_DEV_NAME, sizeof(ifr.ifr_name));
        if( ioctl(sockfd, SIOCGIFADDR, &ifr) >= 0 )
            memcpy(&(info.saddr), ifr.ifr_addr.sa_data+2, 4); // 2 is the port len
        close(sockfd);
        // remote hwaddr
        info.daddr = (__u32)(l2tp_tunnel_get_peer(t))->if_peer_addr.s_addr;
        memset(remote_ipaddr, 0, sizeof(remote_ipaddr));
        inet_ntop(AF_INET, (const void *)(&(l2tp_tunnel_get_peer(t)->if_peer_addr.s_addr)), remote_ipaddr, (socklen_t)sizeof(remote_ipaddr));
        // l2tp server may not directly linked
        
        memset(remote_hwaddr, 0, sizeof(remote_hwaddr));
        ...
        do
        {
            ...
            memset(remote_hwaddr, 0, sizeof(remote_hwaddr));
            fgets(buf, sizeof(buf)-1, fp_arp);

            // if l2tp directly linked
            if( strstr( buf, remote_ipaddr))
            {
                sscanf(buf, "%*s%*s%*s%s", remote_hwaddr);
                sscanf(remote_hwaddr, "%x:%x:%x:%x:%x:%x", &mac_0, &mac_1, &mac_2, &mac_3, &mac_4, &mac_5); 
                //sscanf(remote_hwaddr, "%x:%x:%x:%x:%x:%x", &info.dmac[0], &info.dmac[1], &info.dmac[2], &info.dmac[3], &info.dmac[4], &info.dmac[5]);
                info.dmac[0] = mac_0;
                info.dmac[1] = mac_1;
                info.dmac[2] = mac_2;
                info.dmac[3] = mac_3;
        ...
        fclose(fp_arp);
        if( !(info.dmac[0]|info.dmac[1]|info.dmac[2]|info.dmac[3]|info.dmac[4]|info.dmac[5]) )
        {
            if( !(gw_hwaddr[0]|gw_hwaddr[1]|gw_hwaddr[2]|gw_hwaddr[3]|gw_hwaddr[4]|gw_hwaddr[5]) )
            {
                // error occured, just dont set
                return ;
            }
            info.dmac[0] = gw_hwaddr[0];
            info.dmac[1] = gw_hwaddr[1];
            info.dmac[2] = gw_hwaddr[2];
            info.dmac[3] = gw_hwaddr[3];
            info.dmac[4] = gw_hwaddr[4];
            info.dmac[5] = gw_hwaddr[5];
        }
    }
    else
        ;//just need type

    // write info to /proc/l2tp_plugin_config
    fp_config = fopen(L2TP_CONFIG_PROC_NAME , "w");
    if( !fp_config )
    {
        return ;
    }
    fwrite((const void*)&info, sizeof(info), 1, fp_config);
    fclose(fp_config);
    return;
}
#endif
...
#ifdef __SC_BUILD__
    write_config_to_proc_l2tp(tunnel, session, 1);
#endif
out:
	return;
					...
					uint16_t session_id, struct l2tp_avp_desc *avps)
{
	...
	l2tp_tunnel_log(tunnel, L2TP_PROTOCOL, LOG_INFO, "PROTO: tunl %hu/%hu: CDN received from peer %hu", 
			l2tp_tunnel_id(tunnel), session_id, l2tp_tunnel_peer_id(tunnel));
#ifdef __SC_BUILD__
    log_wan(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "L2TP receive CDN from remote peer.\n");
#endif
#ifdef __SC_BUILD__
    write_config_to_proc_l2tp(tunnel, session, 0);
#endif
...
static int l2tp_session_send_cdn(void *tun, void *sess)
{
	...
	l2tp_tunnel_log(tunnel, L2TP_PROTOCOL, LOG_INFO, "PROTO: tunl %hu/%hu: sending CDN to peer %hu/%hu", 
			...
			session->status.peer_session_id);
#ifdef __SC_BUILD__
    log_wan(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "L2TP send CDN to remote peer.\n");
#endif

	result = l2tp_net_send(tunnel, l2tp_tunnel_peer_id(tunnel), session->status.peer_session_id, pkt, L2TP_AVP_MSG_CDN);
#ifdef __SC_BUILD__
    write_config_to_proc_l2tp(NULL, NULL, 0);
#endif
