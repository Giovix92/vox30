...
#ifdef __SC_BUILD__
    static pppd[32];
    extern int sc_wan_id ;
    snprintf(pppd, sizeof(pppd), "pppd_%d", sc_wan_id);
    argv[arg++] = pppd;
#else
	argv[arg++] = "pppd";
#endif
...
static int ppp_unix_pppd_spawn(struct ppp_context *ppp)
{
	...
	if (pid == 0) { 
        ...
        if(sc_wan_id >= 0)
        {
            static wanid[4] = {0};
            static unit[4] = {0};
            sprintf(unit, "%d", sc_wan_id);
            argv[arg++] = "unit";
            argv[arg++] = unit;
            sprintf(wanid, "%d", sc_wan_id);
            argv[arg++] = "wanid";
            argv[arg++] = wanid;
        }
        argv[arg++] = "ip-up-script";
        argv[arg++] = "/tmp/ppp/vpn_ip_up";
        argv[arg++] = "ip-down-script";
        argv[arg++] = "/tmp/ppp/vpn_ip_down";
#endif
        ...
        if(sc_wan_id >= 0)
        {
            char new_ppp_path[64] = {0};

            sprintf(new_ppp_path, "/tmp/0/%d/pppd_%d", sc_wan_id, sc_wan_id);
            result = execv(new_ppp_path, argv);
        }
        else
        {
