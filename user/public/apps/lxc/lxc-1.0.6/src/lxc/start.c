...
#ifdef __SC_BUILD__
    snprintf(env_name, sizeof(env_name), "EXECENV_INDEX=%s", handler->name);
    putenv(env_name);


    if (putenv("PATH=/mnt/rootfs/sbin:/mnt/rootfs/bin:/sbin:/bin:/usr/sbin:/usr/bin:/usr/local/sbin:/usr/local/bin")) {
		SYSERROR("failed to set environment variable: PATH");
		goto out_warn_father;
	}
    if (putenv("LD_LIBRARY_PATH=/usr/lib:/lib:/mnt/rootfs/lib:/mnt/rootfs/usr/local/lib")) {
		SYSERROR("failed to set environment variable: LD_LIBRARY_PATH");
		goto out_warn_father;
	}
#endif
	close(handler->sigfd);
