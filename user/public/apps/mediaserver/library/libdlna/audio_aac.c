...
audio_profile_guess_aac_priv (AVCodecContext *ac, aac_object_type_t type)
{
  ...
  case AAC_LC_ER:
  {
    if (ac->sample_rate < 8*KHZ || ac->sample_rate > 48*KHZ)
      break;

    if (ac->channels <= 2) /* AAC @ Level 1/2 */
    {
      if (ac->bit_rate <= 320*MBPS)
        return AUDIO_PROFILE_AAC_320;

      if (ac->bit_rate <= 576*MBPS)
  ...
  case AAC_LTP_ER:
  {
    if (ac->sample_rate < 8*KHZ)
      break;

    if (ac->sample_rate <= 48*KHZ)
    {
      if (ac->channels <= 2 && ac->bit_rate <= 576*KBPS)
        return AUDIO_PROFILE_AAC_LTP;

      break;
    }
    else if (ac->sample_rate <= 96*KHZ)
    {
      if (ac->channels <= 6 && ac->bit_rate <= 2880*KBPS)
        return AUDIO_PROFILE_AAC_LTP_MULT5;

      if (ac->channels <= 8 && ac->bit_rate <= 4032*KBPS)
  ...
  case AAC_HE_L3:
  {
    ...
    if (ac->sample_rate <= 24*KHZ) /* HE-AAC @ Level 2 */
    {
      if (ac->channels > 2)
        break;

      if (ac->bit_rate <= 320*KBPS)
        return AUDIO_PROFILE_AAC_HE_L2_320;

      if (ac->bit_rate <= 576*KBPS)
        return AUDIO_PROFILE_AAC_HE_L2;

      break;
    }
    else if (ac->sample_rate <= 48*KHZ)
    {
      /* HE-AAC @ Level 3 */
      if (ac->channels <= 2 && ac->bit_rate <= 576*KBPS)
      ...
      /* HE-AAC @ Level 4/5 */
      if (ac->channels <= 6 && ac->bit_rate <= 1440*KBPS)
  ...
  case AAC_BSAC_ER:
  {
    if (ac->sample_rate < 16*KHZ || ac->sample_rate > 48*KHZ)
      break;

    if (ac->bit_rate > 128*KBPS)
      break;
