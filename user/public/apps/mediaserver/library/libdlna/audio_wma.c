...
//    return ap;
//printf("ac->bit_rate=%d\n",ac->bit_rate);
//printf("ac->sample_rate=%d\n",ac->sample_rate);
  if((ac->channels >2 && ac->channels <= 8) || ac->bit_rate > 383*KBPS)
  	return AUDIO_PROFILE_WMA_PRO;
  else if(ac->bit_rate < 192999){
  	if(ac->bit_rate>=191*KBPS && ac->sample_rate==48*KHZ)
  ...
  {
    if (ac->bit_rate <= 193*KBPS)
    {
      /* WMA Baseline: stereo, up to 48 KHz, up to 192,999 bps */
      if (ac->channels <= 2)
        return AUDIO_PROFILE_WMA_BASELINE;
    }
    else if (ac->bit_rate <= 385*KBPS)
    {
  ...
  else if (ac->sample_rate <= 96*KHZ)
  {
    ...
    if (ac->channels <= 8 && ac->bit_rate <= 1.5*MBPS)
      return AUDIO_PROFILE_WMA_PRO;
  }
#endif  
           ...
           av_codecs_t *codecs)
{
  ...
  case AUDIO_PROFILE_WMA_BASELINE:
  	if(ctx->title && strstr(ctx->title, "WAF"))
  		return &wmafull;
  	else
    return &wmabase;
