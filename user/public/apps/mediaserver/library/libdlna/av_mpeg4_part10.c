...
avc_video_get_profile (AVFormatContext *ctx, AVStream *vs, AVCodecContext *vc)
{
  ...
  if (vc->bit_rate <= 384*KBPS && ctx->bit_rate <= 600*KBPS &&
  ...
  {
    /* QCIF */
    if (vc->bit_rate <= 128*KBPS && ctx->bit_rate <= 256*KBPS)
    {
    ...
    /* CIF15 */
    if (ctx->bit_rate <= 520*KBPS) /* 520 kbps max system bitrate */
      return AVC_VIDEO_PROFILE_BL_CIF15_520;
    if (ctx->bit_rate <= 540*KBPS) /* 540 kbps max system bitrate */
