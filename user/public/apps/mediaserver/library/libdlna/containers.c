...
{
  unsigned char buffer[MPEG_READ_BUFFER+1];
  int fd, i;
  ...
  if(fd<0)
  	return CT_UNKNOWN;
  read (fd, buffer, MPEG_READ_BUFFER); 
  close (fd);
  /* check for MPEG-TS container */
	for (i = 0; i < MPEG_READ_BUFFER; i++) {
	...
	for (i = 0; i < MPEG_READ_BUFFER; i++) {
		if (buffer[i] == MPEG_TS_SYNC_CODE) {
			if (buffer[i + MPEG_TS_PACKET_LENGTH_DLNA] == MPEG_TS_SYNC_CODE) {   
				i+=MPEG_TS_PACKET_LENGTH;  	
				if (buffer[i] == 0x00 && buffer [i+1] == 0x00 &&
						buffer [i+2] == 0x00 && buffer [i+3] == 0x00)
					return CT_MPEG_TRANSPORT_STREAM_DLNA_NO_TS; /* empty timestamp */
				else{
					if (buffer[0] == 0x00 && buffer[1] == 0x00 && buffer[2] == 0x01 && buffer[3] == MPEG_PACK_HEADER)        
						return CT_MPEG_PROGRAM_STREAM;
					else
