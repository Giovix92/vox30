...
int watch_dir (int fd, const char *dirname, long int mask)
{
        ...
        iwr.mask = mask;
        iwr.fd = open(dirname, O_RDONLY);
        if(iwr.fd<0)
                return -1;
        wd = ioctl(fd, INOTIFY_WATCH, &iwr);
        if (wd < 0) { 
                printf("ioctl(fd, INOTIFY_WATCH, {%d, %d}) = %d", iwr.fd, iwr.mask, wd); 
                fflush(stdout);
                perror (" ");
        }
        close (iwr.fd);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 13)
	int status = ignore_wd( inotify_fd, w->wd );
#else	
 ...
 * @internal
 */
void create_watch(int wd, char *filename) {
	if ( wd < 0 || !filename) return;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 13)
		wd = watch_dir(inotify_fd, filenames[i], events );
#else		
...
#define RETURN(A) {\
	...
	if(cur_event != 0)
	{
		...
		//return &event[first_byte++];
		RETURN (&event[cur_event++]);
	}
	...
	rc = read(inotify_fd, &event[0], sizeof(struct inotify_event)*MAX_EVENTS);
	if(rc <= 0)
		return NULL;
	for(rc = 0; rc < MAX_EVENTS; rc++)
	{
		if(event[rc].mask <= 0)
			break;
		event_count++;
