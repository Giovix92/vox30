...
#define CODEC_ID_ADPCM_IMA_QT AV_CODEC_ID_ADPCM_IMA_QT 
...
#define CODEC_ID_WMAPRO AV_CODEC_ID_WMAPRO 
#define CODEC_ID_WMAV1 AV_CODEC_ID_WMAV1 
#define CODEC_ID_WMAV2 AV_CODEC_ID_WMAV2 
...
#ifdef __SC_BUILD__
    int ms=(int)song.song_length%1000;
    if(ms == 0)
    {
        asprintf(&m.duration, "%d:%02d:%02d",
...
int get_bmp_info(const char *pFileName,int *img_width,int *img_height)
{
	int fd=0, len=0, width=0, height=0;
	char buf[27]={0};
	
#ifdef __SC_BUILD__
    if(is_share_path(pFileName) == 0)
        return -1;
#endif
...
int get_gif_info(const char *pFileName,int *img_width,int *img_height)
{
	int fd=0, len=0, width=0, height=0;
	char buf[11]={0};
...
int get_png_info(const char *pFileName,int *img_width,int *img_height)
{
	png_structp png_ptr;
	png_infop info_ptr;
...
int get_tiff_info(const char *pFileName,int *img_width,int *img_height)
{
#ifdef __SC_BUILD__
    if(is_share_path(pFileName) == 0)
        return -1;
...
GetImageMetadata(const char * path, char * name)
{
	...
	if( ends_with(path, ".jpeg")|| ends_with(path, ".jpg"))
	...
	if(image_type!=0) goto no_exifdata;
...
no_exifdata:
	...
	//DEBUG DPRINTF(E_DEBUG, L_METADATA, " * resolution: %dx%d\n", width, height);
		...
		get_png_info(path,&width,&height);
	}else if(image_type==2){
		get_gif_info(path,&width,&height);
	}else if(image_type==3){
		get_bmp_info(path,&width,&height);
	}else if(image_type==4){
		get_tiff_info(path,&width,&height);
	}
	if( !width || !height )
	{
		free_metadata(&m, free_flags);
	...
	if(image_type==0){
		...
		if( width <= 640 && height <= 480 )
			m.dlna_pn = strdup("GIF_SM");
		else if( width <= 1024 && height <= 768 )
			m.dlna_pn = strdup("GIF_MED");
		else
			m.dlna_pn = strdup("GIF_LRG");
	}else if(image_type==3){
		if( width <= 640 && height <= 480 )
			m.dlna_pn = strdup("BMP_SM");
		else if( width <= 1024 && height <= 768 )
			m.dlna_pn = strdup("BMP_MED");
		else
			m.dlna_pn = strdup("BMP_LRG");
	}else if(image_type==4){
		if( width <= 640 && height <= 480 )
			m.dlna_pn = strdup("TIFF_SM");
		else if( width <= 1024 && height <= 768 )
			m.dlna_pn = strdup("TIFF_MED");
		else
			m.dlna_pn = strdup("TIFF_LRG");
	}
...
GetVideoMetadata(const char * path, char * name)
{
	...
	if( vc )
	{
		...
		if( ctx->duration > 0 ) {
            ...
            if(ms==0)
            {
                asprintf(&m.duration,"%d:%02d:%02d", hours, min, sec);
            }
            else{
				         ...
				         (ctx->bit_rate/8 <= 20000000) )
				{
					...
					switch( audio_profile )
					{
						...
						case PROFILE_AUDIO_WMA_FULL:
							off += sprintf(m.dlna_pn+off, "FULL");
							break;
                                        /*Audio bitrate of Content file is "192", DLNA assigns profile is "PROFILE_AUDIO_WMA_BASE",
                                        But dlna profile doesn't classify this case. 
                                        So if video bitrate is "HIGH" , Audio bitrate is "PROFILE_AUDIO_WMA_BASE".
                                        We assign PN value is "WMVHIGH_FULL"*/
						case PROFILE_AUDIO_WMA_BASE:
							off += sprintf(m.dlna_pn+off, "FULL");
							break;
						default:
