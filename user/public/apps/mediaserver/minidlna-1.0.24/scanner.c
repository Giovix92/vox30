...
static int sc_ignore_file_check(const char *d_name)
{
...
{
	return ( (sc_ignore_file_check(d->d_name)) &&
	         ((d->d_type == DT_DIR) ||
	          (d->d_type == DT_LNK) ||
	          (d->d_type == DT_UNKNOWN) ||
		  ((d->d_type == DT_REG) &&
...
void reScanDirectory(const char * dir, enum media_types dir_type)
{
    struct dirent **namelist;
    int i, n;
    char full_path[PATH_MAX];
    char * name = NULL,*check_id;
    static long long unsigned int fileno = 0;
    enum file_types type;
    ...
    for (i=0; i < n; i++)
    {
        ...
        if( (type == TYPE_DIR) && (access(full_path, R_OK|X_OK) == 0) )
        {
            if( (check_id=sql_get_text_field(db,"SELECT ID from DETAILS where PATH='%q'",full_path)) == NULL )
            {
                reinotify_insert_directory(name, full_path);
            }
            reScanDirectory(full_path, dir_type);
        }
        else if( type == TYPE_FILE && (access(full_path, R_OK) == 0) )
        {
            if( (check_id = sql_get_text_field(db,"SELECT ID from DETAILS where PATH='%q'",full_path)) == NULL )
            {
                if( inotify_insert_file(name, full_path) )
                    fileno++;
            }
        }
        free(name);
...
void rescanner()
{
    ...
    while( media_path )
    {
        ...
        reScanDirectory(media_path->path,media_path->type);
...
start_scanner()
{
    ...
    sqlite_int64 id = 0;
#ifdef __SC_BUILD__
   // sal_misc_set_media_status("scanning");//scanning
#endif
	...
	while( media_path )
	{
		...
		sql_exec(db, "UPDATE DETAILS set TIMESTAMP = %d where ID = %lld", media_path->type, id);
    ...
    if(a == 0 && v == 0 && p == 0)
        sal_misc_set_media_status("no_files");//no media files
    else
        sal_misc_set_media_status("working_ok");//working ok
#endif
    return 0;
#endif
}
