...
{
	return (ends_with(file, ".jpg") || ends_with(file, ".jpeg")||ends_with(file, ".png")||ends_with(file, ".gif")||ends_with(file, ".bmp")
...
int is_share_path(const char * s_dir)
{
    char *d_dir = "/mnt/shares/sd";
    char real_path[1024];
    if(!realpath(s_dir,real_path))
        return 0;
    if (!util_is_valid_string(real_path))
        return 0;
    if (-1 == access(real_path,F_OK))
        return 0;
    if (strncmp(real_path,d_dir,strlen(d_dir)) != 0 )
        return 0;
    else
        return 1;
}
