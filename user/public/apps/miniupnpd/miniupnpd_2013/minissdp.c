                    ...
                    if( 2 == i){
                        if(1 == st_ver){
                            for(k = 0;k <DEVICE_ARRAY_SIZE ;++k){
                                if(devicearray[k].ip_addr.s_addr == ((struct sockaddr_in *)sender)->sin_addr.s_addr){
                                    flag_find = 1;
                                    break;
                                }
                            }
                            if(flag_find == 0){
                                if(arraynumber < DEVICE_ARRAY_SIZE){
                                    devicearray[arraynumber].ip_addr.s_addr=((struct sockaddr_in *)sender)->sin_addr.s_addr;
