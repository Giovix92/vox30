...
#ifdef __SC_BUILD__
#ifdef IGD_V2
static int getPeerMacbySocketFd( int sockfd, char *buf) 
{ 
...
#ifdef __SC_BUILD__
                    listen_IP_addr = lan_addr->addr;
                    listen_IP_mask = lan_addr->mask;          
#endif     
                    break;
                case UPNPPORT:
...
#ifdef __SC_BUILD__
                case UPNPEXT_IFID:
                    ext_ifid = atoi(ary_options[i].value);
                    break;
                case UPNPMANUFACTURE:
                    memset(manufacture,0,NAME_MAX_LEN);
                    strncpy(manufacture,ary_options[i].value,NAME_MAX_LEN -1);
                    manufacture[NAME_MAX_LEN -1] = 0;
                    break;
                case UPNPMODULENUMBER:
                    memset(module_number,0,NAME_MAX_LEN);
                    strncpy(module_number,ary_options[i].value,NAME_MAX_LEN -1);
                    module_number[NAME_MAX_LEN -1] = 0;
                    break;
                case UPNPMODE:
                    upnp_mode = atoi(ary_options[i].value);
                    break;
                case UPNPEXT_IFACTYPE:
                    strncpy(ext_if_actype,ary_options[i].value,NAME_MAX_LEN - 1);
                    break;
                case UPNPMODULE_DES:
                    strncpy(module_des,ary_options[i].value,NAME_MAX_LEN - 1);
                    break;
                case UPNPMODULE_NAME:
                    strncpy(module_name,ary_options[i].value,NAME_MAX_LEN - 1);
                    break;
                case UPNPMODULE_MODULE_URL:
                    strncpy(module_url,ary_options[i].value,NAME_MAX_LEN - 1);
                    break;
                case UPNPCONNECTION_TYPE:
                    strncpy(connectionType,ary_options[i].value,NAME_MAX_LEN - 1);
                    break;
#ifdef IGD_V2
                case UPNPIGDMACPREFIX:
                    strncpy(igd1_mac_prefix,ary_options[i].value,9);
                    break;
#endif
...
static int __get_upnp_portmapping(struct ubus_context *ctx, struct ubus_object *obj,
        struct ubus_request_data *req, const char *method,
        struct blob_attr *msg)
{
    static struct blob_buf data;
    ...
    array = blobmsg_open_array(&data, "Security.PortMapping");
    list = get_upnp_rules_state_list(0);
    while(list)
    {
        tmp = list;
        list = tmp->next;
        table = blobmsg_open_table(&data, NULL);
        blobmsg_add_string(&data, "InternalClient", tmp->addr);
        blobmsg_add_u32(&data, "InternalPort", tmp->iport); //lan port
        blobmsg_add_u32(&data, "ExternalPort", tmp->eport); //external port
        blobmsg_add_string(&data, "Potocol", (tmp->proto==IPPROTO_TCP)?"TCP":"UDP"); //TCD UDP
        blobmsg_add_string(&data, "Description", tmp->description); //description
        blobmsg_close_table(&data, table);
        free(tmp);
    }
    blobmsg_close_array(&data, array);

    ubus_send_reply(ctx, req, data.head);
    blob_buf_free(&data);
    return UBUS_STATUS_OK;
}
static void __upnp_port_mapping(void *arg)
{
    static const struct ubus_method methods[] = {
        UBUS_METHOD_NOARG("get", __get_upnp_portmapping),
    };

    static struct ubus_object_type type =
        UBUS_OBJECT_TYPE("v1.Security.PortMapping", methods);

    static struct ubus_object portmapping_obj = {
        .name = "v1.Security.PortMapping",
        .type = &type,
        .methods = methods,
    ...
    if (ubus_add_object(ubus_ctx, &portmapping_obj))
        goto __end;
    uloop_run();
__end:
    ubus_free(ubus_ctx);
...
#ifdef __SC_BUILD__
        log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Starting UPnP IGD Fail.");
#else
...
#ifdef __SC_BUILD__
        log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "HTTP listening on port %d", v.port);
#else
...
#endif
#ifdef __SC_BUILD__
    log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Starting UPnP IGD OK.\n");
#ifdef CONFIG_SUPPORT_UBUS
    pthread_t ubus_id;
    while (0 == pthread_create(&ubus_id, NULL, __upnp_port_mapping, NULL))
        break;
#endif
#endif
...
#ifdef __SC_BUILD__
            sysinfo(&info);
            if(time_after(info.uptime, record_time + 2))     /*delay 2s before sending notify event*/
            {
#endif
#ifdef DEBUG
        ...
        for(e = upnphttphead.lh_first; e != NULL; e = e->entries.le_next)
        {
            if(e->socket >= 0)
            {
                        ...
                        FD_ISSET(e->socket, &writeset))
                {
                    ...
                    getPeerMacbySocketFd(e->socket,mac_addr);
                    for(k = 0;k <DEVICE_ARRAY_SIZE ;++k){
                        if(devicearray[k].ip_addr.s_addr == e->clientaddr.s_addr){
                            if(strlen(devicearray[k].mac_addr) == 0){
                                strcpy(devicearray[arraynumber].mac_addr,mac_addr);
                                flag_find = 1;   
                                break;
                            }else{
                                if(0 == strcmp(mac_addr,devicearray[k].mac_addr)){
                                    flag_find = 1;
                                    break;
                                }else{
                                    devicearray[k].ip_addr.s_addr = 0;
                                    memset(devicearray[k].mac_addr,0,18);
                                    flag_find = 0;
                                    break;
                                }
                            }   
                    ...
                    if(flag_find == 0){
                        if(0 == strncasecmp(mac_addr,igd1_mac_prefix,strlen(igd1_mac_prefix))){
                            if(arraynumber < DEVICE_ARRAY_SIZE){
                                strcpy(devicearray[arraynumber].mac_addr,mac_addr);
                                devicearray[arraynumber].ip_addr.s_addr = e->clientaddr.s_addr;
                                arraynumber++;
                            }else{
                                arraynumber = 0;
                                memset(devicearray[arraynumber].mac_addr,0,18);
                                strcpy(devicearray[arraynumber].mac_addr,mac_addr);
                                devicearray[arraynumber].ip_addr.s_addr = e->clientaddr.s_addr;
                                arraynumber++;
                            }
...
shutdown:
#ifdef CONFIG_SUPPORT_PCPD
    /*delete all pcp port mapping*/
    if(enable_pcpd)
    {
        if(pcp_del_all_port_mapping("upnp") < 0)
        {
            syslog(LOG_ERR, "PCP delete all port mapping failed.");
        }
    }
#endif
    ...
    if (GETFLAG(ENABLEUPNPMASK))
    {
        ...
        if(SendSSDPGoodbye(snotify, addr_count) < 0)
            ...
            {
#ifdef __SC_BUILD__
                log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Failed to broadcast good-bye notifications");
#else
                syslog(LOG_ERR, "Failed to broadcast good-bye notifications");
#endif
            }
#ifdef __SC_BUILD__
        log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Success to broadcast good-bye notifications");
#endif
#ifndef ENABLE_IPV6
        for(i = 0; i < addr_count; i++)
#else
