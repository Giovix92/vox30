	...
	case 2:	/* TCP port mapping request */
		{
			...
			if(lifetime == 0) {
				...
				if(iport == 0) {
                    ...
                    if(enable_pcpd)
                    {
                        int ret = pcp_del_all_port_mapping("upnp");
                    }
#endif
                    ...
                    {
                        int ret = -1;
                        ret = pcp_delete_port_mapping(eport, proto);
                    }
#endif
				...
				if(r==0) {
					if(strcmp(senderaddrstr, iaddr_old)==0
				       && iport==iport_old) {
                        ...
                        {
                            int ret = -1;
                            ret = pcp_refresh_port_mapping(eport, proto, lifetime);
                            refresh = 1;
                        }
#endif
...
#endif
#ifdef CONFIG_SUPPORT_PCPD
                    if(!refresh && enable_pcpd)
                    {
                        int ret = -1;
                        ret = pcp_add_port_mapping("upnp", eport, iport, senderaddrstr, proto, 
                                lifetime, desc);
                    }
#endif
