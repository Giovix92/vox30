...
#ifdef __SC_BUILD__
            sub->timeout = (timeout ? time(NULL) + timeout : 1800);
#else
...
#ifdef __SC_BUILD__
    log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Notify Client:%s UUID=%s Success.", sub->notify->addrstr, sub->uuid);
#endif
	return;
error:
	if(obj->s >= 0)
...
#endif
#ifdef __SC_BUILD__
	log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "%s: sending event notify message to %s:%s",
	       "upnp_event_send", obj->addrstr, obj->portstr);
	log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "%s: msg: %s",
	       "upnp_event_send", obj->buffer + obj->sent);
#endif
