...
#ifdef __SC_BUILD__
static int
checkCallbackURL(struct upnphttp * h, char *url)
#else
static int
...
#endif
{
	...
	if(h->req_CallbackOff <= 0 || h->req_CallbackLen < 8)
		return 0;
#ifdef __SC_BUILD__
    buf = h->req_buf + h->req_CallbackOff;
    while(memcmp(buf, "http://", 7) == 0)
    {
        ipv6 = 0;
        i = 0;
        p = buf + 7;
        if(*p == '[') {
            ...
            while(*p != ']' && i < (sizeof(addrstr)-1)
                    && p < (buf + h->req_CallbackLen))
                addrstr[i++] = *(p++);
            snprintf(url, sizeof(url), "%s", buf);
            break;
        } else{
            if(strchr(p, ':'))
            {
                memccpy(ip, p, ':', sizeof(ip));
                q = strchr(ip, ':');
                if(q)
                    *q = '\0';
                if(scIsIpAddress((unsigned char*)ip)){
                    sprintf(addrstr, "%s", ip);
                    sprintf(url, "%s", "http://");
                    memccpy(url + 7, p, '>', sizeof(url)-7);
                    q = strchr(url, '>');
                    if(q)
                        *q = '\0';
                    struct in_addr addr;
...
#endif
#ifdef __SC_BUILD__
			if(checkCallbackURL(h, valid_url)) {
				sid = upnpevents_addSubscriber(path, valid_url, strlen(valid_url), h->req_Timeout);
#else
...
with HTTP error 412 Precondition Failed. */
			...
			if(renewSubscription(h->req_buf + h->req_SIDOff, h->req_SIDLen,
				...
				if(h->req_Timeout)
                    ...
                    BuildResp2_upnphttp(h, 401, "TIMEOUT", 0, 0);
#else
                BuildResp_upnphttp(h, 0, 0);
#endif
            }
...
#ifdef __SC_BUILD__   
#ifdef IGD_V2
        { ROOTDESC_PATH, genRootDesc_camera},
#endif        
#endif        
		{ ROOTDESC_PATH, genRootDesc},
#ifdef __SC_BUILD__
#ifdef IGD_V2
        { WANIPC_PATH, genWANIPCn_camera},
#endif        
#endif        
...
#ifdef __SC_BUILD__
			h->res_buflen += snprintf(h->res_buf + h->res_buflen,
			                          h->res_buf_alloclen - h->res_buflen,"1800\r\n");
#else
			h->res_buflen += snprintf(h->res_buf + h->res_buflen,
			                          h->res_buf_alloclen - h->res_buflen,
			                          "infinite\r\n");
