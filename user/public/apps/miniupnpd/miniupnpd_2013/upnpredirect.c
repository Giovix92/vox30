              ...
              unsigned int leaseduration)
{
	...
	if(r == 0) {
        ...
        {
            int ret = -1;
            ret = pcp_add_port_mapping("upnp", eport, iport, iaddr, proto, timestamp, desc);
        }
#endif
...
#ifdef __SC_BUILD__
#ifdef CONFIG_SUPPORT_CGN
    sal_wan_set_upnp_port_mapping_t(ext_ifid, "1");
    cpm_update(ext_ifid, EVENT_WAN_ENABLE_CGN);
#endif
#endif
	return 0;
}
...
upnp_delete_redirection(unsigned short eport, const char * protocol)
{
    ...
    {
        int ret = -1;
        ret = pcp_delete_port_mapping(eport, proto_atoi(protocol));
    }
#endif
...
get_upnp_rules_state_list(int max_rules_number_target)
{
	...
	while(get_redirect_rule_by_index(i, /*ifname*/0, &tmp->eport, tmp->addr, sizeof(tmp->addr),
	                              &tmp->iport, &proto, tmp->description, sizeof(tmp->description), 0,0, &timestamp,
