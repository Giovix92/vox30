...
#endif   
#ifdef __SC_BUILD__
struct in_addr  listen_IP_addr, listen_IP_mask ;
static unsigned int convert_ip_to_unsignedint(char *value)
 {
...
static int checkip(char * value)
{
  unsigned int ip1 = convert_ip_to_unsignedint(value);
  unsigned int ip2 = listen_IP_addr.s_addr;
  unsigned int mask = listen_IP_mask.s_addr;
  if(ip1 == ip2)
       return 0;
...
GetConnectionTypeInfo(struct upnphttp * h, const char * action)
{
  ...
  char * p;
  char * s;  
  p = strchr(h->req_buf+h->req_soapActionOff, '#');
  if(p == NULL)
     {
        s = "urn:schemas-upnp-org:service:WANIPConnection:2";
     }
  else
     {
       p = p-1;
       ...
       char body[512];
       int bodylen;
       bodylen = snprintf(body, sizeof(body), resp, s, strlen(connectionType)?connectionType:"IP_Routed");
...
GetNATRSIPStatus(struct upnphttp * h, const char * action)
{
	...
	static const char resp[] =
        ...
        bodylen = snprintf(body, sizeof(body), resp, s);
	UNUSED(action);
	/* 2.2.9. RSIPAvailable
...
AddPortMapping(struct upnphttp * h, const char * action)
{
	...
	struct in_addr result_ip;/*unsigned char result_ip[16];*/ /* inet_pton() */
        ...
        {
              s = "urn:schemas-upnp-org:service:WANIPConnection:2";
              snprintf(resp, sizeof(resp), "<u:AddPortMappingResponse xmlns:u=\"%s\"/>", s);
        }
        else
          {
              p = p-1;
...
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Add Port Mapping: 402 %s %s %hu to %s:%hu Fail.",
			desc ? desc : "null", protocol ? protocol : "null", eport ? eport : "null", int_ip ? int_ip : "null", iport ? iport : "null");
#endif
		SoapError(h, 402, "Invalid Args");
		return;
	}
#ifdef __SC_BUILD__
      if((strcmp(int_ip, "255.255.255.255") != 0) && (inet_addr(int_ip) == INADDR_NONE))   
       {
                SoapError(h, 402, "Invalid Args");
                ClearNameValueList(&data);
                return;
       }
      if((checkip(int_ip) != 1) && (strcmp(int_ip, "255.255.255.255") != 0))
      {
                SoapError(h, 402, "Invalid Args");
                ClearNameValueList(&data);
                return; 
      }
     if((strcmp(int_ip, "255.255.255.255") == 0) && (strcasecmp(protocol, "tcp") == 0)) 
      {
                SoapError(h, 402, "Invalid Args");
                ClearNameValueList(&data);
                return; 
	...
	if(GETFLAG(SECUREMODEMASK))
	{
		...
		{
#ifdef __SC_BUILD__
            log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Add Port Mapping: 718 %s %s %hu to %s:%hu Fail.",
                    desc, protocol, eport, int_ip, iport);
#endif
#ifdef DEBUG
	...
	leaseduration_str = GetValueFromNameValueList(&data, "NewLeaseDuration");
        ...
        if((strcasecmp(protocol, "tcp") != 0) && (strcasecmp(protocol, "udp") != 0)) 
         { 
             SoapError(h, 601, "Invalid Args");
             ClearNameValueList(&data);
             return;
         }
        if(atoi(int_port) < 0 || atoi(int_port) > 65535 || atoi(ext_port) < 0 ||atoi(ext_port) > 65535) 
         { 
             SoapError(h, 402, "Invalid Args");
             ClearNameValueList(&data);
             return;
	...
	case 0:	/* success */
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Add Port Mapping: %s %s %hu to %s:%hu Success.",
			desc, protocol, eport, int_ip, iport);
#endif
		BuildSendAndCloseSoapResp(h, resp, sizeof(resp)-1);
		break;
	...
	default:
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Add Port Mapping: 501 %s %s %hu to %s:%hu Fail.",
			desc, protocol, eport, int_ip, iport);
#endif
...
AddAnyPortMapping(struct upnphttp * h, const char * action)
{
...
#ifdef IGD_V2
    int k;
    for(k=0;k<DEVICE_ARRAY_SIZE;++k){
        if(devicearray[k].ip_addr.s_addr==h->clientaddr.s_addr)
            return;
    }
#endif    
#endif
	...
	if (!int_ip || !ext_port || !int_port)
	{
		ClearNameValueList(&data);
#ifdef __SC_BUILD__
        log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "402, Get Specific Port Mapping Entry Fail.");
#endif
		SoapError(h, 402, "Invalid Args");
		return;
	}
...
GetSpecificPortMappingEntry(struct upnphttp * h, const char * action)
{
	...
	{
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "714, Get Specific Port Mapping Entry: '%s':%s:%s->%s:%u Fail.",
		       r_host ? r_host : "All", protocol, ext_port, int_ip, (unsigned int)iport);
#endif		
	...
	{
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Get Specific Port Mapping Entry: '%s':%s:%s->%s:%u Success.",
		       r_host ? r_host : "All", protocol, ext_port, int_ip, (unsigned int)iport);
#endif
#ifdef DEBUG
...
DeletePortMapping(struct upnphttp * h, const char * action)
{
	...
	unsigned short eport;
        ...
        {
              s = "urn:schemas-upnp-org:service:WANIPConnection:2";
              snprintf(resp, sizeof(resp), "<u:DeletePortMappingResponse xmlns:u=\"%s\"></u:DeletePortMappingResponse>", s);
        }
        else
          {
              p = p-1;
...
#endif
	{
		ClearNameValueList(&data);
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "402, Delete Port Mapping: '%s':%s %s to all Fail.", r_host ? r_host : "All", protocol ? protocol : "null", ext_port ? ext_port : "null");
#endif
		SoapError(h, 402, "Invalid Args");
		return;
	}
        ...
        if(atoi(ext_port) < 0 ||atoi(ext_port) > 65535)
        {
                SoapError(h, 402, "Invalid Args");
                ClearNameValueList(&data);       
                return;
	...
	{
#ifdef __SC_BUILD__
   		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "714, Delete Port Mapping: '%s':%s %s to all Fail.", r_host ? r_host : "All", protocol ? protocol : "null", ext_port ? ext_port : "null");
#endif	
		SoapError(h, 714, "NoSuchEntryInArray");
	}
	else
	{
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Delete Port Mapping: '%s':%s %s to all Success.", r_host ? r_host : "All", protocol ? protocol : "null", ext_port ? ext_port : "null");
#endif
		BuildSendAndCloseSoapResp(h, resp, sizeof(resp)-1);
	}
...
DeletePortMappingRange(struct upnphttp * h, const char * action)
{
	...
	UNUSED(action);
        ...
        {
              s = "urn:schemas-upnp-org:service:WANIPConnection:2";
              snprintf(resp, sizeof(resp), "<u:DeletePortMappingRangeResponse xmlns:u=\"%s\"></u:DeletePortMappingRangeResponse>", s);
        }
        else
          {
              p = p-1;
	...
	if(startport_s == NULL || endport_s == NULL || protocol == NULL) {
        ...
        if(atoi(startport_s) < 0 || atoi(startport_s) > 65535 || atoi(endport_s) < 0 || atoi(endport_s) > 65535)
        {
                SoapError(h, 402, "Invalid Args");          
                ClearNameValueList(&data);
                return; 
...
GetGenericPortMappingEntry(struct upnphttp * h, const char * action)
{
	...
	if(!m_index)
	{
		ClearNameValueList(&data);
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "402, Get Generic Port Mapping Entry Fail.");
#endif
		SoapError(h, 402, "Invalid Args");
		return;
	}
	...
	{
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "713, Get Generic Port Mapping Entry Fail.");
#endif
...
GetListOfPortMappings(struct upnphttp * h, const char * action)
{
	...
	if(startport_s == NULL || endport_s == NULL || protocol == NULL || 
	   number_s == NULL ) {
        ...
        if(atoi(startport_s) < 0 || atoi(startport_s) > 65535 || atoi(endport_s) < 0 || atoi(endport_s) > 65535 ||
           atoi(number_s) < 0 || atoi(number_s) > 65535) 
         {
                 SoapError(h,402,"Invalid Args");
                 ClearNameValueList(&data);
                 return;
...
SetConnectionType(struct upnphttp * h, const char * action)
{
    ...
    static const char  resp[] =
        ...
        "</u:SetConnectionTypeResponse>";
    bodylen = snprintf(body, sizeof(body), resp, id);
#endif
...
#endif
#ifdef __SC_BUILD__
    if(strcmp(get_wan_connection_status_str(ext_if_name),"Disconnected"))
    {
        SoapError(h, 703, "InactiveConection StateRequired");
        return;
    }
    if(!strcmp(connection_type, CAL_WAN_HOST) || !strcmp(connection_type, CAL_WAN_ROUTE) || !strcmp(connection_type, CAL_WAN_IPOE_BRIDGE))
    {
        strncpy(connectionType,connection_type,NAME_MAX_LEN - 1);
    }
    else
    {
        SoapError(h, 703, "InactiveConection StateRequired");
...
{  
    ...
    if(strcmp(connectionType,CAL_WAN_ROUTE))
    {
        SoapError(h, 710, "InvalidConnectionType");
        return;
    }
    COMMAND("/usr/sbin/rc cpm start %d &",ext_ifid);
    int counter = 10; 
    int i = 0;
    while(i < counter)
    {
        if(strcmp(get_wan_connection_status_str(ext_if_name),"Connected"))
            sleep(3);
        else
            break;
        i++;
    ...
    int size = sizeof (prt_sin);

  	if (getpeername (h->socket, (struct sockaddr *) &prt_sin, &size) == 0) {
	     char *sa = inet_ntoa (prt_sin.sin_addr);
	     log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Request to start WAN connection from %s.", sa);
	}
	else
	{
	     log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Request to start WAN connection.");
	}
        bodylen = snprintf(body, sizeof(body), resp, s);
	BuildSendAndCloseSoapResp(h, body, bodylen);
#else	
...
ForceTermination(struct upnphttp * h, const char * action)
{
    ...
    COMMAND("/usr/sbin/rc cpm stop %d &",ext_ifid);
    COMMAND("/usr/sbin/rc wanip stop %d &",ext_ifid);
    int counter = 10; 
    int i = 0;
    while(i < counter)
    {
    ...
    int size = sizeof (prt_sin);
  	...
  	if (getpeername (h->socket, (struct sockaddr *) &prt_sin, &size) == 0) {
		char *sa = inet_ntoa (prt_sin.sin_addr);
	    log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Request to stop WAN connection from %s.", sa);
	}
	else
	{
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Request to stop WAN connection.");
	}
        bodylen = snprintf(body, sizeof(body), resp, s);
	BuildSendAndCloseSoapResp(h, body, bodylen);
#else	
...
QueryStateVariable(struct upnphttp * h, const char * action)
{
	...
	{
#ifdef __SC_BUILD__
        log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "402, Query State Variable: null Fail.");
#endif
		SoapError(h, 402, "Invalid Args");
	}
	...
	{
#ifdef __SC_BUILD__
		log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "404, Query State Variable: %s Fail.", var_name ? var_name : "");
#endif
#ifdef DEBUG
...
#ifdef __SC_BUILD__
    int methodType;//1:readable,0:writable
#endif
}
soapMethods[] =
{
	...
	{ "QueryStateVariable", QueryStateVariable, 1},
	{ "GetTotalBytesSent", GetTotalBytesSent, 1},
	{ "GetTotalBytesReceived", GetTotalBytesReceived, 1},
	{ "GetTotalPacketsSent", GetTotalPacketsSent, 1},
	{ "GetTotalPacketsReceived", GetTotalPacketsReceived, 1},
	{ "GetCommonLinkProperties", GetCommonLinkProperties, 1},
	{ "GetStatusInfo", GetStatusInfo, 1},
	/* WANIPConnection */
	{ "GetConnectionTypeInfo", GetConnectionTypeInfo, 1},
	{ "GetNATRSIPStatus", GetNATRSIPStatus, 1},
	{ "GetExternalIPAddress", GetExternalIPAddress, 1},
	{ "AddPortMapping", AddPortMapping, 2},
	{ "DeletePortMappingRange", DeletePortMappingRange, 2},
	{ "DeletePortMapping", DeletePortMapping, 2},
	{ "GetGenericPortMappingEntry", GetGenericPortMappingEntry, 1},
	{ "GetSpecificPortMappingEntry", GetSpecificPortMappingEntry, 1},
/* Required in WANIPConnection:2 */
	{ "SetConnectionType", SetConnectionType, 2},
	{ "RequestConnection", RequestConnection, 2},
	{ "ForceTermination", ForceTermination, 2},
	{ "AddAnyPortMapping", AddAnyPortMapping, 2},
	{ "GetListOfPortMappings", GetListOfPortMappings, 1},
#ifdef ENABLE_L3F_SERVICE
	/* Layer3Forwarding */
	{ "SetDefaultConnectionService", SetDefaultConnectionService, 2},
	{ "GetDefaultConnectionService", GetDefaultConnectionService, 1},
#endif
#ifdef ENABLE_6FC_SERVICE
	/* WANIPv6FirewallControl */
	{ "GetFirewallStatus", GetFirewallStatus, 1},	/* Required */
	{ "AddPinhole", AddPinhole, 2},				/* Required */
	{ "UpdatePinhole", UpdatePinhole, 2},			/* Required */
	{ "GetOutboundPinholeTimeout", GetOutboundPinholeTimeout, 1},	/* Optional */
	{ "DeletePinhole", DeletePinhole, 2},			/* Required */
	{ "CheckPinholeWorking", CheckPinholeWorking, 1},	/* Optional */
	{ "GetPinholePackets", GetPinholePackets, 1},	/* Required */
#endif
#else
	/* WANCommonInterfaceConfig */
...
ExecuteSoapAction(struct upnphttp * h, const char * action, int n)
{
	...
	if(p)
	{
...
#ifdef __SC_BUILD__
            if((strncmp(p, soapMethods[i].methodName, len) == 0) && (upnp_mode & soapMethods[i].methodType))
#else
        ...
        {
            char *sa = inet_ntoa (prt_sin.sin_addr);
            log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "401, Invalid Action:%s from %s.", p, sa);
        }
        else
        {
            log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "401, Invalid Action: %s.", p);
        }
#endif
	}
#ifdef __SC_BUILD__
	else
	{
		...
		int size = sizeof (prt_sin);
	  	...
	  	if (getpeername (h->socket, (struct sockaddr *) &prt_sin, &size) == 0) {
		     char *sa = inet_ntoa (prt_sin.sin_addr);
	 	     log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "401, Invalid Action:null from %s.", sa);
		}
		else
		{
		    log_upnp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "401, Invalid Action: null.");
		}
	}
#endif
