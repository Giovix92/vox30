...
#include "iconv.h"
#define FROM_CODE 	SC_LOCALE_IOCHARSET
#define TO_CODE 	"UCS-2LE"

/* if *outs if not NULL, we will use it to place the output directly, 
 * but if it is not long enough, we can not realloc a larger memory to
 * retry again for the memory may be static. 
 * if *outs if NULL, we will alloc enough(I hope it will be) memory to 
 * to place the output and will set this new memory to *outs if success. */
		...
		int outs_len)
{
	...
	char *mbs, *mbs_c;
	const char *ucs;
	size_t mbs_len, ucs_length, o, nconv;
	int  eo = EINVAL;
	...
	ucs = (const char *)ins;
	ucs_length = ins_len * sizeof(ntfschar); /* length in Bytes */
	/* prepare mbs */
	mbs = *outs;
	mbs_len = outs_len;
	if (mbs && !mbs_len) {
	...
	cd = libiconv_open(FROM_CODE, TO_CODE);
	if (cd == (iconv_t)-1) {
		eo = errno;
		goto _err0;
	}
	mbs_c = mbs; /* keep mbs pointer */
	nconv = libiconv(cd, (char **)&ucs, &ucs_length, (char**)&mbs_c, &mbs_len);
	eo = errno;
	libiconv_close(cd);
	if (nconv == (size_t)-1) {
		goto _err0;
...
int ntfs_mbstoucs(const char *ins, ntfschar **outs, int outs_len)
{
	...
	ntfschar *ucs;
	char *ucs_c;
	size_t ucs_length, ins_size, o, nconv;
	int eo = EINVAL;
	...
	ucs = *outs;
	ucs_length = outs_len*sizeof(ntfschar); /* length in bytes. */
	if ( ucs && !ucs_length ) {
		errno = ENAMETOOLONG;
		return -1;
	}
	...
	s = ins;
	if ( !ucs ) {
		ucs_length = (ins_size + 1) * sizeof(ntfschar) ; /* in Bytes */
		ucs = ntfs_malloc(ucs_length);
		if (!ucs) {
			return -1;
		}
	}
	cd = libiconv_open(TO_CODE, FROM_CODE);
	if (cd == (iconv_t)-1) {
		eo = errno;
		goto _err0;
	}
	ucs_c = (char *)ucs; /* keep ucs pointer */
	nconv = libiconv(cd, (char **)&s, &ins_size, (char **)&ucs_c, &ucs_length);
	eo = errno;
	libiconv_close(cd);
	if (nconv == (size_t)-1) {
		goto _err0;
	}
	*(ntfschar*)ucs_c = cpu_to_le16(L'\0');
	o = (ntfschar*)ucs_c - ucs; /* in unicode */
	if ( *outs != ucs ) {
		*outs = ucs;
	}
	return o;
