...
#ifdef __SC_BUILD__
      if ( (strlen(arg) > 7) 
                  && ( !strncmp(arg, "ftp://", strlen("ftp://"))
                      || !strncmp(arg, "http://", strlen("http://")) 
                      || !strncmp(arg, "https://", strlen("https://"))) ){
	  char *tmp_file;
	  char *file_basec = xstrdup(arg);
	  char *file_base = basename(file_basec);

	  sprintf_alloc(&tmp_file, "%s/%s", OPKG_CONF_DEFAULT_TMP_DIR_BASE, file_base);
	  err = opkg_download(arg, tmp_file, NULL, NULL);
	  if (err) {
	       opkg_msg(NOTICE, "[lcm-err] Cannot download package %s to %s.\n", arg, tmp_file);
	       free(tmp_file);
     	   free(file_basec);
	       return err;
      }
