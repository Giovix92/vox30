...
args_parse(int argc, char *argv[])
{
	...
	while (1) {
#ifdef __SC_BUILD__
		c = getopt_long_only(argc, argv, "Ad:f:no:p:t:vV:e:",
#else
...
#ifdef __SC_BUILD__
	printf("\t-e			Specify operation timeout.\n");
	printf("\tsha256sum			Calculate sha256 checksum.\n");
#endif
...
#ifdef __SC_BUILD__
    if (conf->offline_root && (0 != conf->offline_root[0]))
        setenv("IPKG_INSTROOT", conf->offline_root, 1);
#endif
	cmd_name = argv[opts++];
    
    if (!strcmp(cmd_name,"sha256sum"))
     {
         aa = file_sha256sum_alloc(argv[opts]);
         snprintf(filename, sizeof(filename), "%s.sha256",argv[opts]);
        if (aa)
        {
           fp = fopen(filename,"w+");
