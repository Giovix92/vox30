...
static option_t Options[] = {
    ...
    { "rp_pppoe_wanid",    o_int, (void *)&wanid,
      "wan id" },
    { "host-uniq",      o_string, (void *)&host_uniq,
      "Host-Uniq in PPPoE tag" },
#else
...
PPPOEConnectDevice(void)
{
    ...
    if(wanid != -1)
    {
        int session_num = atoi(sal_wan_get_con_ppp_session_t(wanid));
        if(session_num)
        {
            unsigned long last_echo = strtoul(sal_wan_get_con_last_lcp_time(wanid), NULL, 10);
            unsigned long now = (unsigned long)get_uptime();
            last_echo = last_echo + lcp_echo_fails*lcp_echo_interval;
            if(time_after(last_echo,now))
            {
                conn->session = session_num;
                char* gw_mac = sal_wan_get_con_gw_mac_t(wanid);
                if(gw_mac && *gw_mac)
                {
                    unsigned int mac[ETH_ALEN];
                    int i = 0;
                    sscanf(gw_mac,"%x:%x:%x:%x:%x:%x",&mac[0],&mac[1],&mac[2],&mac[3],&mac[4],&mac[5]);
                    for (i=0; i<ETH_ALEN; i++) {
                        conn->peerEth[i] = (unsigned char) mac[i];
                    }
                    conn->discoverySocket = openInterface(conn->ifName, Eth_PPPOE_Discovery, conn->myEth);
                    sendPADTFromClient(conn,NULL);
    ...
    /*clean the old tags to "" , you should test the strlen() of the tag to use it*/
    sal_wan_clean_ppp_tags(wanid);
#endif
#endif
	discovery(conn);
	if (conn->discoveryState != STATE_SESSION) {
#ifdef __SC_BUILD__
        log_wan(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "Unable to complete PPPoE Discovery for WAN %s\n", pppd_get_wan_connection_name(wanid));
#else
...
waitInfo(void)
{
    // set the timeout 0, non-block
    waitForPADMandPADN(conn, 0);
}
#endif
/**********************************************************************
