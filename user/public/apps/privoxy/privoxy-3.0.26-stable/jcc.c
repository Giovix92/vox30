...
#endif
{
   ...
   {
	   if(strcmp(http->host,(config->domain_table+loop)->originalhost)==0)
	   {
           snprintf(host,sizeof(host),"%s", (config->domain_table+loop)->replacehost);
		   break;
	   }
   }
   server_connection->host = strdup_or_die(host);
...
static void chat(struct client_state *csp)
{
   ...
   else
   {
      ...
      save_connection_destination(csp->server_connection.sfd,
         http, fwd, &csp->server_connection,csp->config);
#else
      save_connection_destination(csp->server_connection.sfd,
