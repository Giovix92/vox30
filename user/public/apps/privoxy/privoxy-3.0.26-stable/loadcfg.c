...
#define hash_listen_port                 1315717461U /* "listen-port" */
#define hash_domaintable                 1245587716U /* "domaintable" */
#define hash_main_subnet                 261151246U /* "mainsubnet" */
#define hash_main_mask                   10444857U /* "mainmask" */
#endif
#endif
...
#ifdef __SC_BUILD__
#ifdef CONFIG_SUPPORT_WEB_PRIVOXY
   memset(config,0,sizeof(struct configuration_spec));
#endif
#endif
   /*
 ...
 * domaintable
 * *************************************************************************/
		 case hash_domaintable :
             p = strtok(arg,";");
			 if(p != NULL)
			 {
				 num_table = atoi(p);
				 if((config->domain_table == NULL) && (num_table > 0))
				 {
					 config->domain_table = (struct domaintable *)malloc(sizeof(struct domaintable)*num_table);
					 if(config->domain_table == NULL)
					 {
						 break;
					 }
					 memset(config->domain_table,0,sizeof(struct domaintable)*num_table);
					 for(loop = 0; loop < num_table; loop ++)
					 {
						 (config->domain_table+loop)->num = num_table;
					 }
				 }
			 }
			 else
			 ...
			 p = strtok(NULL, ";");
			 if((p != NULL) && (config->domain_table != NULL))
			 {
				 original = p;
				 for(loop = 0; loop < config->domain_table->num; loop ++)
				 {
					 if((config->domain_table+loop)->originalhost == NULL)
					 {
						 (config->domain_table+loop)->originalhost = strdup(original);
						 break;
					 }
				 }
			 }
			 ...
			 if((p != NULL) && (config->domain_table != NULL))
			 {
				 ...
				 for(loop = 0; loop < config->domain_table->num; loop ++)
				 {
					 if((config->domain_table+loop)->replacehost == NULL)
					 {
						 (config->domain_table+loop)->replacehost = strdup(replace);
						 break;
					 }
				 }
			 }
 ...
 * replacehost
 * *************************************************************************/
		 case hash_listen_port :
			 config->listen_port = strdup(arg);

			 break;
         case hash_main_subnet :
             config->main_subnet = strdup(arg);
             break;
         case hash_main_mask :
             config->main_mask = strdup(arg);
             break;
