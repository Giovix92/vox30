 ...
 * Function    :  replace
 *
 * Description :  replace sub by dst.
 *
 * Parameters  :
 *          1  :  src = original 
 *          2  :  sub:need be replaced
 *          3  :  dst:replace
 *
 * Returns     :  
 *                NULL: no sub
 *                pRet: value have been replaced
 *******************************************************************/
#ifdef __SC_BUILD__
...
#ifdef __SC_BUILD__
#ifdef CONFIG_SUPPORT_WEB_PRIVOXY
   char originalhost[512] = {'\0'};
   char *replacehost  = NULL;
   int loop = 0;
   int len = 0;
#endif
#endif
...
#ifdef __SC_BUILD__
#ifdef CONFIG_SUPPORT_WEB_PRIVOXY
			if(filter_server_headers == FILTER_CLIENT_HEADERS)
			{
				if( p->str != NULL )
				{
					char *line = NULL;
					for(loop = 0; loop < csp->config->domain_table->num; loop++ )
					{
                        len = strlen((csp->config->domain_table+loop)->originalhost);
						if( len >= 512)
						{
							break;
						}
						memset(originalhost,0,512);
						snprintf(originalhost,sizeof(originalhost),"%s",(csp->config->domain_table+loop)->originalhost);
						snprintf(originalhost+strlen(originalhost),sizeof(originalhost)-strlen(originalhost),":%s",csp->config->listen_port);
						
						replacehost = (csp->config->domain_table+loop)->replacehost; 
						line = replace((p->str), originalhost, (csp->config->domain_table+loop)->originalhost);			
						if(NULL!=line)
						{
							freez(p->str);
							p->str = line;
...
static jb_err client_host(struct client_state *csp, char **header)
{
   ...
   for(loop = 0; loop < csp->config->domain_table->num; loop++ )
   {
	   if(strcmp(csp->http->host,(csp->config->domain_table+loop)->originalhost)==0)
	   {
		   freez(csp->http->host);
		   csp->http->host = NULL;
		   csp->http->host = strdup_or_die((csp->config->domain_table+loop)->originalhost);
		   csp->http->port = csp->http->ssl ? 443 : 80;
		   freez(csp->http->hostport);
		   csp->http->hostport = NULL;
		   strcat(hostport,csp->http->host);
		   strcat(hostport,":");
		   sprintf(port,"%d",csp->http->port);
		   strcat(hostport,port);
		   csp->http->hostport = strdup_or_die(hostport);
		   
		   q = NULL;
		   q = strstr(csp->http->url, "http://");
		   if (q != NULL)
		   {
			   /* Skip protocol */
			   q += strlen("http://");
		   ...
		   strcat(url,q);
		   freez(csp->http->url);
		   csp->http->url = strdup_or_die(url);
		   flag = 1;
		   break;
	   }
   }
...
static jb_err client_xtra_adder(struct client_state *csp)
{
   ...
   jb_err err;
    ...
    subnet.s_addr = ((inet_addr(csp->ip_addr_str))&(inet_addr(csp->config->main_mask)));
    if(strcmp(inet_ntoa(subnet),csp->config->main_subnet)!=0)
    {
        return JB_ERR_OK;
    }
#endif
