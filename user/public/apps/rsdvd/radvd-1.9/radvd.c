...
main(int argc, char *argv[])
{
    ...
    // if rs not enabled or not follow rules
	// exit(1);
	...
	/* parse args */
#ifdef __SC_BUILD__
#define OPTIONS_STR "d:C:l:m:p:t:u:w:vhcsnD"
#else
...
static int check_wan_ip6(char *wan_ifname)
{
	...
	while (fgets(line, LINE_LENGTH, fp) != NULL) {
		if (strstr(line, wan_ifname)!=NULL && line[39]=='0' ) {
			flog(LOG_INFO, "WAN infetface get a GLOBAL IP from RS\n");
			ret = 0;
			break;
		}
	}
...
timer_handler(void *data)
{
	...
	double next = 0;
    next = atoi(cal_nd_get_if_setting_rtr_solicit_interval(wan_id)) / 1000.0;
	...
	if (check_wan_ip6(iface->Name) == 0) {
		...
		//cal_ipv6_routeradvertisement_ifsetting_AdvDefaultLifetime_set("1800");
		if (other_config_flag == 1) {
			system("/usr/sbin/rc dhcp6c restart > /dev/null 2>&1&");
		}
		...
		flog(LOG_INFO, "%s", "no RA prefix available");
	}
    util_mbug("wan id = %d, init rscount = %d, max rtr solicits = %d\n", wan_id, iface->init_rscount, atoi(cal_nd_get_if_setting_max_rtr_solicits(wan_id)));
	if (iface->init_rscount >= atoi(cal_nd_get_if_setting_max_rtr_solicits(wan_id))) 
    {
		char *pidfile;
		...
		flog(LOG_INFO, "removing %s", pidfile);
        	unlink(pidfile);
		flog(LOG_INFO, "%s", "waiting for valid RA packets");
...
kickoff_adverts(void)
{
	...
	for(iface=IfaceList; iface; iface=iface->next)
	{
		...
		gettimeofday(&iface->last_rs_time, NULL);
