...
#ifdef __SC_BUILD__ 
        log_smb(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME,
            "User %s connected to server from %s succeeded.\n",
            user_info->client.account_name, smbd_server_conn->client_id.addr);
#endif
		return nt_status;
	}
#ifdef __SC_BUILD__ 
    if (nt_status.v == NT_STATUS_WRONG_PASSWORD.v ||
            nt_status.v == NT_STATUS_NO_SUCH_USER.v)
        log_smb(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME,
           "User %s connected to server from %s failed.\n",
           user_info->client.account_name, smbd_server_conn->client_id.addr);
#endif
