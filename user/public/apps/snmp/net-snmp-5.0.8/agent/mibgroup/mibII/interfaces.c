...
#endif
#ifdef __SC_BUILD__
        long_return = (u_long) ifnet.if_inupackets;
#else
...
#endif
#ifdef __SC_BUILD__
        long_return = (u_long) ifnet.if_onupackets;
#else
        long_return = (u_long) 0;       /* XXX */
#endif
#endif
...
Interface_Scan_Init(void)
{
    ...
    conf_if_list   *if_ptr;
#ifdef __SC_BUILD__
    unsigned long   rec_fifo_errors, rec_frame_errors, rec_compressed, rec_multicast;
    unsigned long   snd_fifo_errors, snd_carrier_errors, snd_compressed;
    unsigned long   snd_multicast_packets, rec_multicast_bytes, snd_multicast_bytes, rec_unicast_packets, snd_unicast_packets, rec_broadcast_packets, snd_broadcast_packets, rec_unknown_packets;
    const char     *scan_line_2_6 =
    ...
    if (strstr(line, "unicast")) {
        fgets(line, sizeof(line), devin);	/* eat third line */
        scan_line_to_use = scan_line_2_6;
        DEBUGMSGTL(("mibII/interfaces",
                    "using linux 2.6 kernel /proc/net/dev\n"));
    }
	else if (strstr(line, "compressed")) {
        ...
        if ((scan_line_to_use == scan_line_2_2 &&
             sscanf(stats, scan_line_to_use, &rec_oct, &rec_pkt, &rec_err,
                    ...
                    &coll) != 9) || (scan_line_to_use == scan_line_2_0
                                 ...
                                 || (scan_line_to_use == scan_line_2_6 
                                     && sscanf(stats, scan_line_2_6,
                                   /* Basic statistics */
                                     &rec_oct, &rec_pkt, &rec_err, &rec_drop,
                                     &rec_fifo_errors, &rec_frame_errors,
                                     &rec_compressed, &rec_multicast, 
                                     &snd_oct, &snd_pkt, &snd_err, &snd_drop,
                                     &snd_fifo_errors, &coll,
                                     &snd_carrier_errors, &snd_compressed,       
               
                                  /* extended statistics */
                                     &snd_multicast_packets, &rec_multicast_bytes,
                                     &snd_multicast_bytes,
                                     &rec_unicast_packets, &snd_unicast_packets, 
                                     &rec_broadcast_packets, &snd_broadcast_packets, 
                                     &rec_unknown_packets) != 24)
#endif
      ) {
            if ((scan_line_to_use == scan_line_2_2)
...
#ifdef __SC_BUILD__
        } else if (scan_line_to_use == scan_line_2_6)	
        {
            nnew->if_ipackets = rec_unicast_packets;
            nnew->if_inupackets = rec_multicast + rec_broadcast_packets;
            nnew->if_opackets = snd_unicast_packets;
            nnew->if_onupackets = snd_multicast_packets + snd_broadcast_packets;
            nnew->if_ibytes = rec_oct;
            nnew->if_obytes = snd_oct;
            nnew->if_iqdrops = rec_drop;
            nnew->if_snd.ifq_drops = snd_drop;
...
#ifdef __SC_BUILD__
        strncpy(ifrq.ifr_name, ifname, sizeof(ifrq.ifr_name));
        nnew->if_snd.ifq_len = -1;	/* unknown value */
        if (ioctl(fd, SIOCGIFTXQLEN, &ifrq) >= 0)
        nnew->if_snd.ifq_len = ifrq.ifr_qlen;
#endif
