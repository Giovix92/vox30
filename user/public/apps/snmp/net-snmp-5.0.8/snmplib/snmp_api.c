...
#else
    int recv_version = snmp_parse_version(data, length);
    if(g_version != SNMP_DEFAULT_VERSION)
    {
        if(recv_version == g_version)
            pdu->version = g_version;
        else
            pdu->version = SNMPERR_BAD_VERSION; 
    }
    else
    {
        if (session->version != SNMP_DEFAULT_VERSION) {
...
#ifdef __SC_BUILD__
int
sc_snmp_add_var(netsnmp_pdu *pdu,
             oid * name, size_t name_length, char type, const char *value, size_t value_length)
{
    const char     *cp;
    char           *ecp, *vp;
    int             result = SNMPERR_SUCCESS;
