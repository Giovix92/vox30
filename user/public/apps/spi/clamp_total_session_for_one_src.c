...
static DEFINE_SPINLOCK(clamp_hash_table_lock);
#else
static spinlock_t 	detect_lock = SPIN_LOCK_UNLOCKED;
static spinlock_t 	clamp_hash_table_lock = SPIN_LOCK_UNLOCKED;
#endif
...
void ___sc_remote_destroy_hook(struct nf_conn *ct)
{
	...
	if(!list_empty(&ct->clamp_byremote_ip))
	{
		...
		if(lookup->list_count == 0)
		{
			...
			{
				if(printk_ratelimit())
					DEBUG("something wrong, head is NULL clamping\n");
			}
			...
			{
				if(printk_ratelimit())
					DEBUG("something wrong, did't find this entry from the list\n");
			}
		}
...
{
	spin_lock_bh(&clamp_hash_table_lock);
	___sc_remote_destroy_hook(ct);
	spin_unlock_bh(&clamp_hash_table_lock);
}
unsigned int __sc_detect_total_session_for_one_host_hook(struct sk_buff *skb, struct nf_conn *ct)
{
	...
	if(tuple_pt->src.l3num == PF_INET)
	{
		...
		if(list_empty(&dnat_free_queue))
		{
			...
			if(printk_ratelimit())
				DEBUG("clamp session queue is empty, why????? bug !!!!! total_entry used (%d)\n",atomic_read(&total_dnat));
			return ret;
		}
		...
		if(count_per_src == MAX_SESSION_NUM_FOR_ONE_SRC)
		{
			if(sc_add_block_pattern_hook)
			{
                ...
                LOG_FIREWALL(KERN_WARNING,NORM_LOG,LOG_NONUSE_ID,LOG_NONUSE_BLOCK_TIME,
                        "DoS attack: Too many sessions from source: %u.%u.%u.%u:%hu\n",
...
static int __init clamped_init(void)
{
	int i;
#ifdef SEQ_OPERATION
...
static void __exit clamped_fini(void)
{
	struct nf_conn *ct, *n;
	int bucket;
...
module_init(clamped_init);
module_exit(clamped_fini);
