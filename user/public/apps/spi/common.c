		...
		if (get_queue_elem(q, &tmp_elem) == 0)
		{
			...
			else
			{
				...
				for(;q->rear != q->front;)
				{
			                ...
			                tmp_elem = q->base[q->front] + delta;
					if(time_after_eq(now, tmp_elem))
					{
						dequeue(q, &tmp_elem);
					}
					else
				...
				{
					if(printk_ratelimit())
						DEBUG("has delete old entry and enqueu fail again, why????\n");
				}
				return 0;
			}
		}
