...
void clear_oldest_entry(int type)
{
	...
	for(i = 0; i < HASH_SIZE; i++)
	{
		...
		do {
			if(time_after((curr->timestamp + 10*HZ), now)
	...
	if (curr) {
		...
		else
		{
			...
			else
			{
				...
				{
					if(time_after_eq(now, (curr->ports[curr->queue.front].timestamp + 10 * HZ)))
					{
						/* dequeue */
...
static void __exit psd_fini(void)
{
