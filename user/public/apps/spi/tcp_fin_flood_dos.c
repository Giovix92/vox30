...
#endif
    unsigned long times[FIN_MAX_ENTRY];	/* List of time */
    queue_t queue;
};
...
static void clear_oldest_entry(void)
{
    ...
    for(i = 0; i < HASH_SIZE; i++)
    {
        ...
        if((curr = *(head = &state.hash[i])))
            do {
                // timeout
                if(time_after_eq(now, (curr->timestamp + timeout)))
                {
                    curr->timestamp = 0;
...
#endif
unsigned int __sc_tcp_fin_check_hook(struct sk_buff *skb, u_int8_t pf, u_int8_t protonum)
{
    ...
    if (curr) 
    {
        ...
        if(enqueue_and_check(&curr->queue, now, timeout))
        {
            ...
            if (net_ratelimit())
            {
                ...
                {
                    LOG_FIREWALL(KERN_WARNING,CRIT_LOG,LOG_NONUSE_ID,LOG_NONUSE_BLOCK_TIME,
                            "DoS attack: TCP FIN Flood from source: %u.%u.%u.%u\n",  
                            NIPQUAD(iph->saddr));
                }
            }
            ret = NF_DROP;
    ...
    *head = curr;
    ...
    if(nf_ct_tcp_fin_flood_speed > FIN_MAX_ENTRY)
    {
        timeout = FIN_MAX_ENTRY * HZ / nf_ct_tcp_fin_flood_speed;
        threshhold = FIN_MAX_ENTRY; 
    }
    else
    {
        threshhold = nf_ct_tcp_fin_flood_speed; 
    }
...
static int tcp_fin_init(void)
{
	...
	sc_tcp_fin_check_hook = __sc_tcp_fin_check_hook;

	return 0;
}

void tcp_fin_exit(void)
{
	sc_tcp_fin_check_hook = NULL;
}

module_init(tcp_fin_init);
module_exit(tcp_fin_exit);
