	...
	do {
		...
		if (val) {
			...
			case OPTION_BASE64:
				...
				base64Decode((unsigned char*)val,(unsigned char*)decode_str);
				length = length*3/4 - count;
				if (length > 0) {
					if (length > 254) length = 254;
					memcpy(buffer, decode_str, length);
					buffer[length] = '\0';
					retval = 1;
				}
				break;
...
static int read_lan_port_info(char *const_line, void *arg)
{
    struct lan_port_info_s **lan_port = arg;
    struct lan_port_info_s *cur, *new_lan_port;
    char *line;
    char *port_str;
    char *ip_en_str;
    char *gw_en_str;
    ...
    new_lan_port = malloc(sizeof(struct lan_port_info_s));
    if (!new_lan_port)
    {
        return -1;
    }
    memset(new_lan_port, 0, sizeof(struct lan_port_info_s));

    line = (char *) const_line;

    port_str = strtok(line, " \t");
    new_lan_port->port_bit = atoi(port_str);

    ip_en_str = strtok(NULL, " \t");
    new_lan_port->distribute_ip_enable = atoi(ip_en_str);

    gw_en_str = strtok(NULL, " \t");
    new_lan_port->distribute_gw_enable = atoi(gw_en_str);

    dns_en_str = strtok(NULL, " \t");
    new_lan_port->distribute_dns_enable = atoi(dns_en_str);

    new_lan_port->next = NULL;
    if (*lan_port == NULL)
    {
        *lan_port = new_lan_port;
    }
    else
    {
        cur = *lan_port;
...
static struct config_kw_arr k_arr[MAX_INTERFACES] = {
	...
	{"host_bind", read_str, &(server_config[0].host_bind),	""},
#ifdef __SC_BUILD__
	{"serialnumber", read_str, &(server_config[0].serialnumber), ""},
	{"manufac_oui", read_str, &(server_config[0].manufacture_oui), ""},
	{"product_class", read_str, &(server_config[0].product_class), ""},
#endif
    ...
    {"lanpi_enable", read_u32, &(lan_port_info_enable), "0"},
    {"lan_port_info", read_lan_port_info, &(lan_port_header), ""},
	/*ADDME: static lease */}},
	{/* keyword[14]	handler   variable address		default[20] */
	{
	...
	{"host_bind", read_str, &(server_config[1].host_bind),	""},
#ifdef __SC_BUILD__
    {"serialnumber", read_str, &(server_config[1].serialnumber), ""},
    {"manufac_oui", read_str, &(server_config[1].manufacture_oui), ""},
    {"product_class", read_str, &(server_config[1].product_class), ""},
#endif
    ...
    {"host_bind", read_str, &(server_config[2].host_bind),	""},
	{"vendor_bind", read_str, &(server_config[2].vendor_bind),	""},
#ifdef __SC_BUILD__	
    {"serialnumber", read_str, &(server_config[2].serialnumber), ""},
    {"manufac_oui", read_str, &(server_config[2].manufacture_oui), ""},
    {"product_class", read_str, &(server_config[2].product_class), ""},
#endif
	...
	{"host_bind", read_str, &(server_config[3].host_bind),	""},
#ifdef __SC_BUILD__
    {"serialnumber", read_str, &(server_config[3].serialnumber), ""},
    {"manufac_oui", read_str, &(server_config[3].manufacture_oui), ""},
    {"product_class", read_str, &(server_config[3].product_class), ""},
#endif
	...
	{/* keyword[14]	handler   variable address		default[20] */
	{
	{"server",	read_ip,  &(server_config[4].server),	"192.168.0.1"},
	{"start",	read_ip,  &(server_config[4].start),	"192.168.0.2"},
	{"end",		read_ip,  &(server_config[4].end),		"192.168.0.254"},
	{"interface",	read_str, &(server_config[4].interface),	"br0"},
	{"cds_enable",  read_str, &(server_config[4].cds_enable),	"1"},
	{"vendor_bind", read_str, &(server_config[4].vendor_bind),	""},
	{"host_bind", read_str, &(server_config[4].host_bind),	""},
#ifdef __SC_BUILD__
    {"serialnumber", read_str, &(server_config[4].serialnumber), ""},
    {"manufac_oui", read_str, &(server_config[4].manufacture_oui), ""},
    {"product_class", read_str, &(server_config[4].product_class), ""},
#endif
	{"mac_bind", read_str, &(server_config[4].mac_bind),	""},
#if defined(__SC_BUILD__) && defined(__SC_CONDITIONAL_SERVING_PER_IF__)
    {"brg_port_bind", read_u32, &(server_config[4].brg_port_bind), ""},
#endif
	{"wan_bind", read_str, &(server_config[4].wan_bind),	""},
	{"dns_proxy", read_str, &(server_config[4].dns_proxy),	"1"},
	{"dns_server", read_ip_list, &(server_config[4].dns_server),	""},
	...
	{"lease_file",	read_str, &(server_config[4].lease_file),	"/var/udhcpd4.leases"},
	...
	{"static_lease",read_staticlease, &(server_config[4].static_leases),	""},
	{"pipe",read_pipe_option_info, &(server_config[4].pipe),	""},
	{"vendor_id",read_client_option_info, &(vendor_header),	""},
	{"client_id",read_client_option_info, &(client_header),	""},
	{"user_class_id",read_client_option_info, &(user_class_header),	""},
    {"lanpi_enable", read_u32, &(lan_port_info_enable), "0"},
	...
	{/* keyword[14]	handler   variable address		default[20] */
	{
	{"server",	read_ip,  &(server_config[5].server),	"192.168.0.1"},
	{"start",	read_ip,  &(server_config[5].start),	"192.168.0.2"},
	{"end",		read_ip,  &(server_config[5].end),		"192.168.0.254"},
	{"interface",	read_str, &(server_config[5].interface),	"br0"},
	{"cds_enable",  read_str, &(server_config[5].cds_enable),	"1"},
	{"vendor_bind", read_str, &(server_config[5].vendor_bind),	""},
	{"host_bind", read_str, &(server_config[5].host_bind),	""},
#ifdef __SC_BUILD__
    {"serialnumber", read_str, &(server_config[5].serialnumber), ""},
    {"manufac_oui", read_str, &(server_config[5].manufacture_oui), ""},
    {"product_class", read_str, &(server_config[5].product_class), ""},
#endif
	{"mac_bind", read_str, &(server_config[5].mac_bind),	""},
#if defined(__SC_BUILD__) && defined(__SC_CONDITIONAL_SERVING_PER_IF__)
    {"brg_port_bind", read_u32, &(server_config[5].brg_port_bind), ""},
#endif
	{"wan_bind", read_str, &(server_config[5].wan_bind),	""},
	{"dns_proxy", read_str, &(server_config[5].dns_proxy),	"1"},
	{"dns_server", read_ip_list, &(server_config[5].dns_server),	""},
	...
	{"lease_file",	read_str, &(server_config[5].lease_file),	"/var/udhcpd5.leases"},
	...
	{"static_lease",read_staticlease, &(server_config[5].static_leases),	""},
	{"pipe",read_pipe_option_info, &(server_config[5].pipe),	""},
	{"vendor_id",read_client_option_info, &(vendor_header),	""},
	{"client_id",read_client_option_info, &(client_header),	""},
	{"user_class_id",read_client_option_info, &(user_class_header),	""},
    {"lanpi_enable", read_u32, &(lan_port_info_enable), "0"},
...
static void free_lan_port_info(struct lan_port_info_s *lan_port_info)
{
    struct lan_port_info_s *entry, *entry_next;
...
void reconfig_dhcpd(char *conf_file)
{
    ...
    free_lan_port_info(lan_port_header);
    lan_port_header = NULL;
...
#endif
{
    ...
    fd = fileno(fp);
    util_write_lock(fd, 3);
    for (i = 0; i < server_config[ifid].max_leases; i++) {
		mac_filter = (unsigned char *)leases[ifid][i].chaddr;
		sprintf(mac,"%02x:%02x:%02x:%02x:%02x:%02x",mac_filter[0], mac_filter[1], mac_filter[2], mac_filter[3],mac_filter[4], mac_filter[5]);
		if ((leases[ifid][i].yiaddr != 0)&&(0 != strcmp(mac,"00:00:00:00:00:00"))) {
			if (server_config[ifid].remaining) {
				if (lease_expired(&(leases[ifid][i])))
					lease_time = 0;
				else if(leases[ifid][i].expires != 0xFFFFFFFF)
                    lease_time = leases[ifid][i].expires - curr;
                else
                    lease_time = leases[ifid][i].expires;
...
#endif
{
	...
	unsigned int i = 0;
#ifdef __SC_BUILD__
	struct dhcpOfferedAddr_t lease;
#else
	...
	while (i < server_config[ifid].max_leases && (fread(&lease, sizeof lease, 1, fp) == 1)) {
		...
		if (lease.yiaddr >= server_config[ifid].start && lease.yiaddr <= server_config[ifid].end && 
            (!reservedIp(server_config[ifid].static_leases,lease.yiaddr)) || (lease.yiaddr == getIpByMac(server_config[ifid].static_leases,lease.chaddr)))
        {
