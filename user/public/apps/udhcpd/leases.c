...
#endif
{
	...
	if (oldest) {
        ...
        if(lease != 0xFFFFFFFF)
            ...
            oldest->expires = lease;//over range
#ifdef __SC_BUILD__
...
int lease_expired(struct dhcpOfferedAddr *lease)
{
    ...
    if( now - time_bef_NTP > ((2005-2000)*365*24*60*60) )
    {
    	for (c = 0; c < no_of_ifaces; c++)
		{
            ...
            {
        		if (leases[c][i].yiaddr != 0 && server_config[c].remaining && leases[c][i].expires != 0xFFFFFFFF)
...
#ifdef __SC_ARP__
    if (arpping(addr, server_config[ifid].server, server_config[ifid].arp, chaddr, server_config[ifid].interface) == 0) {
#else
	if (arpping(addr, server_config[ifid].server, server_config[ifid].arp, server_config[ifid].interface) == 0) {
#endif
		temp.s_addr = addr;
#ifdef __SC_BUILD__
        LOG(LOG_WARNING, "Request IP [%s] is in used, reserving it for [%ld] seconds"
