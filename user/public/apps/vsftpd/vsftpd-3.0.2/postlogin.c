...
process_post_login(struct vsf_session* p_sess)
{
  ...
  {
      log_ftp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "User %s, %s successfully logged in with protocol %s", str_getbuf(&p_sess->user_str), str_ip_v6+7, (char *)"SFTP/FTP");
  }	
  else
  {
      log_ftp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "User %s, %s successfully logged in with protocol %s", str_getbuf(&p_sess->user_str), str_ip_v6, (char *)"SFTP/FTP");
  }
#endif
#endif
  while(1)
  {
    ...
    {
#ifdef __SC_BUILD__
        if(str_equal_text(&p_sess->user_str, "GUEST") && tunable_guest_world_readable_only)
        {
            vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
            continue;
        }
...
handle_pasv(struct vsf_session* p_sess, int is_epsv)
{
  ...
  if (!is_ipv6)
  {
        ...
        inet_aton(str_getbuf(&p_sess->remote_ip_str), &raddr);

        if((addr.s_addr & mask.s_addr) != (raddr.s_addr & mask.s_addr))
        {
            if(tunable_ftp_server_wan_address != NULL && strlen(tunable_ftp_server_wan_address) > 0)
               if(tunable_sftp_remote_access_enable && p_sess->control_use_ssl == 1)
                   str_append_text(&s_pasv_res_str, tunable_ftp_server_wan_address);
               else
