...
handle_user_command(struct vsf_session* p_sess)
{
  ...
  if (tunable_userlist_enable)
  {
        ...
        (!located && !tunable_userlist_deny))
    {
      ...
      log_ftp(LOG_INFO, CRIT_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "User %s login failed from %s.\n", str_getbuf(&p_sess->user_str), strncmp(remote_ip,"::ffff:",7)? remote_ip: remote_ip+7);
#else
      log_ftp(LOG_INFO, NORM_LOG, LOG_NONUSE_ID, LOG_NONUSE_BLOCK_TIME, "User %s login failed from %s.\n", str_getbuf(&p_sess->user_str), remote_ip);
#endif
#endif
      check_login_delay();
